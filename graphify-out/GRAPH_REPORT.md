# Graph Report - aulas-senai  (2026-09-05)

## Corpus Check
- 423 files · ~2,597,470 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8142 nodes · 8549 edges · 651 communities (634 shown, 17 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 48 edges (avg confidence: 0.89)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `32a68317`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Currículo Técnico - Informática para Internet
- CLAUDE.md — Pasta REFORCO_LINGUAGENS
- EMENTA — TESTES DE FRONTEND
- 5. CHECKLIST DE AVALIAÇÃO RESPONDIDO
- 3. Conteúdo completo
- Ficha de Cadastro de Produto Customizado — Rio do Sul Mais Tech / SENAI
- Introdução à Tecnologia da Informação e Comunicação
- PLANO DE AULAS: Análise de Dados Aplicada à Gestão
- 1.2 Identificação Detalhada de Atributos
- gerar_apostila.js
- professor-senai — Regras do Projeto
- criar_apostila_ferramentas_digitais.py
- gerar_imagens.py
- RELATÓRIO DE ALINHAMENTO — Aulas vs Ementa
- /graphify
- /graphify
- Introdução à Tecnologia da Informação e Comunicação
- TEXTO DE REFORÇO — MÓDULO 3
- AULA 14: Gráficos e Visualização de Dados
- AULA 16: Criação de Dashboard Prático
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- AULA 12: Tabelas Dinâmicas
- Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação
- Introdução à Tecnologia da Informação e Comunicação
- AULA 10: Funções de Busca e Consulta
- AULA 11: Função SE Avançada e ContSE
- AULA 13: Filtros e Validação de Dados
- AULA 10-15-10-2026.md
- AULA 8: Fórmulas e Funções Essenciais
- AULA 9: Formatação e Apresentação de Dados
- AULA 15: Introdução a Dashboards
- 🎓 1º PERÍODO (SEMESTRE)
- ATIVIDADE-EPISÓDIO 02
- Substituições — Reforço de Linguagens
- TEXTO DE REFORÇO — MÓDULO 4
- AULA 07 — Software de Escritório: Editor de Textos
- Introdução à Tecnologia da Informação e Comunicação
- ATIVIDADE — EPISÓDIO 02
- TEXTO DE REFORÇO — MÓDULO 2
- TEXTO DE REFORÇO — MÓDULO 5
- AULA 02: Razão, Proporção e Regra de Três
- AULA 4: Introdução à Estatística Básica
- AULA 6: Sequência Lógica
- AULA 7: Introdução ao Excel e Navegação Básica
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- dashboard.js
- 📄 Google Docs vs Microsoft Word
- E-MAIL-SENAI — Documentação do Projeto
- ATIVIDADE-EPISÓDIO 01
- AULA 01: Introdução e Conjuntos Numéricos
- AULA 5: Área, Volume e Peso
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- gerar-emails-tela.js
- CLAUDE.md — Pasta FICHA-PRODUTO-MAIS-TECH
- AULA 03 — Apresentações: PowerPoint e Google Slides
- AVALIACOES_CRIADAS — Sistema de Gerenciamento de Notas
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- MÓDULO 2 — FRAÇÕES, DECIMAIS E PORCENTAGEM
- AVALIAÇÃO PRÁTICA
- AULA 02 — Planilhas Eletrônicas: Excel e Google Planilhas
- Relatório de Aulas — Introdução à Comunicação Oral e Escrita
- AULA 3: Porcentagem e Conversão de Unidades
- MateriasManager
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- Conteúdo Programático
- ATIVIDADE — EPISÓDIO 01
- AULA 01 — O que é Tecnologia e Dispositivos Digitais no Cotidiano
- AULA 01 — Editores de Texto: Word e Google Docs
- MÓDULO 4 — MEDIDAS E GRANDEZAS
- MÓDULO 5 — RACIOCÍNIO LÓGICO E RESOLUÇÃO DE PROBLEMAS
- AULA 02 — Sistema Operacional e Organização Digital
- AULA 09 — Editor de Apresentações e Projeto Integrador
- AULA 01-09-10-2026.md
- AULA 04-09-10-2026.md
- TEXTO DE REFORÇO — MÓDULO 1
- MÓDULO 3 — PROPORCIONALIDADE
- Encontro 4 — Cálculo Mental, Estimativas e Problemas Cotidianos (2h)
- AULA 06 — Segurança da Informação
- AULA 02-09-10-2026.md
- AULA 03-09-10-2026.md
- PENDENCIAS-PROFESSOR/package.json
- Verificação de Cobertura: Apostila vs. Ementa
- EMENTA: Análise de Dados Aplicada à Gestão
- gerar_atividade_simples.js
- AULA 02 — Cidadania Digital, Segurança e Impactos Sociais
- 📄 Google Docs vs Microsoft Word
- GOOGLE SLIDES vs MICROSOFT POWERPOINT
- Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação
- redefinir-senha.js
- EMENTA — EXPLORAÇÃO DE CARREIRAS INDUSTRIAIS E TECNOLÓGICAS
- AULA 03 — Hardware vs Software: Diferenças Fundamentais
- MÓDULO 6 — INTRODUÇÃO À ÁLGEBRA
- ATIVIDADE AULA 10 — Avaliação Prática + Teórica
- criar_apostila_reforco_linguagens.py
- AULA 05-14-10-2026.md
- Plano de Aulas — Testes de Frontend
- AULA 16 — Scratch: Projetos Criativos Finais e Apresentação
- 📄 GOOGLE DOCS vs MICROSOFT WORD
- GOOGLE SHEETS vs MICROSOFT EXCEL
- EMENTA DA UNIDADE CURRICULAR
- EMENTA DA UNIDADE CURRICULAR
- Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação
- 📋 Instruções de Uso - Dashboard de Controle de Matérias
- ATIVIDADE AULA 02 — Configuração de Ambiente e Ferramentas
- criar-contas-purelymail.js
- AULA 04 — Sistema Operacional: Interface, Arquivos e Pastas
- Conteúdo
- EMENTA — NOÇÕES DE ELETRICIDADE E CIRCUITOS BÁSICOS
- Encontro 13-14 — Pontuação e Seu Uso Comunicativo (4h)
- AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação
- PLANO DE AULAS — Introdução à Tecnologia da Informação e Comunicação
- AULA 07 — Software de Escritório: Editor de Textos
- ATIVIDADE AULA 03 — Testes Unitários em JavaScript (Parte 1)
- ENCONTRO 18 — Projeto Final: Pitch de Negócio
- ENCONTROS
- AULA 05 — Produtividade Digital: Editores de Texto e Processadores de Palavras
- AULA 06 — Produtividade Digital: Apresentações (Google Slides/PowerPoint)
- AULA 07 — Produtividade Digital: Planilhas Eletrônicas (Excel/Google Sheets)
- AULA 08 — Internet: Navegadores e Boas Práticas de Pesquisa
- AVALIAÇÃO FINAL — Fundamentos da Tecnologia e Programação
- Apostila_Comunicacao_15h_Versao_Joao_Revisado_2026_02.md
- 📋 ATIVIDADES MANUAIS — ENTREGA OBRIGATÓRIA
- Conteúdo
- Passos
- Encontro 18-19 — Debate e Argumentação (4h)
- ATIVIDADE AULA 04 — Testes Unitários em JavaScript (Parte 2)
- 2.2 Análise por Entidade
- Ementa — Competências Socioemocionais e Empreendedorismo
- AULA 09 — Segurança na Internet: Senhas, Vírus e Privacidade
- GOOGLE DOCS
- GOOGLE DRIVE
- GOOGLE GMAIL
- Status Geral
- GOOGLE MEET
- GOOGLE SHEETS
- GOOGLE SLIDES
- Passos
- Status Geral
- MICROSOFT EXCEL
- MICROSOFT OUTLOOK
- MICROSOFT POWERPOINT
- MICROSOFT WORD
- MICROSOFT ONEDRIVE
- MICROSOFT TEAMS
- ENCONTRO 7 — Comunicação digital e segurança
- EMENTA — OFICINAS DE IMPRESSÃO 3D E ROBÓTICA
- Encontro 25-26 — Leitura Crítica e Análise de Discurso (4h)
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- AULA 07 — Software de Escritório: Editor de Textos
- AULA 07-14-10-2026.md
- AULA 09-15-10-2026.md
- links-turmas.js
- AULA 10 — Pensamento Computacional: Decomposição e Padrões
- AULA 11 — Algoritmos: Sequência, Decisão e Repetição
- AULA 14 — Scratch: Primeiros Projetos e Movimentação
- AULA 15 — Scratch: Condições, Variáveis e Lógica
- Conhecimentos
- Diário de Classe — Aulas até 01/09/2026
- Técnicas de diagnóstico
- Conteúdo
- ATIVIDADE AULA 01 — Fundamentos de Testes de Software
- 1. Tarefa Principal — Testar Componente de Formulário
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- AULA 06-14-10-2026.md
- AULA 08-14-10-2026.md
- Plano de Aulas — Testes de Frontend
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- api.js
- login.js
- LineForWriting
- FUNDAMENTOS DA TECNOLOGIA E PROGRAMAÇÃO
- AULA 12 — Fluxogramas e Representação Visual de Algoritmos
- 📋 Tarefas Nesta Pasta
- 📋 Tarefas Nesta Pasta
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- 📊 VERSAO-CLAUDE — Infográficos Gerados
- Status Geral
- 📋 Tarefas Nesta Pasta
- 📋 Tarefas Nesta Pasta
- 📊 VERSAO-CLAUDE — Infográficos Gerados
- 3.1 Falar em público sem travar
- ENCONTRO 4 — Reuniões, atendimento e comunicação assertiva
- Conteúdo
- Encontro 1-2 — Tipos de Texto e Contexto de Leitura (4h)
- MÓDULO 7 — TRATAMENTO DA INFORMAÇÃO
- AULA 07 — Software de Escritório: Editor de Textos
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- Status Geral
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- Design: Botão "Analise Ementas" no Modal de Aulas
- 3.2 Análise de Dependências Parciais
- DOCUMENTO TÉCNICO COMPLETO
- Conteúdo
- ENCONTRO 15 — Síntese, Portfólio e Preparação para Continuidade
- ENCONTRO 5 — Redação técnica: princípios e estrutura
- 📚 Conteúdo Programático
- Conteúdo
- Conteúdo
- Conteúdo
- Encontro 10-11 — Coesão Textual: Conectivos e Progressão (4h)
- EMENTA — REFORÇO MATEMÁTICA E RACIOCÍNIO LÓGICO
- AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação
- CRITÉRIOS DE CORREÇÃO — Avaliação Prática
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- 1. Tarefa Principal — Testar Componente com Fetch
- 1. Tarefa Principal — Suite E2E com Page Object Model
- 1. Tarefa Principal — Suite E2E Expandida
- Documentação Padrão
- Documentação Padrão
- ATIVIDADE 05: Aplicação de Normalização até 3ª Forma Normal
- Apostila_Competencias_Socioemocionais_Empreendedorismo.md
- Status Geral
- Status Geral
- Conteúdo
- Conteúdo
- Status Geral
- Conteúdo
- Conteúdo
- Conteúdo
- EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS/gerar_slides.js
- LineForWriting
- 🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais
- 🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais
- ENCONTRO 10 — Aprofundamento: Redação Profissional
- ENCONTRO 1 — Comunicação no mundo do trabalho
- ENCONTRO 2 — Registro adequado e escuta ativa
- ENCONTRO 6 — Gêneros profissionais: comunicado, relato e ata
- 📚 Conteúdo Programático
- Apostila_Nocoes_Eletricidade_Circuitos_Basicos.md
- Conteúdo
- Conteúdo
- Conteúdo
- NOCOES_ELETRICIDADE_CIRCUITOS_BASICOS/gerar_slides.js
- Conteúdo
- Conteúdo
- Conteúdo
- Conteúdo
- Conteúdo
- Apostila_Reforco_Linguagens.md
- REFORCO_LINGUAGENS/gerar_slides.js
- Status Geral
- Status Geral
- Status Geral
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- ATIVIDADE AULA 09 — CI/CD e Boas Práticas
- Global Constraints
- LISTAS — Contatos para importação no Resend
- E-MAIL SENAI
- 4.2 Identificação de Dependências Transitivas
- Status Geral
- ENCONTRO 11 — Identificando Oportunidades de Negócio
- Status Geral
- ENCONTRO 1 — O que é ser empreendedor?
- ENCONTRO 4 — Comunicação, Liderança e Trabalho em Equipe
- ENCONTRO 5 — Sustentabilidade e os 5R's
- ENCONTRO 12 — Planejamento e Estratégia Empreendedora
- compactar_atividade_ferramentas_digitais_facil.py
- ENCONTRO 8 — Carreiras em TI II: Dados, IA e Segurança da Informação
- Status Geral
- ENCONTRO 15 — Entrevista Simulada: Como Se Apresentar
- ENCONTRO 13 — LinkedIn e Redes Profissionais
- ENCONTRO 16 — SENAI: Caminhos de Formação Técnica
- ENCONTRO 7 — Carreiras em TI I: Desenvolvimento de Software
- Apostila Ferramentas Digitais para Comunicação
- Atividade fácil de Ferramentas Digitais
- Atividades de Ferramentas Digitais
- Campo Ensalado nas matérias
- Compactar atividade fácil de Ferramentas Digitais
- 🔴 COMPARATIVOS — Infográficos de Comparação
- 🔴 COMPARATIVOS — Infográficos de Comparação
- ENCONTRO 14 — Aprofundamento: Segurança Digital e Conformidade
- Excluir matérias do curso
- ENCONTRO 8 — Desafio integrador e avaliação
- ENCONTRO 9 — Aprofundamento: Atendimento ao Cliente
- Manter modal do curso ao ver aulas
- 📌 AVALIAÇÃO FINAL INTEGRADA
- 🧠 Banco de Questões de Estudo
- ENCONTRO 4 — LEI DE OHM: A EQUAÇÃO QUE EXPLICA TUDO
- ENCONTRO 14 — MOTORES, VENTILADORES E MOTO-BOMBAS
- ENCONTRO 2 — GRANDEZAS ELÉTRICAS: TENSÃO E CORRENTE
- ENCONTRO 3 — GRANDEZAS ELÉTRICAS: RESISTÊNCIA E POTÊNCIA
- ENCONTRO 5 — INSTRUMENTOS DE MEDIÇÃO
- ENCONTRO 7 — DIAGRAMAS ELÉTRICOS
- ENCONTRO 10 — TOMADAS, PLUGUES E INSTALAÇÕES PREDIAIS
- Status Geral
- ENCONTRO 13 — PROTEÇÃO ELÉTRICA: DISJUNTORES E DR
- Modal de edição de avaliação
- Notificação de pendências nos cursos
- ENCONTRO 16 — Integração: Imprimindo Peças para o Robô
- ENCONTRO 10 — Era dos Robôs: História e Evolução da Robótica
- Ordenar cursos por alertas
- Status Geral
- professor-senai — Regras do Projeto
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- ENCONTRO 2 — Como Funciona uma Impressora 3D: FDM, SLA e SLS
- ENCONTRO 17 — Projeto Final: Montagem do Protótipo Robótico
- ENCONTRO 6 — Projeto 1 no Tinkercad: Modelando um Objeto Funcional
- ENCONTRO 12 — Sensores: Como os Robôs Enxergam o Mundo
- EMENTA — REFORÇO DE LINGUAGENS
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- Remover a palavra fácil da atividade
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- Toggle para cursos bloqueados
- Toggle Ensalado no card da matéria
- 2. Gabarito comentado
- remover_palavra_facil_atividade.py
- 5. Estrutura de Blocos Temáticos e Plano de Aulas
- Migrar status das avaliações para a matéria
- 4. Estrutura de Blocos Temáticos e Plano de Aulas
- Analise de Tamanho - Pastas e Arquivos
- criar_lista_presenca_ceplas_barragem_2026.mjs
- gerar_texto_reforco_25_08_2026.py
- Global Constraints
- 6. PROCEDIMENTOS DE SEGURANÇA E BACKUP
- 11. CHECKLIST DE IMPLEMENTAÇÃO
- ENCONTRO 7 — Agenda Ambiental (A3P) e Primavera Silenciosa
- COMPETÊNCIAS SOCIOEMOCIONAIS E EMPREENDEDORISMO
- ENCONTRO 3 — Inteligência Emocional
- ENCONTRO 17 — Virtudes do Empreendedor de Sucesso
- ENCONTRO 2 — Perfil Empreendedor e Autoconhecimento
- ENCONTRO 6 — Ciclo da Água e Legislação Ambiental
- EXPLORAÇÃO DE CARREIRAS INDUSTRIAIS E TECNOLÓGICAS
- ENCONTRO 11 — Planejamento de Carreira: Da Escola ao Mercado
- ENCONTRO 12 — Currículo para Jovens: Como Montar o Seu
- ENCONTRO 14 — Networking: Construindo Conexões
- ENCONTRO 17 — Projeto Final: Meu Mapa de Carreira
- ENCONTRO 5 — Carreiras Industriais I: Mecânica e Eletrotécnica
- ENCONTRO 6 — Carreiras Industriais II: Automação e Mecatrônica
- 12.1 Memorando: comunicação interna eficiente
- 📌 ENCONTRO 16 — Revisão, Apresentações Finais e Reflexão
- NOÇÕES DE ELETRICIDADE E CIRCUITOS BÁSICOS
- ENCONTRO 17 — PRIMEIROS SOCORROS E PREVENÇÃO DE INCÊNDIOS
- ENCONTRO 6 — CIRCUITOS EM SÉRIE E EM PARALELO
- ENCONTRO 9 — INTERRUPTORES SIMPLES E PARALELO
- OFICINAS DE IMPRESSÃO 3D E ROBÓTICA
- ENCONTRO 3 — Materiais para Impressão 3D: Filamentos e Resinas
- ENCONTRO 9 — Impressão do Projeto 1 e Análise dos Resultados
- REFORÇO DE LINGUAGENS
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- 5.1 Bloco 01 — Fundamentos de Testes de Software
- 5.2 Bloco 02 — Configuração de Ambiente e Ferramentas
- 5.3 Bloco 03 — Testes Unitários em JavaScript
- 5.4 Bloco 04 — Testes de Integração
- 5.5 Bloco 05 — Testes End-to-End (E2E)
- 5.6 Bloco 06 — Testes de Performance e Acessibilidade
- 5.7 Bloco 07 — Integração Contínua (CI/CD) e Automação
- 4.1 Bloco 01 — Fundamentos de Testes — Autogestão e Automação
- 4.2 Bloco 02 — Conceitos Fundamentais e Planejamento de Testes
- 4.3 Bloco 03 — Processo Fundamental de Teste — Planejamento e Design
- 4.5 Bloco 05 — Automação e Otimização de Testes
- 4.6 Bloco 06 — Documentação e Relatórios de Teste
- 4.7 Bloco 07 — Métodos, Normas e Correção de Defeitos
- 4.8 Bloco 08 — Boas Práticas e Projeto Integrador
- 4. Autenticação
- 1. CONCEITOS E TIPOS DE BANCO DE DADOS
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- Path
- Referências e Para Continuar Aprendendo
- ENCONTRO 11 — Aprofundamento: Comunicação Não-Verbal e Empatia
- ENCONTRO 13 — Aprofundamento: Ferramentas Digitais Corporativas
- APOSTILA-CEPLAS-VESPERTINO-2026-02.md
- 📋 INSTRUÇÕES GERAIS DA PROVA
- LISTA DE PRESENÇA
- Conteúdo / Proposta
- ENCONTRO 18 — Apresentação dos Projetos e Avaliação Final
- criar_prova_simples_uc1_tic_equipes.py
- PARTE 2 — ROBÓTICA
- Encontro 3-4 — Tema e Ideia Principal (4h)
- Encontro 5-6 — Intenção Comunicativa e Inferências (4h)
- Encontro 8-9 — Paragrafação e Organização de Ideias (4h)
- Apostila_Reforco_Matematica_Raciocinio_Logico.md
- SLIDE-Matematica-Raciocínio-Logico — Progresso de Criação
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- 6. Sincronização com o Resend
- 9. MCP do Resend (referência)
- 8. CHECKLIST DE NORMALIZAÇÃO
- 13. CONCLUSÕES E RECOMENDAÇÕES
- 4. MODELAGEM DE DADOS
- 8. PROCEDIMENTOS DE SEGURANÇA E BACKUP
- ENCONTRO 10 — Mapa de Habilidades e Interesses
- CLAUDE.md — Pasta `/sistema` — Diretrizes Pedagógicas e Estrutura de Aulas
- ENCONTRO 18 — Apresentações e Encerramento
- ENCONTRO 1 — Quem Sou Eu? Autoconhecimento Inicial
- ENCONTRO 2 — O Mundo do Trabalho: Ontem, Hoje e Amanhã
- ENCONTRO 4 — Automação, Robôs e IA: O Futuro Já Chegou?
- ENCONTRO 9 — Profissões do Futuro: O Que Ainda Não Existe
- Status Geral
- ENCONTRO 1 — O Que É Tecnologia? Dispositivos Digitais no Cotidiano
- Status Geral
- ENCONTRO 14 — Fluxogramas: Desenhando Soluções
- ENCONTRO 15 — Scratch: Primeiros Passos na Programação em Blocos
- ENCONTRO 16 — Scratch: Condições, Repetições e Variáveis
- ENCONTRO 17 — Scratch: Mini Projeto Final
- ENCONTRO 2 — Evolução dos Computadores e História da Tecnologia
- ENCONTRO 3 — Cidadania Digital: Fake News, Cyberbullying e Pegada Digital
- ENCONTRO 4 — Hardware e Software: O Que Tem Dentro de um Computador?
- Status Geral
- ENCONTRO 5 — Sistemas Operacionais: Windows, Linux e ChromeOS
- ENCONTRO 6 — Organização do Computador: Arquivos, Pastas e Atalhos
- Status Geral
- ENCONTRO 7 — Produtividade: Editor de Texto
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- Status Geral
- ENCONTRO 8 — Apresentações Digitais (Google Slides / PowerPoint)
- ENCONTRO 9 — Planilhas Simples (Google Sheets / Excel Básico)
- ENCONTRO 10 — Internet Segura: Vírus, Senhas, Privacidade e Navegadores
- ENCONTRO 11 — Pensamento Computacional: Decomposição e Padrões
- ENCONTRO 12 — Pensamento Computacional: Abstração e Algoritmos Desplugados
- Configurar GEMINI.bat como agente de IA
- Configurar OpenClaude com Alibaba Cloud
- ENCONTRO 13 — Algoritmos: Sequência, Decisão e Repetição
- FUNDAMENTOS_DA_TECNOLOGIA_E_PROGRAMACAO/gerar_slides.js
- Status Geral
- Inicializador OpenClaude com Alibaba Qwen
- Status Geral
- 🎯 FASE 3: ENCERRAMENTO E INTEGRAÇÃO
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- Dashboard de Controle de Pendências
- calcular_notas_pratica.py
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- corretor_prova_pratica.py
- Conhecimentos
- AULA 02 — Sistema Operacional e Organização Digital
- 9. Recursos Necessários
- Tempo mínimo do modal de atualização de pendências
- 7. Recursos Necessários
- 5. Banco de dados
- dependencies
- 5. ANOMALIAS ELIMINADAS PELA NORMALIZAÇÃO
- 7. NORMALIZAÇÃO DE LINGUAGENS DE BANCO DE DADOS
- 12. DEPLOYMENT E OPERAÇÃO
- 2. ARQUITETURA DE BANCO DE DADOS
- 3. ANÁLISE COMPARATIVA DE SGBDs
- Ignorar arquivo ALIBABA_IA.txt
- 7. NORMALIZAÇÃO ATÉ 3ª FORMA NORMAL
- 9. FERRAMENTAS DE MANIPULAÇÃO E EXECUÇÃO
- REFERÊNCIAS E RECURSOS
- AULA 09 — Editor de Apresentações e Projeto Integrador
- Compositor
- LISTAS-EMAIL.js
- PARTE 1 — IMPRESSÃO 3D
- AULA 06 — Segurança da Informação
- AULA 09 — Editor de Apresentações e Projeto Integrador
- AULA 09 — Editor de Apresentações e Projeto Integrador
- AULA 06 — Segurança da Informação
- 11. Referências
- 4. Situação de Aprendizagem — Desafio Central
- 8. Metodologia e Estratégias Transversais
- 11. Observações Importantes
- ClaudeGeradoAulasService
- 6. Metodologia e Estratégias Transversais
- AULA 06 — Segurança da Informação
- 8. Base de alunos — `LISTAS-EMAIL.js`
- 5. SCRIPTS SQL - DDL E DML
- Pesquisa — Planilha Excel de Controle de Abate de Suínos
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- README.md
- GestaoControleMateriais_456h.md
- ATIVIDADE PRÁTICA — Planilha Eletrônica
- Arquivos
- ATIVIDADE INTEGRADORA — UC1 Introdução à TIC
- PLANO DE AULAS — Introdução à Tecnologia da Informação e Comunicação
- Distribuição de Alunos por Desafio — PROVA SIMPLES UC1 TIC
- GERADOR-AULAS — Sistema Inteligente de Geração de Aulas por Ementas
- 13. Textos de Reforço — Regra Obrigatória
- 📋 Temas Sugeridos por Módulo
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- MATERIAIS — Introdução à Tecnologia da Informação e Comunicação
- 11. Script de Geração (`gerar_slides.js`)
- 12. Como trabalhar com esta pasta
- Parte I — Planejamento completo das aulas
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação
- AULA 07 — Software de Escritório: Editor de Textos
- GERADOR DE SLIDES — Dashboard Django + Supabase
- Parte I — Planejamento completo das aulas
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 9. Sobre os Slides (`Slides_Reforco_Linguagens.pptx`)
- CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC
- 14. Checklist de Conformidade
- 15. Referências Rápidas
- 8. Sobre a Apostila (`Apostila_Reforco_Linguagens.md`)
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- Gabarito técnico — Prova Simples UC1 TIC em equipes
- AULA 07 — Software de Escritório: Editor de Textos
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe
- pendencias.js
- Conteúdo Programático
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- views.py
- AULA 09 — Editor de Apresentações e Projeto Integrador
- Parte I — Planejamento completo das aulas
- AULA 09 — Editor de Apresentações e Projeto Integrador
- AULA 06 — Segurança da Informação
- Diagnóstico atual — 04/09/2026, 14:20
- AULA 06 — Segurança da Informação
- Parte I — Planejamento completo das aulas
- Slide
- Conteúdo Programático
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- Conteúdo Programático
- Status Geral
- Status Geral
- Status Geral
- modelo-slide-senai-2026 — Especificação técnica para reprodução
- 2. Gabarito comentado
- Conteúdo Programático
- CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação
- modelo-slide-senai-2026.md
- ESTRUTURA-PROVAS — Documentação Consolidada
- Bloco 2 — Operando o Computador
- SupabaseService
- 🚀 GERADOR-SLIDES com Supabase — Setup Rápido
- GERADOR-SLIDES com Django + Supabase Auth
- Status Geral
- Relatório de risco Git / LFS — aulas-senai
- PADRÃO DE SLIDES — Projeto aulas-senai
- GERADOR DE SLIDES SENAI
- check_auth
- Conteúdo Programático
- Checklist Prova Prática
- SINTAXE MARKDOWN — Entrada do Gerador de Slides
- 4. Conteúdo
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- 🔧 Configuração Inicial
- 🐛 Troubleshooting
- ENTRADAS-AULAS-MARKDOWN
- ✅ Funcionalidades Implementadas
- 🔔 Sistema de Pendências — Regra Crítica
- 🎯 CONTEXTO-PROJETO
- 📊 PADRÃO OBRIGATÓRIO DE INFOGRÁFICOS
- Estrutura de Unidades Curriculares
- Checklist Prova Objetiva
- Graphify
- Modelo Prova Objetiva CT ELETROMECÂNICA (1) (2)
- Modelo Prova Prática CT ELETROTÉCNICA
- PROMPT - Rubricas Capacidades Socioemocionais
- Prova Objetiva
- Prova Prática (3)
- Template Prova Prática
- 7. Tipos de slide
- TASKS — Rastreamento de Gerações de Slides
- AULA 08 — Software de Escritório: Planilhas Eletrônicas
- 🛠️ Operações Comuns
- 📊 Modelos Django
- DashboardConfig
- main
- 0001_initial.py
- 0002_slide_usuariosupabase.py
- asgi.py
- settings.py
- gerador_config/urls.py
- wsgi.py
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- AULA 10 — Avaliação Objetiva e Avaliação Prática
- AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação
- Instruções — Corretor híbrido da Prova Simples UC1 TIC
- Conhecimentos
- 6. Estrutura slide a slide
- 🎨 Estrutura dos 9 Slides
- Conteúdo Programático
- AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos
- AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação
- CRITÉRIOS DE CORREÇÃO — Avaliação Prática
- 3. Unidade Curricular 1: Introdução à Tecnologia da Informação e Comunicação
- .list_materias
- Análise para 3 Alunos (15 anos)
- Fase 3: Geração de Conteúdo
- 📝 REGRAS DE GERAÇÃO
- Conhecimentos
- 🎯 Novo Fluxo: Criar Slide com Metadados
- dashboard
- Tabelas Envolvidas
- MATERIAIS — Introdução à Tecnologia da Informação e Comunicação
- markdown_to_docx
- 🛠️ TECNOLOGIAS E FERRAMENTAS
- 🤝 INTEGRAÇÃO COM ESTRUTURA EXISTENTE
- 🔮 FUNCIONALIDADES FUTURAS (Roadmap)
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 4. Unidade Curricular 2: Saúde e Segurança no Trabalho — *contexto do módulo*
- 6. Configuração Supabase
- 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto
- 🚀 CASOS DE USO E EXEMPLOS
- 📚 REFERÊNCIAS
- 👤 Responsabilidades
- 🔄 Como Usar no Claude Design
- 4. Tipografia
- TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)
- 4. Software de Escritório
- 📋 ESTRUTURA DE ENTRADA: AS EMENTAS
- database.json
- CORRECAO-PROVA-PRATICA.js
- NOTAS-EQUIPE-PROVA-PRATICA.js
- NOTAS-OBJETIVA.js
- NOTAS-PRATICA-FINAL.js
- analise_tamanhos.py
- 🎓 Implementar Integração com Claude API para Geração de Aulas
- 📄 Páginas Implementadas — Gerador de Aulas
- ⚙️ Configuração Avançada
- TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)
- auth_views.py
- main
- 3. Situação de Aprendizagem — Desafio Central

## God Nodes (most connected - your core abstractions)
1. `Introdução à Tecnologia da Informação e Comunicação` - 117 edges
2. `Introdução à Tecnologia da Informação e Comunicação` - 117 edges
3. `Introdução à Tecnologia da Informação e Comunicação` - 117 edges
4. `Introdução à Tecnologia da Informação e Comunicação` - 117 edges
5. `Currículo Técnico - Informática para Internet` - 86 edges
6. `SupabaseService` - 29 edges
7. `FUNDAMENTOS DA TECNOLOGIA E PROGRAMAÇÃO` - 26 edges
8. `MateriasManager` - 25 edges
9. `f()` - 24 edges
10. `novo()` - 23 edges

## Surprising Connections (you probably didn't know these)
- `parse_markdown()` --calls--> `novo()`  [INFERRED]
  GERADOR-SLIDES/scripts/gerar_slides.py → sistema/FICHA-PRODUTO-MAIS-TECH/EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS/MATERIAIS/scripts/gerar_imagens.py
- `login()` --uses--> `SupabaseService`  [INFERRED]
  GERADOR-SLIDES/dashboard/auth_views.py → GERADOR-SLIDES/dashboard/services.py
- `signup()` --uses--> `SupabaseService`  [INFERRED]
  GERADOR-SLIDES/dashboard/auth_views.py → GERADOR-SLIDES/dashboard/services.py
- `logout()` --uses--> `SupabaseService`  [INFERRED]
  GERADOR-SLIDES/dashboard/auth_views.py → GERADOR-SLIDES/dashboard/services.py
- `dashboard()` --uses--> `GeracaoSlide`  [INFERRED]
  GERADOR-SLIDES/dashboard/views.py → GERADOR-SLIDES/dashboard/models.py

## Import Cycles
- None detected.

## Communities (651 total, 17 thin omitted)

### Community 0 - "Currículo Técnico - Informática para Internet"
Cohesion: 0.02
Nodes (86): Currículo Técnico - Informática para Internet, Página 1, Página 10, Página 11, Página 12, Página 13, Página 14, Página 15 (+78 more)

### Community 1 - "CLAUDE.md — Pasta REFORCO_LINGUAGENS"
Cohesion: 0.17
Nodes (11): 10. Arquivo Word (`TEXTO-REFORCO-25-08-2026.docx`), 14. Informações do Programa, 1. O que é esta pasta, 2. Arquivos na pasta, 3. Identificação da UC, 4. Objetivo da UC, 5. Estrutura de Módulos, 6. Capacidades (+3 more)

### Community 2 - "EMENTA — TESTES DE FRONTEND"
Cohesion: 0.05
Nodes (42): 10. QUADRO RESUMO DE CARGA HORÁRIA, 11. CORRELAÇÃO COM MATRIZ CURRICULAR, 12.1 Expectativas de Aprendizagem, 12.2 Acessibilidade, 12.3 Integração com Mercado de Trabalho, 12. OBSERVAÇÕES IMPORTANTES, 13. APROVAÇÃO E ASSINATURA, 14. NOTAS DO PROFESSOR (+34 more)

### Community 3 - "5. CHECKLIST DE AVALIAÇÃO RESPONDIDO"
Cohesion: 0.05
Nodes (39): 1.1.1 Banco de Dados Relacional, 1.1.2 Banco de Dados Não-Relacional (NoSQL), 1.1 Conceitos Fundamentais, 1.2 Análise para Sistema de Delivery de Comida, 1. IDENTIFICAÇÃO DE TIPOS E CARACTERÍSTICAS DE BANCO DE DADOS, 2.1 Arquitetura Cliente-Servidor, 2.2.1 Arquitetura Centralizada (Single Server), 2.2.2 Arquitetura com Replicação (+31 more)

### Community 4 - "3. Conteúdo completo"
Cohesion: 0.05
Nodes (39): 10. Características pedagógicas, 11. Regras para reproduzir o padrão, 12. Resultado esperado, 1. Identificação, 2. Mensagem central, 3.10 Seção 10 — Conecte uma ideia à outra, 3.11 Seção 11 — Slides não são roteiro de leitura, 3.12 Seção 12 — Conclusão: termine com propósito (+31 more)

### Community 5 - "Ficha de Cadastro de Produto Customizado — Rio do Sul Mais Tech / SENAI"
Cohesion: 0.05
Nodes (38): 1. Identificação do Produto, 2.1 Cronograma e Carga Horária, 2.2 Detalhamento por Escola (Sendo:), 2.3 Turnos por Escola, 2.4 Local de Execução, 2. Informações Operacionais, 3. Requisitos de Acesso, 4. Forma de Avaliação (+30 more)

### Community 6 - "Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.02
Nodes (117): Introdução à Tecnologia da Informação e Comunicação, Slide 1, Slide 10, Slide 100, Slide 101, Slide 102, Slide 103, Slide 104 (+109 more)

### Community 7 - "PLANO DE AULAS: Análise de Dados Aplicada à Gestão"
Cohesion: 0.06
Nodes (35): 1. OBJETIVOS DE APRENDIZAGEM, 2. ESTRUTURA DO PLANO (30 horas de aulas), 3. AVALIAÇÕES (2 horas), 4. CRITÉRIOS DE AVALIAÇÃO, 5. METODOLOGIA PEDAGÓGICA, 6. RECURSOS NECESSÁRIOS, 7. CRONOGRAMA, 8. REFERÊNCIAS BIBLIOGRÁFICAS (+27 more)

### Community 8 - "1.2 Identificação Detalhada de Atributos"
Cohesion: 0.06
Nodes (33): 1.1 Levantamento de Requisitos, 1.2 Identificação Detalhada de Atributos, 1. ENTIDADES, ATRIBUTOS E RELACIONAMENTOS, 2.1 Matriz de Relacionamentos, 2.2 Descrição Detalhada dos Relacionamentos, 2. RELACIONAMENTOS E CARDINALIDADES, 3. DIAGRAMA ENTIDADE-RELACIONAMENTO (Notação UML), 4.1 Verificação de 1ª Forma Normal (1FN) (+25 more)

### Community 9 - "gerar_apostila.js"
Cohesion: 0.11
Nodes (32): ENCONTROS, bordas(), BOX(), cel(), cellP(), COD(), dimensoesPng(), doc (+24 more)

### Community 10 - "professor-senai — Regras do Projeto"
Cohesion: 0.17
Nodes (11): Commit e Push obrigatórios, Estrutura obrigatória de cada UC, Git — Execução Automática OBRIGATÓRIA, Nunca perguntar — sempre executar, Organização das provas práticas de Introdução à TIC, Organização das provas teóricas, Pasta PROFESSOR, professor-senai — Regras do Projeto (+3 more)

### Community 11 - "criar_apostila_ferramentas_digitais.py"
Cohesion: 0.24
Nodes (39): add_activity(), add_body(), add_bullets(), add_callout(), add_page_field(), add_steps(), add_two_column_table(), build() (+31 more)

### Community 12 - "gerar_imagens.py"
Cohesion: 0.33
Nodes (33): bloco_texto(), card_carreira(), emj(), f(), faixa_titulo(), icone(), img_capa(), img_clp() (+25 more)

### Community 13 - "RELATÓRIO DE ALINHAMENTO — Aulas vs Ementa"
Cohesion: 0.06
Nodes (33): AVALIAÇÃO FINAL (1h), BLOCO 1 — Tecnologia e Mundo Digital (4h), BLOCO 2 — Hardware, Software e Sistema Operacional (4h), BLOCO 3 — Produtividade Digital (6h), BLOCO 4 — Internet e Uso Seguro (4h), BLOCO 5 — Pensamento Computacional e Algoritmos (6h), BLOCO 6 — Programação em Blocos (Scratch) (8h), Cobertura nas Aulas (+25 more)

### Community 14 - "/graphify"
Cohesion: 0.06
Nodes (32): For --cluster-only, For git commit hook, For /graphify add, For /graphify explain, For /graphify path, For /graphify query, For native AGENTS.md integration, For --update (incremental re-extraction) (+24 more)

### Community 15 - "/graphify"
Cohesion: 0.06
Nodes (32): For --cluster-only, For git commit hook, For /graphify add, For /graphify explain, For /graphify path, For /graphify query, For native CLAUDE.md integration, For --update (incremental re-extraction) (+24 more)

### Community 16 - "Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.02
Nodes (117): Introdução à Tecnologia da Informação e Comunicação, Slide 1, Slide 10, Slide 100, Slide 101, Slide 102, Slide 103, Slide 104 (+109 more)

### Community 17 - "TEXTO DE REFORÇO — MÓDULO 3"
Cohesion: 0.06
Nodes (31): 1. PONTO (.), 2. VÍRGULA (,), 3. PONTO DE INTERROGAÇÃO (?), 4. PONTO DE EXCLAMAÇÃO (!), 5. RETICÊNCIAS (...), 6. TRAVESSÃO (—), A Vírgula, As Regras Básicas (+23 more)

### Community 18 - "AULA 14: Gráficos e Visualização de Dados"
Cohesion: 0.06
Nodes (31): 1.1 Gráfico de Colunas, 1.2 Gráfico de Linhas, 1.3 Gráfico de Setores (Pizza), 1.4 Gráfico de Barras, 1.5 Gráfico de Dispersão (XY), 1. TIPOS DE GRÁFICOS, 2.1 Passos, 2.2 Seleção de Dados (+23 more)

### Community 19 - "AULA 16: Criação de Dashboard Prático"
Cohesion: 0.06
Nodes (31): 1.1 Estrutura, 1.2 Cálculos Necessários, 1.3 Exemplo: Dados de Vendas, 1. PREPARAÇÃO DE DADOS, 2.1 Células com KPIs, 2.2 Gráficos Interligados, 2.3 Tabelas Dinâmicas, 2. INTEGRAÇÃO DE ELEMENTOS (+23 more)

### Community 20 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 21 - "AULA 12: Tabelas Dinâmicas"
Cohesion: 0.06
Nodes (30): 1.1 Definição, 1.2 Vantagens, 1.3 Estrutura, 1. CONCEITO DE TABELA DINÂMICA, 2.1 Passos, 2.2 Exemplo Prático, 2. CRIAÇÃO DE TABELA DINÂMICA, 3.1 Filtro Automático (+22 more)

### Community 22 - "Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.06
Nodes (30): 10. Síntese semântica, 11. Proveniência, 1. Identificação da unidade curricular, 2. Objetivo geral, 3.1 Comunicação oral e escrita no trabalho, 3.2 Segurança da informação, 3.3 Interpretação de informações e textos técnicos, 3.4 Hardware e software (+22 more)

### Community 23 - "Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.02
Nodes (117): Introdução à Tecnologia da Informação e Comunicação, Slide 1, Slide 10, Slide 100, Slide 101, Slide 102, Slide 103, Slide 104 (+109 more)

### Community 24 - "AULA 10: Funções de Busca e Consulta"
Cohesion: 0.07
Nodes (29): 1.1 Sintaxe, 1.2 Parâmetros, 1.3 Exemplo Prático, 1.4 Aplicações, 1. FUNÇÃO PROCV (VLOOKUP), 2.1 Sintaxe, 2.2 Diferença do PROCV, 2.3 Exemplo Prático (+21 more)

### Community 25 - "AULA 11: Função SE Avançada e ContSE"
Cohesion: 0.07
Nodes (29): 1.1 Conceito, 1.2 Sintaxe, 1.3 Exemplos Práticos, 1.4 Limitações, 1. FUNÇÃO SE ANINHADA, 2.1 Sintaxe, 2.2 Exemplos, 2.3 Operadores (+21 more)

### Community 26 - "AULA 13: Filtros e Validação de Dados"
Cohesion: 0.07
Nodes (29): 1.1 Ativação, 1.2 Uso, 1.3 Exemplos, 1. FILTRO AUTOMÁTICO, 2.1 Sintaxe, 2.2 Critérios Múltiplos, 2.3 Exemplo Prático, 2. FILTRO AVANÇADO (+21 more)

### Community 27 - "AULA 10-15-10-2026.md"
Cohesion: 0.07
Nodes (29): 7.2.1 AvaliaÃ§Ã£o PrÃ¡tica â€” 1h, 7.2.2 AvaliaÃ§Ã£o TeÃ³rica Objetiva â€” 1h, AplicaÃ§Ã£o Completa de Conhecimento, Atividade 1: Palestra e DiscussÃ£o (90 min), Atividade 2: AvaliaÃ§Ã£o PrÃ¡tica (60 min), Atividade 3: AvaliaÃ§Ã£o TeÃ³rica (60 min), AvaliaÃ§Ã£o PrÃ¡tica, AvaliaÃ§Ã£o TeÃ³rica (+21 more)

### Community 28 - "AULA 8: Fórmulas e Funções Essenciais"
Cohesion: 0.07
Nodes (28): 1.1 Função SUM (Soma), 1.2 Função AVERAGE (Média), 1.3 Função COUNT (Contar), 1. FUNÇÕES BÁSICAS DE CÁLCULO, 2.1 Sintaxe Básica, 2.2 Exemplos, 2.3 Aplicações, 2.4 Exemplo Prático (+20 more)

### Community 29 - "AULA 9: Formatação e Apresentação de Dados"
Cohesion: 0.07
Nodes (28): 1.1 Formato de Números, 1.2 Formato de Moeda, 1.3 Formato de Percentual, 1.4 Formato de Data, 1. FORMATAÇÃO DE CÉLULAS, 2.1 Conceito, 2.2 Tipos Básicos, 2.3 Exemplo Prático (+20 more)

### Community 30 - "AULA 15: Introdução a Dashboards"
Cohesion: 0.07
Nodes (28): 1.1 Definição, 1.2 Tipos de Dashboards, 1.3 Benefícios, 1. CONCEITO DE DASHBOARD, 2.1 KPI (Indicador-Chave de Desempenho), 2.2 Gráficos, 2.3 Indicadores Visuais, 2.4 Tabelas (+20 more)

### Community 31 - "🎓 1º PERÍODO (SEMESTRE)"
Cohesion: 0.07
Nodes (28): 10. Testes de Front-End, 11. Projeto de Front-End, 12. Sustentabilidade nos Processos Industriais, 13. Introdução a Qualidade e Produtividade, 14. Codificação para Back-End, 15. Desenvolvimento de APIs, 16. Banco de Dados, 17. Testes de Back-End (+20 more)

### Community 32 - "ATIVIDADE-EPISÓDIO 02"
Cohesion: 0.07
Nodes (26): Atividade 2.1, 📝 Atividade 2.1 — Pontuação Correta, Atividade 2.2, ✅ Atividade 2.2 — Concordância Verbal, Atividade 2.3, 🔄 Atividade 2.3 — Concordância Nominal, Atividade 2.4 (Discurso), 🎤 Atividade 2.4 — Preparar um Discurso de 3 Minutos (+18 more)

### Community 33 - "Substituições — Reforço de Linguagens"
Cohesion: 0.07
Nodes (29): 1. Acessar o SGN e Identificar Aulas Faltantes, 2. Selecionar Encontros para Substituição, 3. Preparar Material, 4. Lecionar a Aula de Substituição, 5. Registrar no SGN, 🔗 Acesso ao Diário de Classe (SGN), 📋 Aulas Disponíveis para Substituição, Caso 1: Substituir 1 Encontro (2h) (+21 more)

### Community 34 - "TEXTO DE REFORÇO — MÓDULO 4"
Cohesion: 0.07
Nodes (26): 1. SILÊNCIO ATENCIOSO, 2. CONTATO VISUAL, 3. LINGUAGEM CORPORAL, 4. PARAFRASEIO, 5. PERGUNTAS PERTINENTES, Atividade 1, Atividade 1 — Verdadeiro ou Falso, Atividade 2 (+18 more)

### Community 35 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.07
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 36 - "Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.02
Nodes (117): Introdução à Tecnologia da Informação e Comunicação, Slide 1, Slide 10, Slide 100, Slide 101, Slide 102, Slide 103, Slide 104 (+109 more)

### Community 37 - "ATIVIDADE — EPISÓDIO 02"
Cohesion: 0.08
Nodes (25): ATIVIDADE — EPISÓDIO 02, Desafio Extra (Bônus: +10 pontos), GABARITO (para professor), Pontuação, Questão 1.1, Questão 1.2, Questão 1.3, Questão 2.1 (+17 more)

### Community 38 - "TEXTO DE REFORÇO — MÓDULO 2"
Cohesion: 0.08
Nodes (25): 1. ASSUNTO, 2. SAUDAÇÃO, 3. ABERTURA (1º parágrafo), 4. CORPO (2º parágrafo), 5. ENCERRAMENTO, 6. ASSINATURA, Atividade 1, Atividade 1 — Verdadeiro ou Falso (+17 more)

### Community 39 - "TEXTO DE REFORÇO — MÓDULO 5"
Cohesion: 0.08
Nodes (25): A Verdade sobre Memorização, Atividade 1, Atividade 1 — Verdadeiro ou Falso, Atividade 2, Atividade 2 — Múltipla Escolha, Atividade 3, Atividade 3 — Resposta Breve, Atividades de Compreensão (+17 more)

### Community 40 - "AULA 02: Razão, Proporção e Regra de Três"
Cohesion: 0.08
Nodes (25): 1.1 Definição, 1.2 Exemplos Práticos em Gestão, 1. Conceito de Razão, 2.1 Definição, 2.2 Propriedade Fundamental, 2.3 Exemplos Práticos, 2. Proporção, 3.1 Regra de Três Simples (+17 more)

### Community 41 - "AULA 4: Introdução à Estatística Básica"
Cohesion: 0.08
Nodes (25): 1.1 Definições Básicas, 1.2 Tipos de Dados, 1. CONCEITOS FUNDAMENTAIS, 2.1 Média Aritmética, 2.2 Mediana, 2.3 Moda, 2. MEDIDAS DE TENDÊNCIA CENTRAL, 3.1 Amplitude (+17 more)

### Community 42 - "AULA 6: Sequência Lógica"
Cohesion: 0.08
Nodes (25): 1.1 Conceito, 1.2 Aplicação, 1.3 Exemplo Prático, 1. PROGRESSÕES ARITMÉTICAS (PA), 2.1 Conceito, 2.2 Aplicação, 2.3 Exemplo Prático, 2. PROGRESSÕES GEOMÉTRICAS (PG) (+17 more)

### Community 43 - "AULA 7: Introdução ao Excel e Navegação Básica"
Cohesion: 0.08
Nodes (25): 1.1 Componentes Principais, 1.2 Estrutura de Células, 1. INTERFACE DO EXCEL, 2.1 Movimentação, 2.2 Seleção, 2. NAVEGAÇÃO BÁSICA, 3.1 Entrada de Dados, 3.2 Formatação Básica (+17 more)

### Community 44 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 45 - "dashboard.js"
Cohesion: 0.21
Nodes (24): abrirModal(), atualizarMetricas(), avisar(), carregarContatos(), carregarUnidades(), contatos, escapar(), esconder() (+16 more)

### Community 46 - "📄 Google Docs vs Microsoft Word"
Cohesion: 0.08
Nodes (24): 10. 🎯 Matriz de Decisão, 11. 💡 Quando Usar?, 1. ☁️ Google Docs, 2. 🔄 Colaboração Real-time, 3. 🎨 Formatação, 4. 📤 Exportação & Formatos, 5. ✨ Vantagens Google Docs, 6. 📝 Microsoft Word (+16 more)

### Community 47 - "E-MAIL-SENAI — Documentação do Projeto"
Cohesion: 0.15
Nodes (13): 10. Qual recurso do Resend usar em cada caso, 11. ⚠️ LGPD e segurança, 12. Histórico de decisões, 1. Arquitetura, 2. Estrutura de arquivos, 3. Como rodar, 7. Funcionalidades do dashboard, E-MAIL-SENAI — Documentação do Projeto (+5 more)

### Community 48 - "ATIVIDADE-EPISÓDIO 01"
Cohesion: 0.08
Nodes (24): Atividade 1.1, Atividade 1.1 — Identificar Tipo de Texto, Atividade 1.2, Atividade 1.2 — Tema e Ideia Principal, Atividade 1.3, Atividade 1.3 — Intenção Comunicativa, Atividade 1.4, Atividade 1.4 — Inferência (+16 more)

### Community 49 - "AULA 01: Introdução e Conjuntos Numéricos"
Cohesion: 0.08
Nodes (24): 1.1 Objetivos e Competências, 1.2 Estrutura do Curso, 1.3 Importância da Análise de Dados, 1. Apresentação da Disciplina (30 min), 2.1 Números Naturais (ℕ), 2.2 Números Inteiros (ℤ), 2.3 Números Racionais (ℚ), 2.4 Números Reais (ℝ) (+16 more)

### Community 50 - "AULA 5: Área, Volume e Peso"
Cohesion: 0.08
Nodes (24): 1.1 Figuras Planas Básicas, 1.2 Aplicação em Armazenagem, 1.3 Exemplo Prático, 1. CÁLCULO DE ÁREA, 2.1 Sólidos Básicos, 2.2 Aplicação em Logística, 2.3 Exemplo Prático, 2. CÁLCULO DE VOLUME (+16 more)

### Community 51 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 52 - "gerar-emails-tela.js"
Cohesion: 0.19
Nodes (21): FORMATOS, gerarEmail(), gerarEmailsDeTeste(), gerarParaLista(), gerarParaTodas(), normalizar(), paraCsvConferencia(), partesDoNome() (+13 more)

### Community 53 - "CLAUDE.md — Pasta FICHA-PRODUTO-MAIS-TECH"
Cohesion: 0.07
Nodes (26): 1. O que é esta pasta, 2. Arquivos na raiz da pasta, 3. Estrutura do Modelo de Slides (`MODELO_LAYOUT_APRESENTACAO.pptx`), 4.1 `CompetenciasSocioemocionaisEmpreendedorismo/`, 4.2 `FUNDAMENTOS_DA_TECNOLOGIA_E_PROGRAMACAO/`, 4.3 `NocoesEletricidadeCircuitosBasicos/`, 4.4 `OFICINAS_IMPRESSAO_3D_ROBOTICA/`, 4.5 `INTRODUCAO_COMUNICACAO_ORAL_ESCRITA/` (+18 more)

### Community 54 - "AULA 03 — Apresentações: PowerPoint e Google Slides"
Cohesion: 0.08
Nodes (23): 1. Introdução a Apresentações (15 min), 2. PowerPoint — Interface e Estrutura (20 min), 3. Formatação e Design no PowerPoint (20 min), 4. Inserir Elementos Multimídia (15 min), 5. Animações e Transições (15 min), 6. Google Slides — Colaboração (10 min), 7. Dicas para Apresentação Eficaz (5 min), Atividade 1: Criar Apresentação sobre Tema Livre no PowerPoint (40 min) (+15 more)

### Community 55 - "AVALIACOES_CRIADAS — Sistema de Gerenciamento de Notas"
Cohesion: 0.05
Nodes (39): 1. **NOTAS-OBJETIVA.js**, 1. **Recebimento de Entregas**, 1. **Visualização de Notas por Equipe**, 2. **Cálculo Automático**, 2. **Menu "Notas Prova Objetiva"**, 2. **NOTAS-EQUIPE-PROVA-PRATICA.js**, 2. **Ícone OBS (Observações)**, 3. **Menu "Notas por Aluno (Prática)"** (+31 more)

### Community 56 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.08
Nodes (23): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+15 more)

### Community 57 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.08
Nodes (23): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+15 more)

### Community 58 - "MÓDULO 2 — FRAÇÕES, DECIMAIS E PORCENTAGEM"
Cohesion: 0.09
Nodes (23): 5.1 Representação de Frações, 5.2 Frações Equivalentes, 6.1 Adição e Subtração, 6.2 Multiplicação, 6.3 Divisão, 7.1 Fração → Decimal, 7.2 Decimal → Fração, 8.1 O que é Porcentagem (+15 more)

### Community 59 - "AVALIAÇÃO PRÁTICA"
Cohesion: 0.09
Nodes (21): Aplicação funcional documentada, composta por:, Apresentação à banca técnica (Próxima semana), ATIVIDADE 01: Levantamento conceitual e justificativa para escolha do SGBD, ATIVIDADE 02: Diagrama Entidade-Relacionamento (DER) revisado e validado, ATIVIDADE 03: Script SQL de criação e manipulação de dados (DDL e DML), ATIVIDADE 04: Consultas SQL (relatórios e operações), ATIVIDADE 05: Aplicação de normalização até a 3ª forma normal, ATIVIDADE 06: Entrega final da Situação de Aprendizagem - Documento técnico (+13 more)

### Community 60 - "AULA 02 — Planilhas Eletrônicas: Excel e Google Planilhas"
Cohesion: 0.09
Nodes (21): 1. Introdução a Planilhas Eletrônicas (15 min), 2. Estrutura da Planilha (20 min), 3. Excel — Operações Básicas (25 min), 4. Excel — Fórmulas e Funções (20 min), 5. Google Planilhas — Recursos Colaborativos (15 min), 6. Gráficos (10 min), Atividade 1: Planilha de Controle de Gastos no Excel (35 min), Atividade 2: Análise de Dados no Google Planilhas (30 min) (+13 more)

### Community 61 - "Relatório de Aulas — Introdução à Comunicação Oral e Escrita"
Cohesion: 0.09
Nodes (21): 📝 Aulas Ministradas (20h), Avaliações Pendentes, Avaliações Realizadas, ✅ Checklist de Implementação, ✅ Conteúdos Contemplados (20h), ❌ Conteúdos Faltantes (13h), 📚 Conteúdos Ministrados vs. Planejados, 📊 Cronograma Sugerido (+13 more)

### Community 62 - "AULA 3: Porcentagem e Conversão de Unidades"
Cohesion: 0.09
Nodes (21): 1.1 Conceitos Fundamentais, 1.2 Cálculos Básicos, 1.3 Variação Percentual, 1.4 Aplicações em Gestão, 1. PORCENTAGEM, 2.1 Unidades de Comprimento, 2.2 Unidades de Massa, 2.3 Unidades de Volume (+13 more)

### Community 63 - "MateriasManager"
Cohesion: 0.14
Nodes (3): app, MateriasManager, ThemeManager

### Community 64 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 65 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 66 - "Conteúdo Programático"
Cohesion: 0.10
Nodes (20): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+12 more)

### Community 67 - "ATIVIDADE — EPISÓDIO 01"
Cohesion: 0.10
Nodes (20): ATIVIDADE — EPISÓDIO 01, GABARITO (para professor), Pontuação, Questão 1.1, Questão 1.2, Questão 1.3, Questão 2.1, Questão 2.2 (+12 more)

### Community 68 - "AULA 01 — O que é Tecnologia e Dispositivos Digitais no Cotidiano"
Cohesion: 0.10
Nodes (20): 0. Contextualização Inicial: Um Dia Sem Tecnologia (10 min), 1. O que é Tecnologia? (25 min), 2. Dispositivos Digitais no Cotidiano (30 min), 3. Evolução dos Computadores (40 min), 4. O Mundo Digital e Binário (20 min), 5. Conectividade, IoT e IA (15 min), Atividade 1: Timeline da Tecnologia (30 min), Atividade 2: Meu Dispositivo Favorito (20 min) (+12 more)

### Community 69 - "AULA 01 — Editores de Texto: Word e Google Docs"
Cohesion: 0.10
Nodes (20): 1. Introdução a Editores de Texto (15 min), 2. Microsoft Word — Fundamentos (30 min), 3. Word — Recursos Avançados (20 min), 4. Google Docs — Fundamentos (25 min), 5. Exportação e Compartilhamento (10 min), Atividade 1: Criar Documento Profissional no Word (35 min), Atividade 2: Colaboração no Google Docs (30 min), Atividade 3: Conversão de Formatos (20 min) (+12 more)

### Community 70 - "MÓDULO 4 — MEDIDAS E GRANDEZAS"
Cohesion: 0.10
Nodes (21): 15.1 Unidades de Comprimento, 15.2 Unidades de Massa, 16.1 Unidades de Capacidade, 16.2 Unidades de Tempo, 17.1 Conceito de Perímetro, 17.2 Fórmulas, 18.1 Conceito de Área, 18.2 Fórmulas (+13 more)

### Community 71 - "MÓDULO 5 — RACIOCÍNIO LÓGICO E RESOLUÇÃO DE PROBLEMAS"
Cohesion: 0.10
Nodes (21): 20.1 Sequências Numéricas, 20.2 Sequências com Figuras, 21.1 As 4 Etapas de Polya, 21.2 Estratégias Úteis, 22.1 Proposições, 22.2 Conectivos Lógicos, 22.3 Tabelas-Verdade, 23.1 Problemas de Lógica Clássicos (+13 more)

### Community 72 - "AULA 02 — Sistema Operacional e Organização Digital"
Cohesion: 0.10
Nodes (20): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min) (+12 more)

### Community 73 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 74 - "AULA 01-09-10-2026.md"
Cohesion: 0.10
Nodes (20): Atividade 1: Quiz Interativo (15 min), Atividade 2: Estudo de Caso (30 min), Atividade 3: DiscussÃ£o Guiada (20 min), AutogestÃ£o (1.1), AutomaÃ§Ã£o de Testes (2.1-2.4), Metodologia, Timeline da Aula (4 horas), Tipos de Testes (4.1-4.5) (+12 more)

### Community 75 - "AULA 04-09-10-2026.md"
Cohesion: 0.10
Nodes (20): 7.3 â€” ExecuÃ§Ã£o dos Testes (ContinuaÃ§Ã£o), 7.4 â€” MonitoraÃ§Ã£o e Controle de Teste, 7.5 â€” AvaliaÃ§Ã£o dos Resultados, Atividade 1: ExecuÃ§Ã£o PrÃ¡tica (120 min), Atividade 2: MonitoraÃ§Ã£o (45 min), Atividade 3: AnÃ¡lise Conjunta (30 min), DocumentaÃ§Ã£o de Testes, Metodologia (+12 more)

### Community 76 - "TEXTO DE REFORÇO — MÓDULO 1"
Cohesion: 0.10
Nodes (19): Atividade 1, Atividade 1 — Verdadeiro ou Falso, Atividade 2, Atividade 2 — Múltipla Escolha, Atividade 3, Atividade 3 — Resposta Breve, Atividades de Compreensão, Como Ler com Crítica nas Redes Sociais (+11 more)

### Community 77 - "MÓDULO 3 — PROPORCIONALIDADE"
Cohesion: 0.10
Nodes (20): 10.1 Razão, 10.2 Proporção, 11.1 Grandezas Diretamente Proporcionais (GDP), 11.2 Grandezas Inversamente Proporcionais (GIP), 12.1 Regra de Três Simples — GDP, 12.2 Regra de Três Simples — GIP, 13.1 Quando Usar a Regra de Três Composta, 14.1 A Fórmula Fundamental (+12 more)

### Community 78 - "Encontro 4 — Cálculo Mental, Estimativas e Problemas Cotidianos (2h)"
Cohesion: 0.10
Nodes (20): 1.1 Números Inteiros e a Reta Numérica, 1.2 Adição de Inteiros, 1.3 Subtração de Inteiros, 2.1 Regras de Sinais, 2.2 Multiplicação por 10, 100, 1000, 3.1 A Ordem das Operações (PEMDAS), 4.1 Estratégias de Cálculo Mental, 4.2 Estimativas e Arredondamento (+12 more)

### Community 79 - "AULA 06 — Segurança da Informação"
Cohesion: 0.10
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 80 - "AULA 02-09-10-2026.md"
Cohesion: 0.10
Nodes (19): Atividade 1: AnÃ¡lise de Requisitos (30 min), Atividade 2: CriaÃ§Ã£o de Plano (90 min), Atividade 3: Peer Review (30 min), Conceitos Fundamentais (5.1-5.2), EspecificaÃ§Ãµes TÃ©cnicas de Interface, Metodologia, Planejamento de Testes Client-Side (6.1-6.4), Timeline da Aula (4 horas) (+11 more)

### Community 81 - "AULA 03-09-10-2026.md"
Cohesion: 0.10
Nodes (19): 7.1 â€” Planejamento de Teste, 7.2 â€” Desenho dos Testes, 7.3 â€” ExecuÃ§Ã£o dos Testes (IntroduÃ§Ã£o), Atividade 1: Live Coding (30 min), Atividade 2: ExercÃ­cio PrÃ¡tico (90 min), Atividade 3: ApresentaÃ§Ã£o RÃ¡pida (30 min), Metodologia, Processo Fundamental de Teste (7.1-7.5) â€” PARTE 1 (+11 more)

### Community 82 - "PENDENCIAS-PROFESSOR/package.json"
Cohesion: 0.11
Nodes (18): concurrently, dependencies, concurrently, json-server, devDependencies, vite, name, private (+10 more)

### Community 83 - "Verificação de Cobertura: Apostila vs. Ementa"
Cohesion: 0.11
Nodes (18): 1. IDENTIFICAÇÃO DA UC, 2. OBJETIVO DA UC, 3. CAPACIDADES BÁSICAS (CB), 4. CAPACIDADES TÉCNICAS (CT), 5.1 Fundamentos da Comunicação no Mundo do Trabalho, 5.2 Técnicas de Comunicação Oral, 5.3 Estrutura e Normas de Redação Técnica e Formal, 5.4 Ferramentas Digitais para Comunicação (+10 more)

### Community 84 - "EMENTA: Análise de Dados Aplicada à Gestão"
Cohesion: 0.11
Nodes (18): 1. Operações Matemáticas Aplicadas aos Processos Organizacionais, 2. Excel Aplicado à Gestão Organizacional, Ambientes Pedagógicos, Bibliografia Básica, Capacidades Básicas, Capacidades Socioemocionais, Conhecimentos, Conteúdos Formativos (+10 more)

### Community 85 - "gerar_atividade_simples.js"
Cohesion: 0.13
Nodes (15): colDir, colEsq, doc, {
  Document, Packer, Paragraph, TextRun, AlignmentType,
  Table, TableRow, TableCell, WidthType, ShadingType, BorderStyle,
  VerticalAlign, convertMillimetersToTwip
}, filhos, fs, item(), MG (+7 more)

### Community 86 - "AULA 02 — Cidadania Digital, Segurança e Impactos Sociais"
Cohesion: 0.11
Nodes (17): 1. Cidadania Digital (30 min), 2. Segurança na Internet (30 min), 3. Fake News (20 min), 4. Cyberbullying (20 min), 5. Pegada Digital (10 min), Atividade 1: Caçada de Fake News (40 min), Atividade 2: Força de Senha — Teste Online (20 min), Atividade 3: Pegada Digital Pessoal (20 min) (+9 more)

### Community 87 - "📄 Google Docs vs Microsoft Word"
Cohesion: 0.11
Nodes (17): 10. 🔗 Co-autoria (OneDrive), 11. ⭐ Vantagens Principais, 12. ❌ Limitações, 13. ⚖️ Comparação Lado-a-Lado (6 Critérios), 14. 🎯 Matriz de Decisão, 15. 💡 Casos de Uso Ideais, 1. ☁️ Visão Geral - Google Docs, 2. 🔄 Colaboração Real-time (+9 more)

### Community 88 - "GOOGLE SLIDES vs MICROSOFT POWERPOINT"
Cohesion: 0.11
Nodes (17): 1. Google Slides - Visão Geral, 1. Microsoft PowerPoint - Visão Geral, 2. Templates e Design (Slides), 2. Templates Profissionais (PPT), 3. Colaboração em Tempo Real, 3. Desempenho em Apresentação, 4. Apresentação (Slides), 4. Recursos Avançados (PPT) (+9 more)

### Community 89 - "Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.06
Nodes (30): 10. Síntese semântica, 11. Proveniência, 1. Identificação da unidade curricular, 2. Objetivo geral, 3.1 Comunicação oral e escrita no trabalho, 3.2 Segurança da informação, 3.3 Interpretação de informações e textos técnicos, 3.4 Hardware e software (+22 more)

### Community 90 - "redefinir-senha.js"
Cohesion: 0.14
Nodes (5): token, CONFIG, STATUS_SYNC, avisar(), mostrarLinkInvalido()

### Community 91 - "EMENTA — EXPLORAÇÃO DE CARREIRAS INDUSTRIAIS E TECNOLÓGICAS"
Cohesion: 0.12
Nodes (16): 1. IDENTIFICAÇÃO DA UNIDADE CURRICULAR, 2. OBJETIVO GERAL, 3.1 Capacidades Básicas, 3.2 Capacidades Técnicas, 3. CAPACIDADES, 4.1 Panorama do Mercado Industrial e Tecnológico, 4.2 Carreiras Industriais, 4.3 Carreiras em Tecnologia da Informação (+8 more)

### Community 92 - "AULA 03 — Hardware vs Software: Diferenças Fundamentais"
Cohesion: 0.12
Nodes (16): 1. O que é Hardware? (30 min), 2. O que é Software? (30 min), 3. Hardware + Software = Computador Funcionando (20 min), 4. Tipos de Sistemas Operacionais (20 min), Atividade 1: Dissecação de Computador (30 min), Atividade 2: Inventário de Hardware e Software (30 min), Atividade 3: Diagrama Hardware-Software (20 min), Atividades Práticas (+8 more)

### Community 93 - "MÓDULO 6 — INTRODUÇÃO À ÁLGEBRA"
Cohesion: 0.12
Nodes (17): 25.1 O que é uma Variável, 25.2 Expressões Algébricas, 25.3 Calculando o Valor Numérico, 26.1 O que é uma Equação, 26.2 Resolvendo Equações, 27.1 Da Linguagem ao Símbolo, 28.1 Álgebra no Mundo da Tecnologia, Atividade 6.1 (+9 more)

### Community 94 - "ATIVIDADE AULA 10 — Avaliação Prática + Teórica"
Cohesion: 0.12
Nodes (16): 1.1 Projeto Proposto, 1.2 Sua Tarefa — Criar Suite Completa de Testes, 1.3 Rubrica Avaliação Prática, 2.1 Prova Objetiva, 2.2 Formato das Questões, 2.3 Critério de Aprovação, 2.4 Rubrica Teórica, Aprovação (+8 more)

### Community 95 - "criar_apostila_reforco_linguagens.py"
Cohesion: 0.39
Nodes (10): add_activity(), add_bullets(), add_callout(), add_lesson(), add_module_intro(), add_para(), font(), set_cell_margins() (+2 more)

### Community 96 - "AULA 05-14-10-2026.md"
Cohesion: 0.12
Nodes (16): Atividade 1: DemonstraÃ§Ã£o (30 min), Atividade 2: PrÃ¡tica Guiada (120 min), Atividade 3: DocumentaÃ§Ã£o (30 min), ExecuÃ§Ã£o PrÃ¡tica, Metodologia, Testes de Funcionalidade, Testes de Usabilidade (Parte 1), Timeline da Aula (4 horas) (+8 more)

### Community 97 - "Plano de Aulas — Testes de Frontend"
Cohesion: 0.12
Nodes (16): 10.1 Estrutura Proposta, 10.2 Ajustes Pendentes, 10.3 Cronograma, 10.4 Métricas de Sucesso, 10. Observações e Próximos Passos, 1. Identificação do Plano de Aulas, 2. Objetivo Geral da Unidade Curricular, 3.1 Capacidades Técnicas (+8 more)

### Community 98 - "AULA 16 — Scratch: Projetos Criativos Finais e Apresentação"
Cohesion: 0.12
Nodes (15): 1. Recapitulação de Aprendizados (15 min), 2. Ideias de Projetos (20 min), 3. Critérios de Qualidade (15 min), 4. Processo de Desenvolvimento (10 min), Atividade 1: Desenvolvimento do Projeto (80 min), Atividade 2: Apresentação para Turma (40 min), Atividades Práticas, AULA 16 — Scratch: Projetos Criativos Finais e Apresentação (+7 more)

### Community 99 - "📄 GOOGLE DOCS vs MICROSOFT WORD"
Cohesion: 0.12
Nodes (15): 1. Google Docs - Visão Geral, 1. Microsoft Word - Visão Geral, 2. Recursos de Colaboração (Google), 2. Recursos de Colaboração (Word), 3. Formatação e Estilo (Google), 3. Formatação e Estilo (Word), 4. Exportação e Integração (Google), 4. Recursos Avançados (Word) (+7 more)

### Community 100 - "GOOGLE SHEETS vs MICROSOFT EXCEL"
Cohesion: 0.12
Nodes (15): 1. Google Sheets - Visão Geral, 1. Microsoft Excel - Visão Geral, 2. Fórmulas Avançadas (Excel), 2. Fórmulas e Funções (Sheets), 3. Colaboração em Tempo Real, 3. Desempenho em Grandes Dados, 4. Análise de Dados (Sheets), 4. Análise e Visualização (Excel) (+7 more)

### Community 101 - "EMENTA DA UNIDADE CURRICULAR"
Cohesion: 0.12
Nodes (15): 1. Identificação da UC, 2. Objetivo da UC, 3.1 Capacidades Básicas, 3.2 Capacidades Técnicas, 3. Capacidades, 4.1 Fundamentos da Comunicação no Mundo do Trabalho, 4.2 Técnicas de Comunicação Oral, 4.3 Estrutura e Normas de Redação Técnica e Formal (+7 more)

### Community 102 - "EMENTA DA UNIDADE CURRICULAR"
Cohesion: 0.12
Nodes (15): 1. Identificação da UC, 2. Objetivo da UC, 3.1 Capacidades Básicas, 3.2 Capacidades Técnicas, 3. Capacidades, 4.1 Fundamentos da Comunicação no Mundo do Trabalho, 4.2 Técnicas de Comunicação Oral, 4.3 Estrutura e Normas de Redação Técnica e Formal (+7 more)

### Community 103 - "Ementa Explicada - Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.06
Nodes (30): 10. Síntese semântica, 11. Proveniência, 1. Identificação da unidade curricular, 2. Objetivo geral, 3.1 Comunicação oral e escrita no trabalho, 3.2 Segurança da informação, 3.3 Interpretação de informações e textos técnicos, 3.4 Hardware e software (+22 more)

### Community 104 - "📋 Instruções de Uso - Dashboard de Controle de Matérias"
Cohesion: 0.07
Nodes (26): 🛠️ API REST (JSON Server), 🔍 Buscar Matérias, 📊 Campos de Cada Matéria, 🚀 Como Executar, ✅ Criar Nova Matéria, Dados não aparecem, 💾 Dados Persistem, 🗑️ Deletar Matéria (+18 more)

### Community 105 - "ATIVIDADE AULA 02 — Configuração de Ambiente e Ferramentas"
Cohesion: 0.12
Nodes (15): 1.1 Objetivos da Atividade, 1.2 Instruções Passo-a-Passo, 1. Tarefa Principal — Setup Completo de Projeto, 2. Checklist de Configuração, 3. Tarefa Complementar — Troubleshooting, 4. Critério de Avaliação, 5. Recursos Fornecidos, 6. Próximos Passos (+7 more)

### Community 106 - "criar-contas-purelymail.js"
Cohesion: 0.19
Nodes (13): args, criarUsuario(), DOMINIO, ENTRADA, escaparCsv(), EXECUTAR, gerarSenha(), lerContatos() (+5 more)

### Community 107 - "AULA 04 — Sistema Operacional: Interface, Arquivos e Pastas"
Cohesion: 0.13
Nodes (14): 1. Componentes da Interface (30 min), 2. Arquivos e Pastas (30 min), 3. Operações com Arquivos (30 min), 4. Explorando o Explorador de Arquivos (20 min), Atividade 1: Organizar Pasta de Documentos (40 min), Atividade 2: Jogos de Atalhos (20 min), Atividade 3: Limpeza da Lixeira (20 min), Atividades Práticas (+6 more)

### Community 108 - "Conteúdo"
Cohesion: 0.13
Nodes (14): 1. O que é Programação? (15 min), 2. Linguagens de Programação (20 min), 3. Conceitos Básicos — Variáveis e Tipos de Dados (20 min), 4. Tipos de Dados (15 min), 5. Operadores (15 min), 6. Estrutura de Programa (10 min), 7. Estrutura de Programa (10 min), Atividade 1: Criar Variáveis (30 min) (+6 more)

### Community 109 - "EMENTA — NOÇÕES DE ELETRICIDADE E CIRCUITOS BÁSICOS"
Cohesion: 0.13
Nodes (14): 1. IDENTIFICAÇÃO DA UNIDADE CURRICULAR, 2. OBJETIVO, 3.1 Capacidades Básicas, 3.2 Capacidades Técnicas, 3. CAPACIDADES, 4.1 Noções de Eletricidade Básica, 4.2 Instalações Elétricas Prediais, 4.3 Técnicas de Diagnóstico (+6 more)

### Community 110 - "Encontro 13-14 — Pontuação e Seu Uso Comunicativo (4h)"
Cohesion: 0.13
Nodes (15): Atividades, Atividades, Atividades, Encontro 13-14 — Pontuação e Seu Uso Comunicativo (4h), Encontro 15-16 — Concordância Verbal (4h), Encontro 17 — Concordância Nominal e Revisão (4h), Explicação: O adjetivo concorda com o substantivo, Explicação: O verbo concorda com o sujeito (+7 more)

### Community 111 - "AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.18
Nodes (11): AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação, CENÁRIO, Conteúdo obrigatório, Distribuição dos Pontos, ENTREGA, Exigências, Formatação obrigatória, Instruções Gerais (+3 more)

### Community 112 - "PLANO DE AULAS — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.13
Nodes (14): 1. Estrutura Geral, 2. Blocos de Conteúdo, 3. Cobertura da Ementa, 4. Ambientes e Recursos, 5. Avaliação, 6. Encadeamento dos Produtos do Aluno, 7. Arquivos Gerados, 8. Preparação Prévia do Professor (+6 more)

### Community 113 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.07
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 114 - "ATIVIDADE AULA 03 — Testes Unitários em JavaScript (Parte 1)"
Cohesion: 0.13
Nodes (14): 1.1 Contexto, 1.2 Arquivo a Testar: `src/User.js`, 1.3 Instruções — Escrever Testes, 1.4 Seu Trabalho, 1.5 Matchers Permitidos, 1. Tarefa Principal — Suite de Testes Unitários Completa, 2. Tarefa Complementar — Code Review, 3. Executar Testes (+6 more)

### Community 115 - "ENCONTRO 18 — Projeto Final: Pitch de Negócio"
Cohesion: 0.14
Nodes (14): Atividade Final, Como dar um feedback construtivo, Conteúdo, Dicas para uma boa apresentação, Documentários e vídeos sugeridos, ENCONTRO 18 — Projeto Final: Pitch de Negócio, Estrutura do pitch (5 minutos), Glossário (+6 more)

### Community 116 - "ENCONTROS"
Cohesion: 0.14
Nodes (13): Atividade 3 — Mapa Mental da Indústria 4.0, Canais do YouTube (recomendados), ENCONTRO 3 — Indústria 4.0: A Revolução Que Está Acontecendo Agora, ENCONTROS, GLOSSÁRIO, Livros e Publicações, Objetivos, REFERÊNCIAS SUGERIDAS (+5 more)

### Community 117 - "AULA 05 — Produtividade Digital: Editores de Texto e Processadores de Palavras"
Cohesion: 0.14
Nodes (13): 1. Tipos de Editores de Texto (20 min), 2. Interface de Editor de Texto (30 min), 3. Criando Documento Profissional (30 min), 4. Recursos Avançados (20 min), Atividade 1: Criar Currículo Simples (45 min), Atividade 2: Carta Formal (30 min), Atividades Práticas, AULA 05 — Produtividade Digital: Editores de Texto e Processadores de Palavras (+5 more)

### Community 118 - "AULA 06 — Produtividade Digital: Apresentações (Google Slides/PowerPoint)"
Cohesion: 0.14
Nodes (13): 1. Introdução a Apresentações (20 min), 2. Estrutura de Apresentação (20 min), 3. Design e Formatação (20 min), 4. Recursos e Funcionalidades (20 min), Atividade 1: Criar Apresentação sobre Tecnologia (50 min), Atividade 2: Apresentar para Turma (30 min), Atividades Práticas, AULA 06 — Produtividade Digital: Apresentações (Google Slides/PowerPoint) (+5 more)

### Community 119 - "AULA 07 — Produtividade Digital: Planilhas Eletrônicas (Excel/Google Sheets)"
Cohesion: 0.14
Nodes (13): 1. O que é Planilha Eletrônica? (15 min), 2. Operações Básicas (25 min), 3. Fórmulas Simples (30 min), 4. Gráficos (20 min), Atividade 1: Planilha de Notas (40 min), Atividade 2: Análise de Gastos (40 min), Atividades Práticas, AULA 07 — Produtividade Digital: Planilhas Eletrônicas (Excel/Google Sheets) (+5 more)

### Community 120 - "AULA 08 — Internet: Navegadores e Boas Práticas de Pesquisa"
Cohesion: 0.14
Nodes (13): 1. Navegadores Web (30 min), 2. Pesquisando Efetivamente (30 min), 3. Segurança e Privacidade (30 min), 4. Pesquisa Acadêmica (20 min), Atividade 1: Buscas Progressivas (30 min), Atividade 2: Análise de Confiabilidade (30 min), Atividade 3: Pesquisa Acadêmica (20 min), Atividades Práticas (+5 more)

### Community 121 - "AVALIAÇÃO FINAL — Fundamentos da Tecnologia e Programação"
Cohesion: 0.14
Nodes (13): AVALIAÇÃO FINAL — Fundamentos da Tecnologia e Programação, Estrutura da Avaliação, GABARITO (Uso do Professor), Instruções, NOTA FINAL, Objetivos da Avaliação, Observações Finais, PARTE 1 — TESTE TEÓRICO (25 minutos) (+5 more)

### Community 122 - "Apostila_Comunicacao_15h_Versao_Joao_Revisado_2026_02.md"
Cohesion: 0.14
Nodes (13): Como Esta Apostila Está Organizada, Como você será avaliado, GLOSSÁRIO, Livros sugeridos:, Normas técnicas:, Objetivo da Unidade Curricular, Percurso Formativo Completo, Recursos online: (+5 more)

### Community 123 - "📋 ATIVIDADES MANUAIS — ENTREGA OBRIGATÓRIA"
Cohesion: 0.14
Nodes (13): ✍️ ATIVIDADE 1 — Mapa Mental de Ferramentas, ✍️ ATIVIDADE 2 — Simulação de E-mail Profissional, ✍️ ATIVIDADE 3 — Análise de E-mails Suspeitos, ✍️ ATIVIDADE 4 — Criador de Senhas Forte, ✍️ ATIVIDADE 5 — Roteiro de Apresentação, ✍️ ATIVIDADE 6 — Avaliação de Colegas, ✍️ ATIVIDADE 7 — Plano da Avaliação Final, ✍️ ATIVIDADE 8 — Autorreflexão Final (+5 more)

### Community 124 - "Conteúdo"
Cohesion: 0.14
Nodes (14): Atividade Prática, Comparativo geral, Conteúdo, ENCONTRO 11 — TIPOS DE LÂMPADAS, Evolução das lâmpadas, Lâmpada de Vapor de Mercúrio, Lâmpada de Vapor de Sódio, Lâmpada de Vapor Metálico (+6 more)

### Community 125 - "Passos"
Cohesion: 0.22
Nodes (8): Criar Apostila de Reforço de Linguagens, Passo 1: Estrutura pedagógica e visual, Passo 2: Conteúdo, Passo 3: Geração, Passo 4: Acabamento estrutural, Passo 5: Commit, Passos, Status Geral

### Community 126 - "Encontro 18-19 — Debate e Argumentação (4h)"
Cohesion: 0.14
Nodes (14): Atividade: Apresentação de 3 minutos, Atividade: Mini-debate, Atividades de simulação, Encontro 18-19 — Debate e Argumentação (4h), Encontro 20-21 — Apresentação Oral e Escuta Ativa (4h), Encontro 22 — Simulação de Situações Comunicativas (4h), Explicação: Como argumentar bem?, Explicação: Como fazer uma boa apresentação? (+6 more)

### Community 127 - "ATIVIDADE AULA 04 — Testes Unitários em JavaScript (Parte 2)"
Cohesion: 0.14
Nodes (13): 1.1 Arquivo a Testar: `src/UserService.js`, 1.2 Instruções — Criar Suite de Testes, 1.3 Seu Trabalho — Completar a Suite, 1.4 Dicas de Implementação, 1. Tarefa Principal — Testes Assíncronos com Mocks, 2. Tarefa Complementar — Refactoring, 3. Executar Testes, 4. Critério de Avaliação (+5 more)

### Community 128 - "2.2 Análise por Entidade"
Cohesion: 0.15
Nodes (13): 2.1 Definição de 1FN, 2.2 Análise por Entidade, 2.3 Conclusão sobre 1FN, 2. VERIFICAÇÃO DE 1ª FORMA NORMAL (1FN), AVALIACAO - Verificação 1FN, CARDAPIO - Verificação 1FN, ENDERECO - Verificação 1FN, ENTREGADOR - Verificação 1FN (+5 more)

### Community 129 - "Ementa — Competências Socioemocionais e Empreendedorismo"
Cohesion: 0.15
Nodes (12): Avaliação, Bloco 1 — Sustentabilidade e Responsabilidade Ambiental, Bloco 2 — Empreendedorismo, Básicas / Técnicas, Capacidades, Conhecimentos, Ementa — Competências Socioemocionais e Empreendedorismo, Estrutura de Encontros (+4 more)

### Community 130 - "AULA 09 — Segurança na Internet: Senhas, Vírus e Privacidade"
Cohesion: 0.15
Nodes (12): 1. Senhas Seguras (25 min), 2. Vírus e Malware (25 min), 3. Privacidade Online (20 min), 4. Autenticação Segura (20 min), Atividade 1: Teste de Força de Senha (25 min), Atividade 2: Verificar Exposição de Dados (25 min), Atividade 3: Limpar Histórico e Cookies (20 min), Atividades Práticas (+4 more)

### Community 131 - "GOOGLE DOCS"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Recursos Principais, 3. Colaboração Real-time, 4. Templates, 5. Integração, 6. Exportação, 7. Compatibilidade DOCX, 8. Segurança e Privacidade (+4 more)

### Community 132 - "GOOGLE DRIVE"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Sincronização, 3. Compartilhamento, 4. Integração Google, 5. Controle de Versão, 6. Segurança, 7. Busca e Organização, 8. Integração API (+4 more)

### Community 133 - "GOOGLE GMAIL"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Funcionalidades, 3. Segurança, 4. Produtividade, 5. Colaboração, 6. Integração, 7. Gerenciamento, 8. Customização (+4 more)

### Community 134 - "Status Geral"
Cohesion: 0.25
Nodes (7): Atualizar Graphify e Resumir Estrutura, Passo 1: Atualizar o grafo, Passo 2: Inventariar a raiz, Passo 3: Inventariar `sistema/`, Passo 4: Atualizar o contexto, Passo 5: Commit e push, Status Geral

### Community 135 - "GOOGLE MEET"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Funcionalidades, 3. Qualidade de Áudio/Vídeo, 4. Recursos Avançados, 5. Segurança e Privacidade, 6. Integração, 7. Gravação e Transcrição, 8. Dispositivos Suportados (+4 more)

### Community 136 - "GOOGLE SHEETS"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Fórmulas Criativas, 3. Análise de Dados, 4. Colaboração, 5. Integração, 6. Automação (Apps Script), 7. Exportação, 8. Performance (+4 more)

### Community 137 - "GOOGLE SLIDES"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Templates e Design, 3. Animações, 4. Colaboração Real-time, 5. Apresentação, 6. Engajamento Audiência, 7. Integração, 8. Exportação (+4 more)

### Community 138 - "Passos"
Cohesion: 0.25
Nodes (7): Converter Lista de Presença CEPLAS Barragem 2026, Passo 1: Extrair o PDF, Passo 2: Estruturar os dados, Passo 3: Criar a planilha, Passo 4: Commit, Passos, Status Geral

### Community 139 - "Status Geral"
Cohesion: 0.25
Nodes (7): Correção do salvamento de avaliações, Passo 1: Identificar a causa, Passo 2: Implementar gravação compatível, Passo 3: Melhorar a mensagem, Passo 4: Revisar, Passo 5: Commit e push, Status Geral

### Community 140 - "MICROSOFT EXCEL"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Fórmulas Poderosas, 3. Análise de Dados, 4. Power Query (ETL), 5. Power Pivot (OLAP), 6. Macros e Automação, 7. Simulação e Cenários, 8. Compartilhamento (+4 more)

### Community 141 - "MICROSOFT OUTLOOK"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Gerenciamento Email, 3. Calendário Integrado, 4. Gerenciamento de Tarefas, 5. Contatos, 6. Segurança, 7. Colaboração, 8. Produtividade IA (+4 more)

### Community 142 - "MICROSOFT POWERPOINT"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Templates Profissionais, 3. Animações e Transições, 4. Elementos Visuais, 5. Apresentação Avançada, 6. Gravação e Edição, 7. Interatividade, 8. Colaboração (+4 more)

### Community 143 - "MICROSOFT WORD"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Formatação Avançada, 3. Recursos Documentos, 4. Colaboração, 5. Tabelas e Imagens, 6. Templates Profissionais, 7. Segurança Documentos, 8. Exportação (+4 more)

### Community 144 - "MICROSOFT ONEDRIVE"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Sincronização Desktop, 3. Compartilhamento, 4. Integração Office 365, 5. Controle de Versão, 6. Segurança, 7. Colaboração, 8. Busca e Organização (+4 more)

### Community 145 - "MICROSOFT TEAMS"
Cohesion: 0.15
Nodes (12): 1. Visão Geral, 2. Chat e Mensagens, 3. Canais, 4. Videoconferência, 5. Compartilhamento de Arquivos, 6. Integrações e Apps, 7. Segurança, 8. Administração (+4 more)

### Community 146 - "ENCONTRO 7 — Comunicação digital e segurança"
Cohesion: 0.15
Nodes (13): 7.1 O e-mail profissional, 7.2 Netiqueta e escolha do canal, 7.3 Segurança na comunicação digital, Ao final deste encontro, você será capaz de:, Atividade 7.1 — Do erro ao acerto (35 min — duplas), Atividade 7.2 — Detector de golpes (40 min — grupos), ENCONTRO 7 — Comunicação digital e segurança, Erros que custam caro (+5 more)

### Community 147 - "EMENTA — OFICINAS DE IMPRESSÃO 3D E ROBÓTICA"
Cohesion: 0.15
Nodes (12): 1. IDENTIFICAÇÃO DA UNIDADE CURRICULAR, 2. OBJETIVO, 3.1 Impressão 3D, 3.2 Robótica, 3. CAPACIDADES BÁSICAS E TÉCNICAS, 4.1 Modelagem e Impressão 3D, 4.2 Robótica, 4. CONHECIMENTOS (+4 more)

### Community 148 - "Encontro 25-26 — Leitura Crítica e Análise de Discurso (4h)"
Cohesion: 0.11
Nodes (18): Atividade final, Atividades, Atividades, Atividades, Encontro 23-24 — Gêneros Textuais no Cotidiano (4h), Encontro 25-26 — Leitura Crítica e Análise de Discurso (4h), Encontro 27-28 — Produção de Sínteses e Mapas Mentais (4h), Encontro 29-30 — Avaliação Final e Socialização (3h) (+10 more)

### Community 149 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 150 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.08
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 151 - "AULA 07-14-10-2026.md"
Cohesion: 0.15
Nodes (12): AutomaÃ§Ã£o de Testes (2.1-2.4), ConfiguraÃ§Ã£o de Ambiente, Metodologia, Primeiros Scripts de Teste, Timeline da Aula (4 horas), âœ… CritÃ©rios de AvaliaÃ§Ã£o, ðŸ« EstratÃ©gias de Ensino, ðŸ› ï¸ Recursos NecessÃ¡rios (+4 more)

### Community 152 - "AULA 09-15-10-2026.md"
Cohesion: 0.15
Nodes (12): DocumentaÃ§Ã£o de Testes (Bloco 06), IntegraÃ§Ã£o ContÃ­nua (CI/CD), Metodologia (Parte 1: DocumentaÃ§Ã£o â€” 2h), Metodologia (Parte 2: CI/CD â€” 2h), MÃ©todos, Normas e Defeitos (Bloco 07), âœ… CritÃ©rios de AvaliaÃ§Ã£o, ðŸ« EstratÃ©gias de Ensino, ðŸ› ï¸ Recursos NecessÃ¡rios (+4 more)

### Community 153 - "links-turmas.js"
Cohesion: 0.39
Nodes (11): alternarAtivo(), avisar(), carregar(), copiar(), escapar(), fecharModal(), links, projetar() (+3 more)

### Community 154 - "AULA 10 — Pensamento Computacional: Decomposição e Padrões"
Cohesion: 0.17
Nodes (11): 1. O que é Pensamento Computacional? (20 min), 2. Decomposição (25 min), 3. Reconhecimento de Padrões (25 min), 4. Abstração (15 min), Atividade 1: Decomposição em Grupo (30 min), Atividade 2: Padrões — Desafio Desplugado (30 min), Atividades Práticas — Desplugadas (Sem Computador), AULA 10 — Pensamento Computacional: Decomposição e Padrões (+3 more)

### Community 155 - "AULA 11 — Algoritmos: Sequência, Decisão e Repetição"
Cohesion: 0.17
Nodes (11): 1. O que é Algoritmo? (15 min), 2. Sequência (20 min), 3. Decisão / Condicional (25 min), 4. Repetição / Laço (25 min), Atividade 1: Fluxogramas (35 min), Atividade 2: Pseudocódigo (35 min), Atividades Práticas, AULA 11 — Algoritmos: Sequência, Decisão e Repetição (+3 more)

### Community 156 - "AULA 14 — Scratch: Primeiros Projetos e Movimentação"
Cohesion: 0.17
Nodes (11): 1. Introdução ao Scratch (15 min), 2. Sprites e Palco (20 min), 3. Blocos de Movimento (25 min), 4. Blocos de Evento (15 min), Atividade 1: Animar Sprite (40 min), Atividade 2: Controlar Sprite com Teclado (40 min), Atividades Práticas, AULA 14 — Scratch: Primeiros Projetos e Movimentação (+3 more)

### Community 157 - "AULA 15 — Scratch: Condições, Variáveis e Lógica"
Cohesion: 0.17
Nodes (11): 1. Variáveis em Scratch (20 min), 2. Condições (SE/ENTÃO) (20 min), 3. Blocos Importantes (25 min), 4. Colisão e Detecção (15 min), Atividade 1: Jogo de Caça (50 min), Atividade 2: Desafio Criativo (30 min), Atividades Práticas, AULA 15 — Scratch: Condições, Variáveis e Lógica (+3 more)

### Community 158 - "Conhecimentos"
Cohesion: 0.17
Nodes (11): Bloco 1 — Tecnologia e Mundo Digital, Bloco 2 — Hardware, Software e Sistema Operacional, Bloco 3 — Produtividade Digital, Bloco 4 — Internet e Uso Seguro, Bloco 5 — Pensamento Computacional e Algoritmos, Bloco 6 — Programação em Blocos (Scratch), Capacidades Básicas e Técnicas, Conhecimentos (+3 more)

### Community 159 - "Diário de Classe — Aulas até 01/09/2026"
Cohesion: 0.17
Nodes (11): Aulas Confirmadas (com conteúdo definido), Diário de Classe — Aulas até 01/09/2026, Docentes, Estrutura de Aulas, Módulo 1: Introdução e Fundamentos (Aulas 1-2), Módulo 2: Comunicação Profissional Aplicada (Aulas 3-5), Módulo 3: Projetos Integrados (Aula 6), Módulo 4: Complementares (+3 more)

### Community 160 - "Técnicas de diagnóstico"
Cohesion: 0.17
Nodes (12): 1. Inspeção Visual, 2. Diagnóstico por Teste, 3. Diagnóstico por Comparação com Componente, 4. Diagnóstico por Comparação com Outro Equipamento, 5. Análise do Funcionamento, A importância do diagnóstico, Atividade Prática, Conteúdo (+4 more)

### Community 161 - "Conteúdo"
Cohesion: 0.17
Nodes (12): Atividade Prática, Conteúdo, ENCONTRO 8 — Boas Práticas de Design para Impressão 3D, Objetivos, Por que o design para impressão 3D é especial?, Quando usar suportes?, Regra 1 — Espessura mínima de paredes, Regra 2 — Regra dos 45° (+4 more)

### Community 162 - "ATIVIDADE AULA 01 — Fundamentos de Testes de Software"
Cohesion: 0.17
Nodes (11): 1.1 Contexto, 1.2 Instruções Passo-a-Passo, 1. Tarefa Principal — Análise de Projeto Real, 2. Tarefa Complementar — Quiz Interativo, 3. Critério de Avaliação, 4. Recursos Fornecidos, 5. Dúvidas Frequentes, 6. Próximos Passos (+3 more)

### Community 163 - "1. Tarefa Principal — Testar Componente de Formulário"
Cohesion: 0.17
Nodes (11): 1.1 Arquivo HTML a Testar: `src/LoginForm.html`, 1.2 Seu Trabalho — Criar Testes de Integração, 1.3 Requisitos de Testes, 1.4 Dicas Técnicas, 1. Tarefa Principal — Testar Componente de Formulário, 2. Tarefa Complementar — Adicionar Novo Teste, 3. Executar Testes, 4. Critério de Avaliação (+3 more)

### Community 164 - "Status Geral"
Cohesion: 0.29
Nodes (6): Adicionar Apostila por Matéria, Passo 1: Persistir o caminho, Passo 2: Integrar ao cadastro, Passo 3: Adicionar botão e modal, Passo 4: Commit e publicação, Status Geral

### Community 165 - "Status Geral"
Cohesion: 0.29
Nodes (6): Adicionar e Editar Matérias no Curso, Passo 1: Criar formulário, Passo 2: Cadastrar e vincular, Passo 3: Editar matéria, Passo 4: Commit e publicação, Status Geral

### Community 166 - "Status Geral"
Cohesion: 0.29
Nodes (6): Adicionar Status de Plano de Aula e Plano de Ensino, Passo 1: Adicionar campos no banco, Passo 2: Adicionar seletores, Passo 3: Exibir e persistir, Passo 4: Commit e publicação, Status Geral

### Community 167 - "Status Geral"
Cohesion: 0.29
Nodes (6): Ajuste das Ementas, Passo 1: Mapear caminhos-padrão, Passo 2: Aplicar fallback, Passo 3: Revisar alterações, Passo 4: Commit e publicação, Status Geral

### Community 168 - "AULA 06-14-10-2026.md"
Cohesion: 0.17
Nodes (11): AnÃ¡lise de Defeitos, ContinuaÃ§Ã£o de ExecuÃ§Ã£o PrÃ¡tica, Metodologia, Testes de Usabilidade (Parte 2), âœ… CritÃ©rios de AvaliaÃ§Ã£o, ðŸ« EstratÃ©gias de Ensino, ðŸ› ï¸ Recursos NecessÃ¡rios, ðŸ“Œ InformaÃ§Ãµes Gerais (+3 more)

### Community 169 - "AULA 08-14-10-2026.md"
Cohesion: 0.17
Nodes (11): Metodologia, MonitoraÃ§Ã£o e Controle (7.4), Testes de IntegraÃ§Ã£o, Testes End-to-End (E2E), âœ… CritÃ©rios de AvaliaÃ§Ã£o, ðŸ« EstratÃ©gias de Ensino, ðŸ› ï¸ Recursos NecessÃ¡rios, ðŸ“Œ InformaÃ§Ãµes Gerais (+3 more)

### Community 170 - "Plano de Aulas — Testes de Frontend"
Cohesion: 0.17
Nodes (11): 10. Métricas de Sucesso, 1. Identificação do Plano de Aulas, 2. Objetivo Geral da Unidade Curricular, 5.1 Avaliação Prática — 4 horas (Aula 10 — parte final), 5.2 Avaliação Teórica Objetiva — 4 horas (Aula 10 — final), 5.3 Cálculo Final de Notas, 5.4 Relatório de Competências, 5. Avaliações (+3 more)

### Community 171 - "Status Geral"
Cohesion: 0.29
Nodes (6): Avaliações por Matéria, Passo 1: Criar estrutura no Supabase, Passo 2: Adicionar botão e modal, Passo 3: Garantir e listar avaliações, Passo 4: Commit e publicação, Status Geral

### Community 172 - "Status Geral"
Cohesion: 0.29
Nodes (6): Converter Ementa de Introdução à TIC para Markdown, Passo 1: Extrair o conteúdo integral, Passo 2: Organizar e explicar para IA, Passo 3: Criar o Markdown solicitado, Passo 4: Commit e publicação, Status Geral

### Community 173 - "Status Geral"
Cohesion: 0.29
Nodes (6): Corrigir Carregamento da Ementa de TIC, Passo 1: Identificar a causa, Passo 2: Adicionar o caminho-padrão, Passo 3: Aplicar fallback, Passo 4: Commit e publicação, Status Geral

### Community 174 - "Status Geral"
Cohesion: 0.29
Nodes (6): Corrigir Listas Numeradas da Apostila, Passo 1: Identificar a causa, Passo 2: Corrigir a captura, Passo 3: Revisar os blocos afetados, Passo 4: Commit e publicação, Status Geral

### Community 175 - "api.js"
Cohesion: 0.24
Nodes (10): chamarAuth(), Db, exigirLogin(), requisitar(), Sessao, sincronizarResend(), testarResend(), traduzirErro() (+2 more)

### Community 176 - "login.js"
Cohesion: 0.20
Nodes (8): Auth, abaCadastrar, abaEntrar, aviso, esconderAviso(), formCadastrar, formEntrar, trocarAba()

### Community 177 - "LineForWriting"
Cohesion: 0.24
Nodes (8): determine_lines_needed(), LineForWriting, main(), markdown_to_pdf(), Flowable, Cria linhas para os alunos escreverem respostas, Determina a quantidade de linhas necessárias conforme a pergunta, Converte Markdown para PDF com quantidade inteligente de linhas

### Community 178 - "FUNDAMENTOS DA TECNOLOGIA E PROGRAMAÇÃO"
Cohesion: 0.18
Nodes (10): APOSTILA DO ALUNO, Apresentação, Como Esta Apostila Está Organizada, FUNDAMENTOS DA TECNOLOGIA E PROGRAMAÇÃO, Glossário, Objetivo da UC, Percurso de Aprendizagem, RIO DO SUL MAIS TECH (+2 more)

### Community 179 - "AULA 12 — Fluxogramas e Representação Visual de Algoritmos"
Cohesion: 0.18
Nodes (10): 1. Símbolos de Fluxograma (20 min), 2. Regras de Fluxograma (15 min), 3. Exemplos Práticos (30 min), Atividade 1: Desenhar Fluxogramas (50 min), Atividade 2: Interpretar Fluxograma (40 min), Atividades Práticas, AULA 12 — Fluxogramas e Representação Visual de Algoritmos, Conteúdo (+2 more)

### Community 180 - "📋 Tarefas Nesta Pasta"
Cohesion: 0.18
Nodes (10): 1. GOOGLE-DOCS-VERSAO-CLAUDE.png, 2. GOOGLE-SHEETS-VERSAO-CLAUDE.png, 3. GOOGLE-SLIDES-VERSAO-CLAUDE.png, 4. GOOGLE-GMAIL-VERSAO-CLAUDE.png, 5. GOOGLE-DRIVE-VERSAO-CLAUDE.png, 6. GOOGLE-MEET-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🟢 GOOGLE WORKSPACE — Infográficos Detalhados do Google (+2 more)

### Community 181 - "📋 Tarefas Nesta Pasta"
Cohesion: 0.18
Nodes (10): 1. MICROSOFT-WORD-VERSAO-CLAUDE.png, 2. MICROSOFT-EXCEL-VERSAO-CLAUDE.png, 3. MICROSOFT-POWERPOINT-VERSAO-CLAUDE.png, 4. MICROSOFT-ONENOTE-VERSAO-CLAUDE.png, 5. MICROSOFT-OUTLOOK-VERSAO-CLAUDE.png, 6. MICROSOFT-ACCESS-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🔵 MICROSOFT OFFICE — Infográficos Detalhados do Microsoft (+2 more)

### Community 182 - "Status Geral"
Cohesion: 0.29
Nodes (6): Criar Apostila de Comunicação Oral e Escrita - Versão Codex, Passo 1: Interpretar a ementa, Passo 2: Criar o conteúdo didático, Passo 3: Gerar a apostila DOCX, Passo 4: Commit e publicação, Status Geral

### Community 183 - "Status Geral"
Cohesion: 0.29
Nodes (6): Criar Texto de Reforço de Linguagens — 25/08/2026, Passo 1: Ler a ementa, Passo 2: Redigir o texto, Passo 3: Gerar o Word, Passo 4: Commit e push, Status Geral

### Community 184 - "Status Geral"
Cohesion: 0.29
Nodes (6): CRUD de Avaliações por Matéria, Passo 1: Criar formulário, Passo 2: Cadastrar e editar, Passo 3: Excluir mantendo o mínimo, Passo 4: Commit e publicação, Status Geral

### Community 185 - "Status Geral"
Cohesion: 0.29
Nodes (6): CRUD Minhas Pendências, Passo 1: Criar formulário e ações, Passo 2: Implementar persistência e edição no Supabase, Passo 3: Excluir e criar estrutura SQL, Passo 4: Commit e publicação, Status Geral

### Community 186 - "📊 VERSAO-CLAUDE — Infográficos Gerados"
Cohesion: 0.18
Nodes (10): 🔴 01-COMPARATIVOS (3 infográficos), 🟢 02-GOOGLE-WORKSPACE (6 infográficos), 🔵 03-MICROSOFT-OFFICE (6 infográficos), 🟠 04-MICROSOFT-ECOSYSTEM (3 infográficos), 📂 Estrutura de Pastas, 📝 Notas, ✅ Requisitos de Qualidade, 🎯 Resumo por Categoria (+2 more)

### Community 187 - "Status Geral"
Cohesion: 0.29
Nodes (6): Ementa por Matéria, Passo 1: Persistir caminhos, Passo 2: Editar caminho, Passo 3: Visualizar ementa, Passo 4: Commit e publicação, Status Geral

### Community 188 - "📋 Tarefas Nesta Pasta"
Cohesion: 0.18
Nodes (10): 1. GOOGLE-DOCS-VERSAO-CLAUDE.png, 2. GOOGLE-SHEETS-VERSAO-CLAUDE.png, 3. GOOGLE-SLIDES-VERSAO-CLAUDE.png, 4. GOOGLE-GMAIL-VERSAO-CLAUDE.png, 5. GOOGLE-DRIVE-VERSAO-CLAUDE.png, 6. GOOGLE-MEET-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🟢 GOOGLE WORKSPACE — Infográficos Detalhados do Google (+2 more)

### Community 189 - "📋 Tarefas Nesta Pasta"
Cohesion: 0.18
Nodes (10): 1. MICROSOFT-WORD-VERSAO-CLAUDE.png, 2. MICROSOFT-EXCEL-VERSAO-CLAUDE.png, 3. MICROSOFT-POWERPOINT-VERSAO-CLAUDE.png, 4. MICROSOFT-ONENOTE-VERSAO-CLAUDE.png, 5. MICROSOFT-OUTLOOK-VERSAO-CLAUDE.png, 6. MICROSOFT-ACCESS-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🔵 MICROSOFT OFFICE — Infográficos Detalhados do Microsoft (+2 more)

### Community 190 - "📊 VERSAO-CLAUDE — Infográficos Gerados"
Cohesion: 0.18
Nodes (10): 🔴 01-COMPARATIVOS (3 infográficos), 🟢 02-GOOGLE-WORKSPACE (6 infográficos), 🔵 03-MICROSOFT-OFFICE (6 infográficos), 🟠 04-MICROSOFT-ECOSYSTEM (3 infográficos), 📂 Estrutura de Pastas, 📝 Notas, ✅ Requisitos de Qualidade, 🎯 Resumo por Categoria (+2 more)

### Community 191 - "3.1 Falar em público sem travar"
Cohesion: 0.18
Nodes (11): 3.1 Falar em público sem travar, 3.2 Dar e receber retorno, Antes de falar: os três Ps, Ao final deste encontro, você será capaz de:, Atividade 3.1 — Um minuto sobre mim (60 min — individual), Atividade 3.2 — Rodada de retorno (20 min — em grupos), Como dar um bom retorno:, Dica Prática (+3 more)

### Community 192 - "ENCONTRO 4 — Reuniões, atendimento e comunicação assertiva"
Cohesion: 0.18
Nodes (11): 4.1 Reunião: falar na hora certa, 4.2 Assertividade: nem passivo, nem agressivo, 4.3 Atendimento: a voz da empresa, Ao final deste encontro, você será capaz de:, As quatro etapas do atendimento:, Atividade 4.1 — Reunião simulada (45 min — grupos de 5), Atividade 4.2 — Dramatização de atendimento (30 min — duplas), Como participar efetivamente: (+3 more)

### Community 193 - "Conteúdo"
Cohesion: 0.18
Nodes (11): Atividade Prática, CIPA, Conteúdo, ENCONTRO 16 — SEGURANÇA DO TRABALHO: NR 10, EPI E EPC, EPC — Equipamento de Proteção Coletiva, EPI — Equipamento de Proteção Individual, Fatores de Risco, NR 10 — Segurança em Instalações e Serviços em Eletricidade (+3 more)

### Community 194 - "Encontro 1-2 — Tipos de Texto e Contexto de Leitura (4h)"
Cohesion: 0.18
Nodes (11): Atividade integradora, Atividades, Autoavaliação do Módulo 1, Encontro 1-2 — Tipos de Texto e Contexto de Leitura (4h), Encontro 7 — Fechamento do Módulo 1 (3h), Explicação: O que são gêneros textuais?, MÓDULO 1 — LEITURA E COMPREENSÃO TEXTUAL, Objetivos (+3 more)

### Community 195 - "MÓDULO 7 — TRATAMENTO DA INFORMAÇÃO"
Cohesion: 0.18
Nodes (11): 29.1 Leitura de Tabelas, 30.3 Média Aritmética, 31.1 Coleta e Organização de Dados, 31.2 Atividade Final Integradora — Em Grupo, Atividade 7.1, Atividade 7.2, Encontro 29 — Tabelas de Dados (2h), Encontro 30 — Média Aritmética (2h) (+3 more)

### Community 196 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.08
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 197 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 198 - "Status Geral"
Cohesion: 0.29
Nodes (6): Insert das Matérias Mais Tech, Passo 1: Mapear matérias, Passo 2: Gerar inserts, Passo 3: Vincular ao curso, Passo 4: Commit e publicação, Status Geral

### Community 199 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 200 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 201 - "Design: Botão "Analise Ementas" no Modal de Aulas"
Cohesion: 0.20
Nodes (9): 1. Botão "Analise Ementas", 2. Modal `ementaAnaliseModal`, 3. Lógica de Matching, 4. Keywords por Tópico, Arquivos Modificados, Contexto, Design: Botão "Analise Ementas" no Modal de Aulas, Não está no escopo (+1 more)

### Community 202 - "3.2 Análise de Dependências Parciais"
Cohesion: 0.20
Nodes (10): 3.2 Análise de Dependências Parciais, AVALIACAO - Verificação 2FN, CARDAPIO - Verificação 2FN, ENDERECO - Verificação 2FN, ENTREGADOR - Verificação 2FN, ITEM_PEDIDO - Verificação 2FN (Potencial Problema), PAGAMENTO - Verificação 2FN, PEDIDO - Verificação 2FN (+2 more)

### Community 203 - "DOCUMENTO TÉCNICO COMPLETO"
Cohesion: 0.20
Nodes (9): 10. APLICAÇÃO DE NORMALIZAÇÃO - RESUMO, 6.1 Consultas Implementadas, 6.2 Exemplo: Relatório de Faturamento Diário, 6. CONSULTAS SQL - RELATÓRIOS E OPERAÇÕES, ANEXOS, APROVAÇÃO, DOCUMENTO TÉCNICO COMPLETO, Sistema de Delivery de Comida - Projeto Integrador (+1 more)

### Community 204 - "Conteúdo"
Cohesion: 0.20
Nodes (10): Atividade Prática, Conteúdo, Definição de MPE, Desafios das MPEs, ENCONTRO 10 — Micro e Pequenas Empresas (MPEs), MPEs: o coração da economia brasileira, O contexto local: Rio do Sul e região, Objetivos (+2 more)

### Community 205 - "ENCONTRO 15 — Síntese, Portfólio e Preparação para Continuidade"
Cohesion: 0.20
Nodes (10): 15.1 Seu portfólio de comunicação, 15.2 Reflexão final e plano de ação, Ao final deste encontro, você será capaz de:, Atividade 15.1 — Montagem do portfólio (60 min — individual), Atividade 15.2 — Plano pessoal de desenvolvimento (60 min — individual), ENCONTRO 15 — Síntese, Portfólio e Preparação para Continuidade, O que guardar:, Perguntas para refletir: (+2 more)

### Community 206 - "ENCONTRO 5 — Redação técnica: princípios e estrutura"
Cohesion: 0.20
Nodes (10): 5.1 Escrever no trabalho é diferente, 5.2 A estrutura de um texto profissional, 5.3 Revisar antes de enviar, Ao final deste encontro, você será capaz de:, Atividade 5.1 — Oficina de reescrita (45 min — duplas), Checklist de Revisão, ENCONTRO 5 — Redação técnica: princípios e estrutura, Os quatro princípios da redação técnica (+2 more)

### Community 207 - "📚 Conteúdo Programático"
Cohesion: 0.20
Nodes (10): **1. O que é Google Workspace?**, **2. O que é Microsoft 365?**, **3. E-mail Corporativo: Regras Profissionais**, **4. Colaboração em Tempo Real**, **5. Organização de Arquivos**, ✍️ ATIVIDADE 1 — Mapa Mental de Ferramentas, ✍️ ATIVIDADE 2 — Simulação de E-mail Profissional, 📚 Conteúdo Programático (+2 more)

### Community 208 - "Conteúdo"
Cohesion: 0.20
Nodes (10): 1. Robôs Industriais, 2. Robôs de Serviço, 3. Robôs Móveis, Atividade Prática, Conteúdo, ENCONTRO 11 — Classificação dos Robôs: Industriais, de Serviço e Móveis, Objetivos, Por que classificar robôs? (+2 more)

### Community 209 - "Conteúdo"
Cohesion: 0.20
Nodes (10): A anatomia do Arduino Uno, A estrutura de um programa Arduino, Arduino — o microcontrolador mais popular do mundo, Atividade Prática, Conteúdo, ENCONTRO 14 — Controladores e Microcontroladores: O Cérebro do Robô, O que é um controlador?, Objetivos (+2 more)

### Community 210 - "Conteúdo"
Cohesion: 0.20
Nodes (10): Atividade Prática, Conteúdo, Controlando direção, Controlando motores com Arduino, Controlando velocidade (PWM), ENCONTRO 15 — Programação Básica: Trajetórias e Sequências de Movimento, Objetivos, Programando trajetórias (+2 more)

### Community 211 - "Encontro 10-11 — Coesão Textual: Conectivos e Progressão (4h)"
Cohesion: 0.20
Nodes (10): Atividades, Atividades, Encontro 10-11 — Coesão Textual: Conectivos e Progressão (4h), Encontro 12 — Coerência e Revisão de Textos (4h), Explicação: Coerência — o texto faz sentido?, Explicação: O que é coesão?, MÓDULO 2 — PRODUÇÃO TEXTUAL, Objetivos (+2 more)

### Community 212 - "EMENTA — REFORÇO MATEMÁTICA E RACIOCÍNIO LÓGICO"
Cohesion: 0.20
Nodes (9): Avaliação, Capacidades Básicas / Técnicas, Conhecimentos, Conteúdos Formativos, EMENTA — REFORÇO MATEMÁTICA E RACIOCÍNIO LÓGICO, Identificação da Unidade Curricular, Metodologia, Objetivo (+1 more)

### Community 213 - "AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.20
Nodes (10): AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação, EIXO 1 — Hardware, Software e Sistema Operacional (questões 01 a 08), EIXO 2 — Comunicação Profissional (questões 09 a 14), EIXO 3 — Textos Técnicos (questões 15 a 19), EIXO 4 — Internet, Web e Nuvem (questões 20 a 26), EIXO 5 — Segurança da Informação (questões 27 a 33), EIXO 6 — Editor de Textos (questões 34 a 36), EIXO 7 — Planilhas e Apresentações (questões 37 a 40) (+2 more)

### Community 214 - "CRITÉRIOS DE CORREÇÃO — Avaliação Prática"
Cohesion: 0.20
Nodes (10): Conteúdo (1,0), CRITÉRIOS DE CORREÇÃO — Avaliação Prática, Formatação (1,0), QUADRO-RESUMO DA NOTA, Registro Qualitativo (acompanhamento formativo das Aulas 01 a 09), Relação com as Capacidades da Ementa, TAREFA 1 — Organização, Backup e Comunicação (1,5 ponto), TAREFA 2 — Relatório Técnico (2,0 pontos) (+2 more)

### Community 215 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 216 - "1. Tarefa Principal — Testar Componente com Fetch"
Cohesion: 0.20
Nodes (9): 1.1 Arquivo: `src/UserList.js`, 1.2 HTML Fixture: `tests/fixtures/userlist.html`, 1.3 Seu Trabalho — Criar Testes, 1.4 Requisitos, 1. Tarefa Principal — Testar Componente com Fetch, 2. Tarefa Complementar — Tratamento de Erros, 3. Executar Testes, 4. Critério de Avaliação (+1 more)

### Community 217 - "1. Tarefa Principal — Suite E2E com Page Object Model"
Cohesion: 0.20
Nodes (9): 1.1 Setup Inicial, 1.2 Página de Teste: `examples/dashboard.html`, 1.3 Seu Trabalho — Implementar POM + E2E Tests, 1.4 Requisitos de Testes, 1. Tarefa Principal — Suite E2E com Page Object Model, 2. Tarefa Complementar — Verificações Visuais, 3. Executar Testes, 4. Critério de Avaliação (+1 more)

### Community 218 - "1. Tarefa Principal — Suite E2E Expandida"
Cohesion: 0.20
Nodes (9): 1.1 Arquivo: `tests/e2e/features.spec.js`, 1.2 Testes de Performance, 1.3 Testes de Acessibilidade, 1.4 Requisitos, 1. Tarefa Principal — Suite E2E Expandida, 2. Tarefa Complementar — Screenshots e Vídeos, 3. Executar Testes, 4. Critério de Avaliação (+1 more)

### Community 219 - "Documentação Padrão"
Cohesion: 0.22
Nodes (8): Documentação Padrão, Fluxo Obrigatório, Modelo do Arquivo, Overview, Red Flags — PARE e Corrija, Regras, Regras dos Passos, Ícones de Status

### Community 220 - "Documentação Padrão"
Cohesion: 0.22
Nodes (8): Documentação Padrão, Fluxo Obrigatório, Modelo do Arquivo, Overview, Red Flags — PARE e Corrija, Regras, Regras dos Passos, Ícones de Status

### Community 221 - "ATIVIDADE 05: Aplicação de Normalização até 3ª Forma Normal"
Cohesion: 0.22
Nodes (8): 1.1 Definição, 1.2 Progressão das Formas Normais, 1. CONCEITO DE FORMAS NORMAIS, 3.1 Definição de 2FN, 3.3 Conclusão sobre 2FN, 3. VERIFICAÇÃO DE 2ª FORMA NORMAL (2FN), ATIVIDADE 05: Aplicação de Normalização até 3ª Forma Normal, CONCLUSÃO

### Community 222 - "Apostila_Competencias_Socioemocionais_Empreendedorismo.md"
Cohesion: 0.22
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 9 — Tipos de Negócios e Empreendimentos, Objetivos, Os tipos de negócios na economia, PREFEITURA MUNICIPAL DE RIO DO SUL, RIO DO SUL MAIS TECH — SENAI, Síntese

### Community 223 - "Status Geral"
Cohesion: 0.29
Nodes (6): Minhas Pendências no Dashboard, Passo 1: Corrigir o arquivo JSON, Passo 2: Criar card e modal, Passo 3: Carregar e renderizar os dados, Passo 4: Commit e publicação, Status Geral

### Community 224 - "Status Geral"
Cohesion: 0.29
Nodes (6): Modal de Matérias por Curso, Passo 1: Redirecionar o botão Acessar, Passo 2: Criar cards de matérias, Passo 3: Integrar com Supabase, Passo 4: Commit e publicação, Status Geral

### Community 225 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, Descrição da Empresa, Diferença entre Canvas e Plano de Negócios, ENCONTRO 13 — Plano de Negócios — Parte 1, Estrutura do Plano de Negócios, O que é um plano de negócios?, Objetivos (+1 more)

### Community 226 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Calculando o ponto de equilíbrio, Conteúdo, DRE simplificado (Demonstração do Resultado do Exercício), ENCONTRO 14 — Plano de Negócios — Parte 2, O plano financeiro: por que é tão importante?, Objetivos, Síntese (+1 more)

### Community 227 - "Status Geral"
Cohesion: 0.29
Nodes (6): Passo 1: Adicionar coluna, Passo 2: Adicionar ao formulário, Passo 3: Exibir e persistir, Passo 4: Commit e publicação, Status de Aplicação da Avaliação, Status Geral

### Community 228 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Como ser um intraempreendedor?, Conteúdo, ENCONTRO 15 — Intraempreendedorismo e Inovação, Inovação: tipos e exemplos, O que é intraempreendedorismo?, Objetivos, Por que as empresas querem intraempreendedores? (+1 more)

### Community 229 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, ENCONTRO 16 — Negociação e Pontos Críticos, O que é negociação?, Objetivos, Os elementos de uma negociação, Pontos críticos numa negociação, Síntese (+1 more)

### Community 230 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, Dilemas éticos, ENCONTRO 8 — Ética nos Negócios e no Mercado de Trabalho, O que é ética?, Objetivos, Síntese, Ética no trabalho (+1 more)

### Community 231 - "EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS/gerar_slides.js"
Cohesion: 0.28
Nodes (6): addFooter(), contentSlide(), pptx, PptxGenJS, sfinal, twoColSlide()

### Community 232 - "LineForWriting"
Cohesion: 0.28
Nodes (6): LineForWriting, main(), markdown_to_pdf(), Flowable, Cria linhas para os alunos escreverem respostas, Converte Markdown para PDF

### Community 233 - "🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais"
Cohesion: 0.22
Nodes (8): 1. MICROSOFT-ONEDRIVE-VERSAO-CLAUDE.png, 2. MICROSOFT-OUTLOOK-VERSAO-CLAUDE.png, 3. MICROSOFT-TEAMS-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais, 📝 Notas Importantes, 📊 Status, 📋 Tarefas Nesta Pasta

### Community 234 - "🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais"
Cohesion: 0.22
Nodes (8): 1. MICROSOFT-ONEDRIVE-VERSAO-CLAUDE.png, 2. MICROSOFT-OUTLOOK-VERSAO-CLAUDE.png, 3. MICROSOFT-TEAMS-VERSAO-CLAUDE.png, ✅ Estrutura Padrão por Infográfico, 🟠 MICROSOFT ECOSYSTEM — Infográficos de Ferramentas Adicionais, 📝 Notas Importantes, 📊 Status, 📋 Tarefas Nesta Pasta

### Community 235 - "ENCONTRO 10 — Aprofundamento: Redação Profissional"
Cohesion: 0.22
Nodes (9): 10.1 Coesão: como as ideias se conectam, 10.2 Coerência: o texto faz sentido?, Ao final deste encontro, você será capaz de:, Atividade 10.1 — Cirurgia de coesão (30 min — duplas), Atividade 10.2 — Teste de coerência (30 min — individual), Checklist de coerência:, Conectivos mais usados no ambiente profissional:, ENCONTRO 10 — Aprofundamento: Redação Profissional (+1 more)

### Community 236 - "ENCONTRO 1 — Comunicação no mundo do trabalho"
Cohesion: 0.22
Nodes (9): 1.1 Comunicar é fazer-se entender, 1.2 Ruído: o inimigo silencioso, Ao final deste encontro, você será capaz de:, Atividade 1.1 — Mapa da comunicação (25 min — individual), Atividade 1.2 — Caça aos ruídos (40 min — em duplas), ENCONTRO 1 — Comunicação no mundo do trabalho, Fique de Olho, Os elementos da comunicação (+1 more)

### Community 237 - "ENCONTRO 2 — Registro adequado e escuta ativa"
Cohesion: 0.22
Nodes (9): 2.1 Cada contexto pede um tom, 2.2 Escuta ativa: metade da comunicação, Ao final deste encontro, você será capaz de:, Atividade 2.1 — Tradutor de registros (35 min — individual), Atividade 2.2 — Teste da escuta (40 min — trios), Como praticar escuta ativa:, Cuidado com os Extremos, ENCONTRO 2 — Registro adequado e escuta ativa (+1 more)

### Community 238 - "ENCONTRO 6 — Gêneros profissionais: comunicado, relato e ata"
Cohesion: 0.22
Nodes (9): 6.1 Comunicado, 6.2 Relato de ocorrência, 6.3 Ata resumida, Ao final deste encontro, você será capaz de:, Atividade 6.1 — Produção de texto profissional (60 min — individual), ENCONTRO 6 — Gêneros profissionais: comunicado, relato e ata, Por que isso importa, Resumo dos três gêneros (+1 more)

### Community 239 - "📚 Conteúdo Programático"
Cohesion: 0.22
Nodes (9): **1. O que é Netiqueta?**, **2. Identificando Golpes: Phishing**, **3. Protegendo suas Senhas**, **4. Privacidade no Ambiente Corporativo**, ✍️ ATIVIDADE 3 — Análise de E-mails Suspeitos, ✍️ ATIVIDADE 4 — Criador de Senhas Forte, 📚 Conteúdo Programático, 📌 ENCONTRO 15 — Netiqueta e Segurança Digital (+1 more)

### Community 240 - "Apostila_Nocoes_Eletricidade_Circuitos_Basicos.md"
Cohesion: 0.22
Nodes (8): Aplicativos Úteis, GLOSSÁRIO, Livros, Normas Técnicas, Recursos Online Gratuitos, REFERÊNCIAS SUGERIDAS, RIO DO SUL MAIS TECH, SENAI — PREFEITURA MUNICIPAL DE RIO DO SUL

### Community 241 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, Corrente contínua (CC) e corrente alternada (CA), ENCONTRO 1 — O QUE É ELETRICIDADE?, Materiais condutores e isolantes, O que é corrente elétrica?, Objetivos, Síntese (+1 more)

### Community 242 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, Emenda de condutores, ENCONTRO 8 — CONDUTORES, ISOLANTES E FIOS ELÉTRICOS, Identificação pela cor dos fios (ABNT NBR 5410), Objetivos, Seção dos condutores (bitola), Síntese (+1 more)

### Community 243 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, ENCONTRO 12 — AUTOMAÇÃO RESIDENCIAL BÁSICA, Fotocélula (Sensor Fotoelétrico), Objetivos, Programador Horário (Timer), Sensor de Presença (PIR — Passive Infrared Sensor), Síntese (+1 more)

### Community 244 - "NOCOES_ELETRICIDADE_CIRCUITOS_BASICOS/gerar_slides.js"
Cohesion: 0.31
Nodes (8): addRodape(), outPath, path, pptx, PptxGenJS, slideApresentacao(), slideConteudo(), slideDois()

### Community 245 - "Conteúdo"
Cohesion: 0.22
Nodes (9): A interface do Tinkercad, Adicionando e manipulando formas, Atividade Prática, Conteúdo, ENCONTRO 4 — Primeiros Passos no Tinkercad: Interface e Ferramentas Básicas, Navegando no espaço 3D, O que é o Tinkercad?, Objetivos (+1 more)

### Community 246 - "Conteúdo"
Cohesion: 0.22
Nodes (9): A mágica dos furos, Atividade Prática, Combinando formas, Conteúdo, ENCONTRO 5 — Criando Sólidos no Tinkercad: Formas, Furos e Combinações, Objetivos, Operações básicas, Síntese (+1 more)

### Community 247 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, ENCONTRO 1 — Do Passado ao Futuro: História e Impacto da Impressão 3D, O que é impressão 3D?, Objetivos, Para que serve a impressão 3D hoje?, Por que aprender impressão 3D?, Síntese (+1 more)

### Community 248 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Conteúdo, ENCONTRO 13 — Atuadores e Motores: Como os Robôs Agem, O motor de passo na impressora 3D, O que são atuadores?, Objetivos, Outros tipos de atuadores, Síntese (+1 more)

### Community 249 - "Conteúdo"
Cohesion: 0.22
Nodes (9): Atividade Prática, Configurações principais do slicer, Conteúdo, ENCONTRO 7 — Do Modelo ao Arquivo: Exportando STL e Configurando o Fatiamento, Entendendo o infill, O arquivo STL, O software de fatiamento (Slicer), Objetivos (+1 more)

### Community 250 - "Apostila_Reforco_Linguagens.md"
Cohesion: 0.22
Nodes (8): GABARITO — EXERCÍCIOS SELECIONADOS, GLOSSÁRIO, Módulo 1 — Encontro 3-4, Módulo 2 — Encontro 10-11, Módulo 3 — Encontro 15-16, Módulo 5 — Encontro 25-26, REFERÊNCIAS, RIO DO SUL MAIS TECH — SENAI / PREFEITURA MUNICIPAL DE RIO DO SUL

### Community 251 - "REFORCO_LINGUAGENS/gerar_slides.js"
Cohesion: 0.22
Nodes (3): COR, pptxgen, pres

### Community 252 - "Status Geral"
Cohesion: 0.33
Nodes (5): Borda vermelha nos cards pendentes, Passo 1: Aplicar a borda, Passo 2: Revisar o diff, Passo 3: Commit e push, Status Geral

### Community 253 - "Status Geral"
Cohesion: 0.33
Nodes (5): Combobox de Tipo de Curso, Passo 1: Substituir campo, Passo 2: Preencher opções, Passo 3: Commit e publicação, Status Geral

### Community 254 - "Status Geral"
Cohesion: 0.33
Nodes (5): Corrigir Hover de Editar Matéria, Passo 1: Identificar conflito, Passo 2: Corrigir hover, Passo 3: Commit e publicação, Status Geral

### Community 256 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.08
Nodes (23): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+15 more)

### Community 257 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.08
Nodes (23): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+15 more)

### Community 258 - "ATIVIDADE AULA 09 — CI/CD e Boas Práticas"
Cohesion: 0.22
Nodes (8): 1.1 Arquivo: `.github/workflows/test.yml`, 1.2 Seu Trabalho — Completar o Pipeline, 1.3 Arquivo: `docs/TESTING-STRATEGY.md`, 1. Tarefa Principal — Configurar Pipeline CI/CD, 2. Tarefa Complementar — Badge de Status, 3. Verificação, 4. Critério de Avaliação, ATIVIDADE AULA 09 — CI/CD e Boas Práticas

### Community 259 - "Global Constraints"
Cohesion: 0.25
Nodes (7): Analise Ementas — Implementation Plan, Global Constraints, Self-Review, Task 1: CSS — Estilos do botão e do modal de análise, Task 2: HTML — Botão na toolbar e modal de análise, Task 3: JavaScript — Dicionário, matching e funções de abrir/fechar, Task 4: Commit e push

### Community 260 - "LISTAS — Contatos para importação no Resend"
Cohesion: 0.18
Nodes (8): 1. Formato esperado, 2. Nomenclatura dos arquivos, 3. Como entregar as listas, 4. O que será feito depois, 5. ⚠️ Privacidade e segurança, Colunas customizadas (viram *Contact Properties*), Colunas padrão do Resend (obrigatórias), LISTAS — Contatos para importação no Resend

### Community 261 - "E-MAIL SENAI"
Cohesion: 0.25
Nodes (8): 1. Como rodar, 2. Primeiro acesso, 3. Estrutura, 4. Banco de dados, 5. ⬜ Passo pendente: cadastrar a chave do Resend, 6. Funcionalidades, 7. ⚠️ Atenção nos dados atuais, E-MAIL SENAI

### Community 262 - "4.2 Identificação de Dependências Transitivas"
Cohesion: 0.25
Nodes (8): 4.1 Definição de 3FN, 4.2 Identificação de Dependências Transitivas, 4.3 Conclusão sobre 3FN, 4. VERIFICAÇÃO DE 3ª FORMA NORMAL (3FN), ENTREGADOR - Análise Similar, Outras Entidades - Verificação 3FN, RESTAURANTE - Análise Crítica, SQL: Criar Trigger para Manter avaliacao_media Sincronizada

### Community 263 - "Status Geral"
Cohesion: 0.33
Nodes (5): Efeito de profundidade nos cards pendentes, Passo 1: Ajustar a animação, Passo 2: Revisar o diff, Passo 3: Commit e push, Status Geral

### Community 264 - "ENCONTRO 11 — Identificando Oportunidades de Negócio"
Cohesion: 0.25
Nodes (8): A técnica dos 5 Porquês, Análise SWOT (FOFA em português), Atividade Prática, Conteúdo, ENCONTRO 11 — Identificando Oportunidades de Negócio, O que é uma oportunidade de negócio?, Objetivos, Síntese

### Community 265 - "Status Geral"
Cohesion: 0.33
Nodes (5): Passo 1: Atualizar documentação histórica, Passo 2: Excluir JSON local, Passo 3: Commit e publicação, Remover Pendências JSON, Status Geral

### Community 266 - "ENCONTRO 1 — O que é ser empreendedor?"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 1 — O que é ser empreendedor?, O que é empreendedorismo?, Objetivos, Por que aprender sobre empreendedorismo agora?, Síntese, Tipos de empreendedorismo

### Community 267 - "ENCONTRO 4 — Comunicação, Liderança e Trabalho em Equipe"
Cohesion: 0.25
Nodes (8): Atividade Prática, Comunicação: muito mais do que falar, Conteúdo, ENCONTRO 4 — Comunicação, Liderança e Trabalho em Equipe, Liderança: não é só dar ordens, Objetivos, Síntese, Trabalho em equipe

### Community 268 - "ENCONTRO 5 — Sustentabilidade e os 5R's"
Cohesion: 0.25
Nodes (8): Atividade Prática, Coleta seletiva, Conteúdo, ENCONTRO 5 — Sustentabilidade e os 5R's, Objetivos, Os 5R's, Por que falar de sustentabilidade numa UC de empreendedorismo?, Síntese

### Community 269 - "ENCONTRO 12 — Planejamento e Estratégia Empreendedora"
Cohesion: 0.25
Nodes (8): Atividade Prática, Canvas do Modelo de Negócios (Business Model Canvas), Conteúdo, ENCONTRO 12 — Planejamento e Estratégia Empreendedora, Metas SMART, Objetivos, Por que planejar?, Síntese

### Community 270 - "compactar_atividade_ferramentas_digitais_facil.py"
Cohesion: 0.67
Nodes (5): compact_document(), compact_paragraph(), compact_table(), remove_manual_page_breaks(), remove_paragraph()

### Community 271 - "ENCONTRO 8 — Carreiras em TI II: Dados, IA e Segurança da Informação"
Cohesion: 0.25
Nodes (8): Analista de Inteligência Artificial, Analista de Segurança da Informação (Cibersegurança), Atividade 8 — Caça ao Dado, Cientista de Dados, ENCONTRO 8 — Carreiras em TI II: Dados, IA e Segurança da Informação, Objetivos, Síntese, Texto: Quem controla os dados, controla o futuro

### Community 272 - "Status Geral"
Cohesion: 0.29
Nodes (6): Leitura do Plano de 3 Aulas — Fundamentos, Passo 1: Inventariar os arquivos, Passo 2: Ler todo o conteúdo, Passo 3: Consolidar o contexto, Passo 4: Commit e publicação, Status Geral

### Community 273 - "ENCONTRO 15 — Entrevista Simulada: Como Se Apresentar"
Cohesion: 0.25
Nodes (8): Antes da entrevista, Após a entrevista, Atividade 15 — Entrevista Simulada, Durante a entrevista, ENCONTRO 15 — Entrevista Simulada: Como Se Apresentar, Objetivos, Síntese, Texto: A entrevista começa antes de entrar na sala

### Community 274 - "ENCONTRO 13 — LinkedIn e Redes Profissionais"
Cohesion: 0.25
Nodes (8): Atividade 13 — Perfil Profissional Simulado, Como montar um bom perfil no LinkedIn, Cuidados com a presença digital, ENCONTRO 13 — LinkedIn e Redes Profissionais, Objetivos, Por que criar um perfil no LinkedIn?, Síntese, Texto: A rede social que abre portas

### Community 275 - "ENCONTRO 16 — SENAI: Caminhos de Formação Técnica"
Cohesion: 0.25
Nodes (8): Atividade 16 — Pesquisa de Cursos, Cursos técnicos disponíveis no SENAI (exemplos), ENCONTRO 16 — SENAI: Caminhos de Formação Técnica, O Jovem Aprendiz no SENAI, Objetivos, Por que escolher o SENAI?, Síntese, Texto: O SENAI como seu próximo passo

### Community 276 - "ENCONTRO 7 — Carreiras em TI I: Desenvolvimento de Software"
Cohesion: 0.25
Nodes (8): Atividade 7 — Meu Primeiro "Olá, Mundo!", ENCONTRO 7 — Carreiras em TI I: Desenvolvimento de Software, Linguagens mais usadas, Objetivos, Principais cargos em desenvolvimento, Quanto ganha um desenvolvedor?, Síntese, Texto: O desenvolvedor que constrói o mundo digital

### Community 277 - "Apostila Ferramentas Digitais para Comunicação"
Cohesion: 0.40
Nodes (4): Apostila Ferramentas Digitais para Comunicação, Estrutura planejada, Restrição de verificação, Status Geral

### Community 278 - "Atividade fácil de Ferramentas Digitais"
Cohesion: 0.40
Nodes (4): Adaptações inclusivas, Atividade fácil de Ferramentas Digitais, Restrição de verificação, Status Geral

### Community 279 - "Atividades de Ferramentas Digitais"
Cohesion: 0.40
Nodes (4): Atividades, Atividades de Ferramentas Digitais, Restrição de verificação, Status Geral

### Community 280 - "Campo Ensalado nas matérias"
Cohesion: 0.40
Nodes (4): Campo Ensalado nas matérias, Regras, Status Geral, Verificação

### Community 281 - "Compactar atividade fácil de Ferramentas Digitais"
Cohesion: 0.40
Nodes (4): Ajustes, Compactar atividade fácil de Ferramentas Digitais, Restrição de verificação, Status Geral

### Community 282 - "🔴 COMPARATIVOS — Infográficos de Comparação"
Cohesion: 0.25
Nodes (7): 1. GOOGLE-DOCS-X-MICROSOFT-WORD-VERSAO-CLAUDE.png, 2. GOOGLE-SLIDES-X-MICROSOFT-POWERPOINT-VERSAO-CLAUDE.png, 3. GOOGLE-SHEETS-X-MICROSOFT-EXCEL-VERSAO-CLAUDE.png, ✅ Checklist de Qualidade, 🔴 COMPARATIVOS — Infográficos de Comparação, 📊 Status, 📋 Tarefas Nesta Pasta

### Community 283 - "🔴 COMPARATIVOS — Infográficos de Comparação"
Cohesion: 0.25
Nodes (7): 1. GOOGLE-DOCS-X-MICROSOFT-WORD-VERSAO-CLAUDE.png, 2. GOOGLE-SLIDES-X-MICROSOFT-POWERPOINT-VERSAO-CLAUDE.png, 3. GOOGLE-SHEETS-X-MICROSOFT-EXCEL-VERSAO-CLAUDE.png, ✅ Checklist de Qualidade, 🔴 COMPARATIVOS — Infográficos de Comparação, 📊 Status, 📋 Tarefas Nesta Pasta

### Community 284 - "ENCONTRO 14 — Aprofundamento: Segurança Digital e Conformidade"
Cohesion: 0.25
Nodes (8): 14.1 Golpes digitais e como evitar, 14.2 LGPD: Lei Geral de Proteção de Dados, Ao final deste encontro, você será capaz de:, Atividade 14.1 — Simulação de golpe (30 min — grupos), Atividade 14.2 — Cenários LGPD (30 min — duplas), ENCONTRO 14 — Aprofundamento: Segurança Digital e Conformidade, Golpes mais comuns:, Síntese do Encontro 14

### Community 285 - "Excluir matérias do curso"
Cohesion: 0.40
Nodes (4): Excluir matérias do curso, Regra de exclusão, Status Geral, Verificação

### Community 286 - "ENCONTRO 8 — Desafio integrador e avaliação"
Cohesion: 0.25
Nodes (8): 8.1 O desafio, 8.2 Critérios de avaliação, 8.3 Autoavaliação, Ao final deste encontro, você será capaz de:, ENCONTRO 8 — Desafio integrador e avaliação, Entregas do desafio, Situação-Problema, Síntese do Encontro 8

### Community 287 - "ENCONTRO 9 — Aprofundamento: Atendimento ao Cliente"
Cohesion: 0.25
Nodes (8): 9.1 Além das quatro etapas: a empatia, 9.2 Atendimento em situações difíceis, Ao final deste encontro, você será capaz de:, Atividade 9.1 — Atendimento desafiador (60 min — grupos), Como demonstrar empatia:, ENCONTRO 9 — Aprofundamento: Atendimento ao Cliente, Estratégias para situações difíceis:, Síntese do Encontro 9

### Community 288 - "Manter modal do curso ao ver aulas"
Cohesion: 0.40
Nodes (4): Causa, Manter modal do curso ao ver aulas, Status Geral, Verificação

### Community 289 - "📌 AVALIAÇÃO FINAL INTEGRADA"
Cohesion: 0.25
Nodes (8): ✍️ ATIVIDADE 7 — Plano da Avaliação Final, ✍️ ATIVIDADE 8 — Autorreflexão Final, 📌 AVALIAÇÃO FINAL INTEGRADA, 📋 Instruções da Avaliação Final, 🎓 Objetivo, **OPÇÃO A: Documento de Comunicação Profissional (Recomendado)**, **OPÇÃO B: Projeto Integrador (Avançado)**, 📊 Rubrica de Avaliação

### Community 290 - "🧠 Banco de Questões de Estudo"
Cohesion: 0.25
Nodes (8): 🧠 Banco de Questões de Estudo, **Tema 1: Elementos da Comunicação**, **Tema 2: Comunicação Profissional**, **Tema 3: E-mail Corporativo**, **Tema 4: Redação Profissional**, **Tema 5: Ferramentas Digitais (Google Workspace)**, **Tema 6: Netiqueta**, **Tema 7: Segurança Digital**

### Community 291 - "ENCONTRO 4 — LEI DE OHM: A EQUAÇÃO QUE EXPLICA TUDO"
Cohesion: 0.25
Nodes (8): A Lei de Ohm, Atividade Prática, Conteúdo, ENCONTRO 4 — LEI DE OHM: A EQUAÇÃO QUE EXPLICA TUDO, Exemplos resolvidos, Objetivos, Resistência do corpo humano, Síntese

### Community 292 - "ENCONTRO 14 — MOTORES, VENTILADORES E MOTO-BOMBAS"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 14 — MOTORES, VENTILADORES E MOTO-BOMBAS, Moto-Bomba com Boia Elétrica, Motor Elétrico: princípio básico, Objetivos, Síntese, Ventiladores e Exaustores

### Community 293 - "ENCONTRO 2 — GRANDEZAS ELÉTRICAS: TENSÃO E CORRENTE"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 2 — GRANDEZAS ELÉTRICAS: TENSÃO E CORRENTE, O que é Corrente Elétrica?, O que é Tensão Elétrica?, Objetivos, Símbolos e prefixos, Síntese

### Community 294 - "ENCONTRO 3 — GRANDEZAS ELÉTRICAS: RESISTÊNCIA E POTÊNCIA"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 3 — GRANDEZAS ELÉTRICAS: RESISTÊNCIA E POTÊNCIA, O que é Potência Elétrica?, O que é Resistência Elétrica?, Objetivos, Potências típicas de aparelhos domésticos, Síntese

### Community 295 - "ENCONTRO 5 — INSTRUMENTOS DE MEDIÇÃO"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, Cuidados gerais com multímetro, ENCONTRO 5 — INSTRUMENTOS DE MEDIÇÃO, O Multímetro — o instrumento mais importante, Objetivos, Principais instrumentos de medição, Síntese

### Community 296 - "ENCONTRO 7 — DIAGRAMAS ELÉTRICOS"
Cohesion: 0.25
Nodes (8): Atividade Prática, Como ler um diagrama, Conteúdo, ENCONTRO 7 — DIAGRAMAS ELÉTRICOS, Objetivos, Por que usamos diagramas?, Símbolos elétricos básicos (ABNT), Síntese

### Community 297 - "ENCONTRO 10 — TOMADAS, PLUGUES E INSTALAÇÕES PREDIAIS"
Cohesion: 0.25
Nodes (8): Atividade Prática, Circuitos de instalação predial, Conteúdo, ENCONTRO 10 — TOMADAS, PLUGUES E INSTALAÇÕES PREDIAIS, Objetivos, Padrão brasileiro de tomadas — NBR 14136, Quadro de distribuição (QD), Síntese

### Community 298 - "Status Geral"
Cohesion: 0.29
Nodes (6): Excluir Node Modules de Pendências do Graphify, Passo 1: Criar regra de exclusão, Passo 2: Atualizar o grafo, Passo 3: Atualizar o contexto, Passo 4: Commit e push, Status Geral

### Community 299 - "ENCONTRO 13 — PROTEÇÃO ELÉTRICA: DISJUNTORES E DR"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, Disjuntor Termomagnético, Dispositivo Residual Diferencial (DR), ENCONTRO 13 — PROTEÇÃO ELÉTRICA: DISJUNTORES E DR, Objetivos, Por que precisamos de proteção?, Síntese

### Community 300 - "Modal de edição de avaliação"
Cohesion: 0.40
Nodes (4): Fluxo, Modal de edição de avaliação, Status Geral, Verificação

### Community 301 - "Notificação de pendências nos cursos"
Cohesion: 0.40
Nodes (4): Etapas, Notificação de pendências nos cursos, Objetivo, Verificação prevista

### Community 302 - "ENCONTRO 16 — Integração: Imprimindo Peças para o Robô"
Cohesion: 0.25
Nodes (8): A impressão 3D na robótica, Atividade Prática, Conteúdo, ENCONTRO 16 — Integração: Imprimindo Peças para o Robô, Integrando sensores em peças impressas, Objetivos, Princípios de design de peças robóticas, Síntese

### Community 303 - "ENCONTRO 10 — Era dos Robôs: História e Evolução da Robótica"
Cohesion: 0.25
Nodes (8): A linha do tempo da robótica, As 3 Leis da Robótica de Asimov, Atividade Prática, Conteúdo, ENCONTRO 10 — Era dos Robôs: História e Evolução da Robótica, O que é um robô?, Objetivos, Síntese

### Community 304 - "Ordenar cursos por alertas"
Cohesion: 0.40
Nodes (4): Ordenar cursos por alertas, Regra, Status Geral, Verificação

### Community 305 - "Status Geral"
Cohesion: 0.33
Nodes (5): Adicionar Regra de Prova Teórica, Passo 1: Atualizar a estrutura das UCs, Passo 2: Registrar a tarefa, Passo 3: Commit e push, Status Geral

### Community 306 - "professor-senai — Regras do Projeto"
Cohesion: 0.18
Nodes (10): Commit e Push obrigatórios, Estrutura de Unidades Curriculares, Estrutura obrigatória de cada UC, Graphify, Nunca perguntar — sempre executar, Pasta PROFESSOR, professor-senai — Regras do Projeto, Slides (+2 more)

### Community 307 - "Status Geral"
Cohesion: 0.33
Nodes (5): Definir Ficha Produto Mais Tech como Curso, Passo 1: Atualizar a classificação, Passo 2: Registrar a tarefa, Passo 3: Commit e push, Status Geral

### Community 308 - "Status Geral"
Cohesion: 0.33
Nodes (5): Definir Gestão e Controle de Materiais como Curso, Passo 1: Atualizar a classificação, Passo 2: Registrar a tarefa, Passo 3: Commit e push, Status Geral

### Community 309 - "Status Geral"
Cohesion: 0.33
Nodes (5): Definir Técnico em Desenvolvimento de Sistemas como Curso, Passo 1: Atualizar a classificação, Passo 2: Registrar a tarefa, Passo 3: Commit e push, Status Geral

### Community 310 - "Status Geral"
Cohesion: 0.33
Nodes (5): Definir Técnico de Informática para Internet como Curso, Passo 1: Atualizar a classificação, Passo 2: Registrar a tarefa, Passo 3: Commit e push, Status Geral

### Community 311 - "ENCONTRO 2 — Como Funciona uma Impressora 3D: FDM, SLA e SLS"
Cohesion: 0.25
Nodes (8): As três tecnologias principais, Atividade Prática, Comparativo, Conteúdo, ENCONTRO 2 — Como Funciona uma Impressora 3D: FDM, SLA e SLS, O processo geral, Objetivos, Síntese

### Community 312 - "ENCONTRO 17 — Projeto Final: Montagem do Protótipo Robótico"
Cohesion: 0.25
Nodes (8): Atividade Prática, Checklist antes de ligar, Conteúdo, Depuração (debugging) de problemas comuns, ENCONTRO 17 — Projeto Final: Montagem do Protótipo Robótico, O processo de integração, Objetivos, Síntese

### Community 313 - "ENCONTRO 6 — Projeto 1 no Tinkercad: Modelando um Objeto Funcional"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, ENCONTRO 6 — Projeto 1 no Tinkercad: Modelando um Objeto Funcional, Objetivos, Opções de objetos funcionais para o Projeto 1, Planejamento: a etapa mais importante, Processo de modelagem, Síntese

### Community 314 - "ENCONTRO 12 — Sensores: Como os Robôs Enxergam o Mundo"
Cohesion: 0.25
Nodes (8): Atividade Prática, Conteúdo, Da leitura à ação, ENCONTRO 12 — Sensores: Como os Robôs Enxergam o Mundo, O que são sensores?, Objetivos, Síntese, Tipos de sensores

### Community 315 - "EMENTA — REFORÇO DE LINGUAGENS"
Cohesion: 0.25
Nodes (7): Básicas, Capacidades, Conhecimentos, EMENTA — REFORÇO DE LINGUAGENS, Identificação da Unidade Curricular, Objetivo, Organização dos Módulos

### Community 316 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 317 - "Remover a palavra fácil da atividade"
Cohesion: 0.40
Nodes (4): Remover a palavra fácil da atividade, Restrição de verificação, Status Geral, Substituições

### Community 318 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 319 - "Toggle para cursos bloqueados"
Cohesion: 0.40
Nodes (4): Comportamento, Status Geral, Toggle para cursos bloqueados, Verificação

### Community 320 - "Toggle Ensalado no card da matéria"
Cohesion: 0.40
Nodes (4): Comportamento, Status Geral, Toggle Ensalado no card da matéria, Verificação

### Community 321 - "2. Gabarito comentado"
Cohesion: 0.17
Nodes (12): 1. Gabarito rápido, 2. Gabarito comentado, 3. Mapa de acertos por capacidade (uso pedagógico), 4. Registro de aplicação, Eixo 1 — Hardware, Software e Sistema Operacional (Aulas 01 e 02), Eixo 2 — Comunicação Profissional (Aula 03), Eixo 3 — Textos Técnicos (Aula 04), Eixo 4 — Internet, Web e Nuvem (Aula 05) (+4 more)

### Community 322 - "remover_palavra_facil_atividade.py"
Cohesion: 0.80
Nodes (4): all_paragraphs(), edit_document(), replace_paragraph(), replace_text()

### Community 323 - "5. Estrutura de Blocos Temáticos e Plano de Aulas"
Cohesion: 0.25
Nodes (8): 5.8 Bloco 08 — Boas Práticas e Projeto Integrador, 5. Estrutura de Blocos Temáticos e Plano de Aulas, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 324 - "Migrar status das avaliações para a matéria"
Cohesion: 0.50
Nodes (3): Migrar status das avaliações para a matéria, Status Geral, Verificação

### Community 325 - "4. Estrutura de Blocos Temáticos e Plano de Aulas"
Cohesion: 0.25
Nodes (8): 4.4 Bloco 04 — Execução de Testes de Interface, 4. Estrutura de Blocos Temáticos e Plano de Aulas, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 326 - "Analise de Tamanho - Pastas e Arquivos"
Cohesion: 0.29
Nodes (6): 1. Maiores pastas (top 30), 2. Maiores arquivos (top 30), 3. Peso por tipo de arquivo, 4. Arvore de pastas (ate nivel 2), 5. Arquivos duplicados (>= 1.00 MB), Analise de Tamanho - Pastas e Arquivos

### Community 327 - "criar_lista_presenca_ceplas_barragem_2026.mjs"
Cohesion: 0.50
Nodes (3): participantes, sheet, workbook

### Community 329 - "Global Constraints"
Cohesion: 0.29
Nodes (6): Global Constraints, Status de planejamento por matéria — Implementation Plan, Task 1: Migrar o modelo SQL, Task 2: Transferir a interface para a matéria, Task 3: Ajustar o contador dos cursos, Task 4: Revisar e versionar

### Community 330 - "6. PROCEDIMENTOS DE SEGURANÇA E BACKUP"
Cohesion: 0.29
Nodes (7): 6.1 Implementação de Backup, 6.2 Recuperação de Desastres, 6.3 Teste de Restauração, 6. PROCEDIMENTOS DE SEGURANÇA E BACKUP, Backup Completo Semanal, Backup Incremental (Binary Log), Procedimento de Recuperação

### Community 331 - "11. CHECKLIST DE IMPLEMENTAÇÃO"
Cohesion: 0.29
Nodes (7): 11. CHECKLIST DE IMPLEMENTAÇÃO, Consultas e Operações, Documentação, Modelagem de Dados, Normalização, Scripts SQL, Segurança e Backup

### Community 332 - "ENCONTRO 7 — Agenda Ambiental (A3P) e Primavera Silenciosa"
Cohesion: 0.29
Nodes (7): Agenda Ambiental na Administração Pública (A3P), Atividade Prática, Conteúdo, ENCONTRO 7 — Agenda Ambiental (A3P) e Primavera Silenciosa, Objetivos, Primavera Silenciosa, Síntese

### Community 333 - "COMPETÊNCIAS SOCIOEMOCIONAIS E EMPREENDEDORISMO"
Cohesion: 0.29
Nodes (7): APOSTILA DO ALUNO, Como a apostila está organizada, COMPETÊNCIAS SOCIOEMOCIONAIS E EMPREENDEDORISMO, Objetivo da UC, Percurso de Aprendizagem, Sobre este material, Uma mensagem para você

### Community 334 - "ENCONTRO 3 — Inteligência Emocional"
Cohesion: 0.29
Nodes (7): As 5 dimensões da inteligência emocional, Atividade Prática, Conteúdo, ENCONTRO 3 — Inteligência Emocional, O que é inteligência emocional?, Objetivos, Síntese

### Community 335 - "ENCONTRO 17 — Virtudes do Empreendedor de Sucesso"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 17 — Virtudes do Empreendedor de Sucesso, Histórias de jovens empreendedores brasileiros, Objetivos, Síntese, Virtudes que fazem a diferença

### Community 336 - "ENCONTRO 2 — Perfil Empreendedor e Autoconhecimento"
Cohesion: 0.29
Nodes (7): Atividade Prática, Autoconhecimento: o primeiro passo, Conteúdo, ENCONTRO 2 — Perfil Empreendedor e Autoconhecimento, Objetivos, Quem é o empreendedor?, Síntese

### Community 337 - "ENCONTRO 6 — Ciclo da Água e Legislação Ambiental"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 6 — Ciclo da Água e Legislação Ambiental, Legislação ambiental brasileira, O ciclo da água, Objetivos, Síntese

### Community 338 - "EXPLORAÇÃO DE CARREIRAS INDUSTRIAIS E TECNOLÓGICAS"
Cohesion: 0.29
Nodes (7): APOSTILA DO ALUNO, COMO ESTA APOSTILA ESTÁ ORGANIZADA, EXPLORAÇÃO DE CARREIRAS INDUSTRIAIS E TECNOLÓGICAS, OBJETIVO DA UC, QUEM VOCÊ QUER SER?, SOBRE ESTE MATERIAL, TABELA DO PERCURSO DE APRENDIZAGEM

### Community 339 - "ENCONTRO 11 — Planejamento de Carreira: Da Escola ao Mercado"
Cohesion: 0.29
Nodes (7): Atividade 11 — Meu Plano de Carreira Inicial, ENCONTRO 11 — Planejamento de Carreira: Da Escola ao Mercado, Itinerários formativos possíveis, Objetivos, Planejamento em 3 horizontes, Síntese, Texto: Construindo a estrada enquanto caminha

### Community 340 - "ENCONTRO 12 — Currículo para Jovens: Como Montar o Seu"
Cohesion: 0.29
Nodes (7): Atividade 12 — Monte Seu Currículo, ENCONTRO 12 — Currículo para Jovens: Como Montar o Seu, Estrutura básica de um currículo para jovens, O que NÃO colocar no currículo, Objetivos, Síntese, Texto: O currículo é sua primeira apresentação

### Community 341 - "ENCONTRO 14 — Networking: Construindo Conexões"
Cohesion: 0.29
Nodes (7): Atividade 14 — Roda de Networking, Como construir networking sendo jovem?, ENCONTRO 14 — Networking: Construindo Conexões, O elevator pitch, Objetivos, Síntese, Texto: Sua rede de contatos vale ouro

### Community 342 - "ENCONTRO 17 — Projeto Final: Meu Mapa de Carreira"
Cohesion: 0.29
Nodes (7): Atividade 17 — Construção do Mapa de Carreira, ENCONTRO 17 — Projeto Final: Meu Mapa de Carreira, Formato, O que é o Mapa de Carreira?, Objetivos, Síntese, Texto: Tudo se conecta

### Community 343 - "ENCONTRO 5 — Carreiras Industriais I: Mecânica e Eletrotécnica"
Cohesion: 0.29
Nodes (7): Atividade 5 — Pesquisa de Profissional, ENCONTRO 5 — Carreiras Industriais I: Mecânica e Eletrotécnica, Objetivos, Síntese, Texto: O coração da indústria bate aqui, Técnico em Eletrotécnica, Técnico em Mecânica Industrial

### Community 344 - "ENCONTRO 6 — Carreiras Industriais II: Automação e Mecatrônica"
Cohesion: 0.29
Nodes (7): Atividade 6 — Simulação de CLP, ENCONTRO 6 — Carreiras Industriais II: Automação e Mecatrônica, Objetivos, Síntese, Texto: Onde a máquina encontra o digital, Técnico em Automação Industrial, Técnico em Mecatrônica

### Community 345 - "12.1 Memorando: comunicação interna eficiente"
Cohesion: 0.29
Nodes (7): 12.1 Memorando: comunicação interna eficiente, Ao final deste encontro, você será capaz de:, Atividade 12.1 — Memorando prático (30 min — individual), Diferenças entre gêneros:, ENCONTRO 12 — Aprofundamento: Memorando e Outros Gêneros, Estrutura do memorando:, Síntese do Encontro 12

### Community 346 - "📌 ENCONTRO 16 — Revisão, Apresentações Finais e Reflexão"
Cohesion: 0.29
Nodes (7): ✍️ ATIVIDADE 5 — Roteiro de Apresentação, ✍️ ATIVIDADE 6 — Avaliação de Colegas, 📚 Conteúdo de Revisão, 📌 ENCONTRO 16 — Revisão, Apresentações Finais e Reflexão, 🎓 Objetivos do Encontro, **PARTE 1: Revisão em "Jogo de Perguntas" (30 min)**, **PARTE 2: Apresentações Orais dos Alunos (2h)**

### Community 347 - "NOÇÕES DE ELETRICIDADE E CIRCUITOS BÁSICOS"
Cohesion: 0.29
Nodes (7): APOSTILA DO ALUNO, APRESENTAÇÃO, COMO A APOSTILA ESTÁ ORGANIZADA, NOÇÕES DE ELETRICIDADE E CIRCUITOS BÁSICOS, OBJETIVO DA UC, PERCURSO DE APRENDIZAGEM, SOBRE ESTE MATERIAL

### Community 348 - "ENCONTRO 17 — PRIMEIROS SOCORROS E PREVENÇÃO DE INCÊNDIOS"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 17 — PRIMEIROS SOCORROS E PREVENÇÃO DE INCÊNDIOS, Objetivos, Prevenção e Combate a Incêndios, Primeiros Socorros em Acidente Elétrico, Síntese

### Community 349 - "ENCONTRO 6 — CIRCUITOS EM SÉRIE E EM PARALELO"
Cohesion: 0.29
Nodes (7): Atividade Prática, Circuito em Paralelo, Circuito em Série, Conteúdo, ENCONTRO 6 — CIRCUITOS EM SÉRIE E EM PARALELO, Objetivos, Síntese

### Community 350 - "ENCONTRO 9 — INTERRUPTORES SIMPLES E PARALELO"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 9 — INTERRUPTORES SIMPLES E PARALELO, Interruptor Paralelo (3 vias / Three-way), Interruptor Simples, Objetivos, Síntese

### Community 351 - "OFICINAS DE IMPRESSÃO 3D E ROBÓTICA"
Cohesion: 0.29
Nodes (7): APOSTILA DO ALUNO, APRESENTAÇÃO, COMO ESTA APOSTILA ESTÁ ORGANIZADA, OBJETIVO DA UC, OFICINAS DE IMPRESSÃO 3D E ROBÓTICA, SOBRE ESTE MATERIAL, TABELA DE PERCURSO

### Community 352 - "ENCONTRO 3 — Materiais para Impressão 3D: Filamentos e Resinas"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 3 — Materiais para Impressão 3D: Filamentos e Resinas, Filamentos para FDM, Objetivos, Resinas para SLA, Síntese

### Community 353 - "ENCONTRO 9 — Impressão do Projeto 1 e Análise dos Resultados"
Cohesion: 0.29
Nodes (7): Atividade Prática, Conteúdo, ENCONTRO 9 — Impressão do Projeto 1 e Análise dos Resultados, O processo de impressão na prática, Objetivos, Pós-processamento, Síntese

### Community 354 - "REFORÇO DE LINGUAGENS"
Cohesion: 0.29
Nodes (7): APOSTILA DO ALUNO, Apresentação, Como a apostila está organizada, Objetivo da UC, Percurso da UC — Tabela de Encontros, REFORÇO DE LINGUAGENS, Sobre este material

### Community 355 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.09
Nodes (23): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+15 more)

### Community 356 - "5.1 Bloco 01 — Fundamentos de Testes de Software"
Cohesion: 0.29
Nodes (7): 5.1 Bloco 01 — Fundamentos de Testes de Software, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 357 - "5.2 Bloco 02 — Configuração de Ambiente e Ferramentas"
Cohesion: 0.29
Nodes (7): 5.2 Bloco 02 — Configuração de Ambiente e Ferramentas, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 358 - "5.3 Bloco 03 — Testes Unitários em JavaScript"
Cohesion: 0.29
Nodes (7): 5.3 Bloco 03 — Testes Unitários em JavaScript, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 359 - "5.4 Bloco 04 — Testes de Integração"
Cohesion: 0.29
Nodes (7): 5.4 Bloco 04 — Testes de Integração, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 360 - "5.5 Bloco 05 — Testes End-to-End (E2E)"
Cohesion: 0.29
Nodes (7): 5.5 Bloco 05 — Testes End-to-End (E2E), Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 361 - "5.6 Bloco 06 — Testes de Performance e Acessibilidade"
Cohesion: 0.29
Nodes (7): 5.6 Bloco 06 — Testes de Performance e Acessibilidade, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 362 - "5.7 Bloco 07 — Integração Contínua (CI/CD) e Automação"
Cohesion: 0.29
Nodes (7): 5.7 Bloco 07 — Integração Contínua (CI/CD) e Automação, Capacidades a Serem Trabalhadas, Conhecimentos Relacionados, Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 363 - "4.1 Bloco 01 — Fundamentos de Testes — Autogestão e Automação"
Cohesion: 0.29
Nodes (7): 4.1 Bloco 01 — Fundamentos de Testes — Autogestão e Automação, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 364 - "4.2 Bloco 02 — Conceitos Fundamentais e Planejamento de Testes"
Cohesion: 0.29
Nodes (7): 4.2 Bloco 02 — Conceitos Fundamentais e Planejamento de Testes, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 365 - "4.3 Bloco 03 — Processo Fundamental de Teste — Planejamento e Design"
Cohesion: 0.29
Nodes (7): 4.3 Bloco 03 — Processo Fundamental de Teste — Planejamento e Design, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 366 - "4.5 Bloco 05 — Automação e Otimização de Testes"
Cohesion: 0.29
Nodes (7): 4.5 Bloco 05 — Automação e Otimização de Testes, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 367 - "4.6 Bloco 06 — Documentação e Relatórios de Teste"
Cohesion: 0.29
Nodes (7): 4.6 Bloco 06 — Documentação e Relatórios de Teste, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 368 - "4.7 Bloco 07 — Métodos, Normas e Correção de Defeitos"
Cohesion: 0.29
Nodes (7): 4.7 Bloco 07 — Métodos, Normas e Correção de Defeitos, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 369 - "4.8 Bloco 08 — Boas Práticas e Projeto Integrador"
Cohesion: 0.29
Nodes (7): 4.8 Bloco 08 — Boas Práticas e Projeto Integrador, Capacidades a Serem Trabalhadas, Conteúdos Programáticos (conforme Ementa), Critérios de Avaliação, Estratégias de Ensino, Instrumentos, Recursos

### Community 370 - "4. Autenticação"
Cohesion: 0.33
Nodes (6): 4. Autenticação, Fluxo de sessão (`assets/js/api.js`), Formas de criar um usuário, Redefinição de senha pela tela, 🔒 Restrição de acesso — `usuario.emailsenai`, Teste realizado em 03-09-2026

### Community 371 - "1. CONCEITOS E TIPOS DE BANCO DE DADOS"
Cohesion: 0.33
Nodes (6): 1.1 Definição de Banco de Dados, 1.2.1 Banco de Dados Relacional, 1.2.2 Banco de Dados Não-Relacional (NoSQL), 1.2 Tipos de Banco de Dados Aplicáveis ao Projeto, 1.3 Características de Armazenamento, 1. CONCEITOS E TIPOS DE BANCO DE DADOS

### Community 372 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.09
Nodes (23): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+15 more)

### Community 373 - "Path"
Cohesion: 0.33
Nodes (6): gerar_slide(), Gera um slide a partir de um arquivo markdown, Path, main(), markdown_to_pdf(), Converte Markdown para PDF usando reportlab

### Community 374 - "Referências e Para Continuar Aprendendo"
Cohesion: 0.33
Nodes (6): Canais no YouTube (em português), Livros Sugeridos, Plataformas de Programação e Tecnologia, Referências e Para Continuar Aprendendo, Segurança Digital, Verificação de Fake News

### Community 375 - "ENCONTRO 11 — Aprofundamento: Comunicação Não-Verbal e Empatia"
Cohesion: 0.33
Nodes (6): 11.1 O que seu corpo diz (sem palavras), Ao final deste encontro, você será capaz de:, Atividade 11.1 — Espelho profissional (30 min — duplas), Elementos da comunicação não-verbal:, ENCONTRO 11 — Aprofundamento: Comunicação Não-Verbal e Empatia, Síntese do Encontro 11

### Community 376 - "ENCONTRO 13 — Aprofundamento: Ferramentas Digitais Corporativas"
Cohesion: 0.33
Nodes (6): 13.1 Google Workspace para comunicação corporativa, Ao final deste encontro, você será capaz de:, Atividade 13.1 — Prática com ferramentas (60 min — grupos), ENCONTRO 13 — Aprofundamento: Ferramentas Digitais Corporativas, Ferramentas principais:, Síntese do Encontro 13

### Community 377 - "APOSTILA-CEPLAS-VESPERTINO-2026-02.md"
Cohesion: 0.33
Nodes (5): APOSTILA — Comunicação Oral e Escrita para o Mundo do Trabalho, 📖 Apresentação ao Aluno, 🎯 FASE 1: FERRAMENTAS DIGITAIS E SEGURANÇA, 🎯 FASE 2: AVALIAÇÃO OBJETIVA, Fase Final: Ferramentas Digitais, Avaliação e Encerramento

### Community 378 - "📋 INSTRUÇÕES GERAIS DA PROVA"
Cohesion: 0.33
Nodes (6): Conteúdos abordados:, 📋 INSTRUÇÕES GERAIS DA PROVA, O que você NÃO pode usar:, O que você pode usar:, Orientações durante a prova:, Tipos de questões:

### Community 379 - "LISTA DE PRESENÇA"
Cohesion: 0.33
Nodes (5): Assinatura do Aplicador, Informações da Turma, Lista de Alunos, LISTA DE PRESENÇA, Ocorrências

### Community 380 - "Conteúdo / Proposta"
Cohesion: 0.33
Nodes (6): Avaliação, Conteúdo / Proposta, ENCONTRO 18 — PROJETO INTEGRADOR, Objetivos, Requisitos da apresentação, Sugestões de projetos

### Community 381 - "ENCONTRO 18 — Apresentação dos Projetos e Avaliação Final"
Cohesion: 0.33
Nodes (6): Atividade Final, Como fazer uma boa apresentação técnica, Conteúdo, ENCONTRO 18 — Apresentação dos Projetos e Avaliação Final, Objetivos, Rubrica de avaliação

### Community 382 - "criar_prova_simples_uc1_tic_equipes.py"
Cohesion: 0.44
Nodes (9): add_bullet(), add_heading(), add_info_table(), add_paragraph(), add_rubric(), set_cell_fill(), set_cell_margins(), set_table_widths() (+1 more)

### Community 383 - "PARTE 2 — ROBÓTICA"
Cohesion: 0.33
Nodes (6): Canais no YouTube (em português), GLOSSÁRIO, Livros, PARTE 2 — ROBÓTICA, REFERÊNCIAS SUGERIDAS, Sites

### Community 384 - "Encontro 3-4 — Tema e Ideia Principal (4h)"
Cohesion: 0.33
Nodes (6): Atividades, Encontro 3-4 — Tema e Ideia Principal (4h), Explicação: Tema x Assunto x Ideia Principal, Objetivos, Síntese do Encontro, Texto base

### Community 385 - "Encontro 5-6 — Intenção Comunicativa e Inferências (4h)"
Cohesion: 0.33
Nodes (6): Atividades, Encontro 5-6 — Intenção Comunicativa e Inferências (4h), Explicação: Intenção comunicativa e o que os textos escondem, Objetivos, Síntese do Encontro, Texto base

### Community 386 - "Encontro 8-9 — Paragrafação e Organização de Ideias (4h)"
Cohesion: 0.33
Nodes (6): Atividades, Encontro 8-9 — Paragrafação e Organização de Ideias (4h), Explicação: Como funciona um parágrafo?, Objetivos, Síntese do Encontro, Texto base

### Community 387 - "Apostila_Reforco_Matematica_Raciocinio_Logico.md"
Cohesion: 0.33
Nodes (5): APOSTILA DO ALUNO, Percurso de Aprendizagem, REFORÇO MATEMÁTICA E RACIOCÍNIO LÓGICO, RIO DO SUL MAIS TECH, SENAI / PREFEITURA MUNICIPAL DE RIO DO SUL

### Community 388 - "SLIDE-Matematica-Raciocínio-Logico — Progresso de Criação"
Cohesion: 0.33
Nodes (5): Atividades planejadas (ATIVIDADES-Matematica-Raciocínio-Logico.pptx), Estrutura planejada (30 slides), Log de execução, SLIDE-Matematica-Raciocínio-Logico — Progresso de Criação, Status: Em andamento

### Community 389 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.09
Nodes (23): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+15 more)

### Community 390 - "6. Sincronização com o Resend"
Cohesion: 0.40
Nodes (5): 6.1 Chaves no Vault — ✅ já cadastradas, 6.2 Testar a integração, 6.3 A função `sincronizar_resend()`, 6.4 Chamada a partir do front-end, 6. Sincronização com o Resend

### Community 391 - "9. MCP do Resend (referência)"
Cohesion: 0.40
Nodes (5): 9. MCP do Resend (referência), Categorias de ferramentas disponíveis, Instalação no Claude Code, Limites relevantes, Servidor local (alternativa, via Stdio)

### Community 392 - "8. CHECKLIST DE NORMALIZAÇÃO"
Cohesion: 0.40
Nodes (5): 8. CHECKLIST DE NORMALIZAÇÃO, Qualidade de Implementação, Verificação de 1FN, Verificação de 2FN, Verificação de 3FN

### Community 393 - "13. CONCLUSÕES E RECOMENDAÇÕES"
Cohesion: 0.40
Nodes (5): 13.1 Validação do Projeto, 13.2 Pontos Fortes da Solução, 13.3 Recomendações Futuras, 13.4 Métricas Esperadas, 13. CONCLUSÕES E RECOMENDAÇÕES

### Community 394 - "4. MODELAGEM DE DADOS"
Cohesion: 0.40
Nodes (5): 4.1 Entidades Identificadas, 4.2 Diagrama Entidade-Relacionamento, 4.3 Atributos-Chave, 4.4 Regras de Integridade, 4. MODELAGEM DE DADOS

### Community 395 - "8. PROCEDIMENTOS DE SEGURANÇA E BACKUP"
Cohesion: 0.40
Nodes (5): 8.1 Plano de Backup, 8.2 Scripts de Backup, 8.3 Procedimento de Recuperação, 8.4 Teste de Restauração, 8. PROCEDIMENTOS DE SEGURANÇA E BACKUP

### Community 396 - "ENCONTRO 10 — Mapa de Habilidades e Interesses"
Cohesion: 0.40
Nodes (5): Atividade 10 — Meu Mapa de Habilidades, ENCONTRO 10 — Mapa de Habilidades e Interesses, Objetivos, Síntese, Texto: Você é mais do que uma nota

### Community 397 - "CLAUDE.md — Pasta `/sistema` — Diretrizes Pedagógicas e Estrutura de Aulas"
Cohesion: 0.07
Nodes (28): Atividade Desplugada (Sem Computador), Atividade Prática (Com Computador), 🚀 BOAS PRÁTICAS COMPROVADAS, Checklist para nova UC:, CLAUDE.md — Pasta `/sistema` — Diretrizes Pedagógicas e Estrutura de Aulas, 📝 CONTEÚDOS COMPLEMENTARES OBRIGATÓRIOS, 📐 DISTRIBUIÇÃO DE CARGA HORÁRIA, 🎬 ESTRUTURA DE ATIVIDADES PRÁTICAS (+20 more)

### Community 398 - "ENCONTRO 18 — Apresentações e Encerramento"
Cohesion: 0.40
Nodes (5): Atividade 18 — Apresentações Finais, ENCONTRO 18 — Apresentações e Encerramento, Objetivos, Síntese, Texto: Você chegou até aqui — e este é só o começo

### Community 399 - "ENCONTRO 1 — Quem Sou Eu? Autoconhecimento Inicial"
Cohesion: 0.40
Nodes (5): Atividade 1 — "Quem Sou Eu em 3 Círculos", ENCONTRO 1 — Quem Sou Eu? Autoconhecimento Inicial, Objetivos, Síntese, Texto: Você é o ponto de partida

### Community 400 - "ENCONTRO 2 — O Mundo do Trabalho: Ontem, Hoje e Amanhã"
Cohesion: 0.40
Nodes (5): Atividade 2 — Linha do Tempo das Profissões, ENCONTRO 2 — O Mundo do Trabalho: Ontem, Hoje e Amanhã, Objetivos, Síntese, Texto: O trabalho nunca foi o mesmo

### Community 401 - "ENCONTRO 4 — Automação, Robôs e IA: O Futuro Já Chegou?"
Cohesion: 0.40
Nodes (5): Atividade 4 — Debate: Automação é Ameaça ou Oportunidade?, ENCONTRO 4 — Automação, Robôs e IA: O Futuro Já Chegou?, Objetivos, Síntese, Texto: Robôs vão roubar meu emprego?

### Community 402 - "ENCONTRO 9 — Profissões do Futuro: O Que Ainda Não Existe"
Cohesion: 0.40
Nodes (5): Atividade 9 — Invente uma Profissão, ENCONTRO 9 — Profissões do Futuro: O Que Ainda Não Existe, Objetivos, Síntese, Texto: Carreiras que seus pais nunca imaginaram

### Community 403 - "Status Geral"
Cohesion: 0.25
Nodes (7): Adicionar Correção Automática à Prova Simples de TIC, Passo 1: Atualizar a prova, Passo 2: Criar o corretor, Passo 3: Documentar o uso, Passo 4: Respeitar a restrição de validação, Passo 5: Commit e publicação, Status Geral

### Community 404 - "ENCONTRO 1 — O Que É Tecnologia? Dispositivos Digitais no Cotidiano"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 1 — O Que É Tecnologia? Dispositivos Digitais no Cotidiano, Na Prática, O que você vai aprender, Síntese

### Community 405 - "Status Geral"
Cohesion: 0.25
Nodes (7): Passo 1: Renomear arquivos, Passo 2: Atualizar prova e gerador, Passo 3: Atualizar o corretor, Passo 4: Respeitar a restrição de validação, Passo 5: Commit e publicação, Renomear Dupla para Equipe na Prova de TIC, Status Geral

### Community 406 - "ENCONTRO 14 — Fluxogramas: Desenhando Soluções"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 14 — Fluxogramas: Desenhando Soluções, Na Prática, O que você vai aprender, Síntese

### Community 407 - "ENCONTRO 15 — Scratch: Primeiros Passos na Programação em Blocos"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 15 — Scratch: Primeiros Passos na Programação em Blocos, Na Prática, O que você vai aprender, Síntese

### Community 408 - "ENCONTRO 16 — Scratch: Condições, Repetições e Variáveis"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 16 — Scratch: Condições, Repetições e Variáveis, Na Prática, O que você vai aprender, Síntese

### Community 409 - "ENCONTRO 17 — Scratch: Mini Projeto Final"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 17 — Scratch: Mini Projeto Final, Na Prática, O que você vai aprender, Síntese do Encontro e da UC

### Community 410 - "ENCONTRO 2 — Evolução dos Computadores e História da Tecnologia"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 2 — Evolução dos Computadores e História da Tecnologia, Na Prática, O que você vai aprender, Síntese

### Community 411 - "ENCONTRO 3 — Cidadania Digital: Fake News, Cyberbullying e Pegada Digital"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 3 — Cidadania Digital: Fake News, Cyberbullying e Pegada Digital, Na Prática, O que você vai aprender, Síntese

### Community 412 - "ENCONTRO 4 — Hardware e Software: O Que Tem Dentro de um Computador?"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 4 — Hardware e Software: O Que Tem Dentro de um Computador?, Na Prática, O que você vai aprender, Síntese

### Community 413 - "Status Geral"
Cohesion: 0.29
Nodes (6): Ajustar botão Atualizar das pendências, Passo 1: Destacar o botão Atualizar, Passo 2: Criar modal de processamento, Passo 3: Controlar o modal durante a atualização, Passo 4: Commit e push, Status Geral

### Community 414 - "ENCONTRO 5 — Sistemas Operacionais: Windows, Linux e ChromeOS"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 5 — Sistemas Operacionais: Windows, Linux e ChromeOS, Na Prática, O que você vai aprender, Síntese

### Community 415 - "ENCONTRO 6 — Organização do Computador: Arquivos, Pastas e Atalhos"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 6 — Organização do Computador: Arquivos, Pastas e Atalhos, Na Prática, O que você vai aprender, Síntese

### Community 416 - "Status Geral"
Cohesion: 0.29
Nodes (6): Corrigir Tabelas da Prova de TIC nas Margens, Passo 1: Diagnosticar, Passo 2: Corrigir a causa, Passo 3: Verificar o documento corrigido, Passo 4: Commit e publicação, Status Geral

### Community 417 - "ENCONTRO 7 — Produtividade: Editor de Texto"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 7 — Produtividade: Editor de Texto, Na Prática, O que você vai aprender, Síntese

### Community 418 - "Status Geral"
Cohesion: 0.29
Nodes (6): Criar Prova Simples UC1 TIC em Equipes, Passo 1: Definir a prova, Passo 2: Criar o documento, Passo 3: Registrar a restrição de validação, Passo 4: Commit e publicação, Status Geral

### Community 419 - "Status Geral"
Cohesion: 0.29
Nodes (6): Definir Foco dos Alunos na Prova Prática de TIC, Passo 1: Atualizar o caminho, Passo 2: Definir responsabilidades, Passo 3: Regenerar a prova, Passo 4: Commit e publicação, Status Geral

### Community 420 - "Status Geral"
Cohesion: 0.29
Nodes (6): Leitura da Ementa de Introdução à TIC, Passo 1: Consultar o contexto do projeto, Passo 2: Ler a ementa, Passo 3: Confirmar a leitura, Passo 4: Commit e publicação, Status Geral

### Community 421 - "Status Geral"
Cohesion: 0.29
Nodes (6): Leitura da Prova Prática UC1 TIC, Passo 1: Localizar o documento, Passo 2: Extrair o conteúdo, Passo 3: Confirmar a leitura, Passo 4: Commit e publicação, Status Geral

### Community 422 - "Status Geral"
Cohesion: 0.29
Nodes (6): Padronizar status de matérias e avaliações, Passo 1: Padronizar os selects, Passo 2: Padronizar as restrições SQL, Passo 3: Tratar valores legados, Passo 4: Commit e push, Status Geral

### Community 423 - "ENCONTRO 8 — Apresentações Digitais (Google Slides / PowerPoint)"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 8 — Apresentações Digitais (Google Slides / PowerPoint), Na Prática, O que você vai aprender, Síntese

### Community 424 - "ENCONTRO 9 — Planilhas Simples (Google Sheets / Excel Básico)"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 9 — Planilhas Simples (Google Sheets / Excel Básico), Na Prática, O que você vai aprender, Síntese

### Community 425 - "ENCONTRO 10 — Internet Segura: Vírus, Senhas, Privacidade e Navegadores"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 10 — Internet Segura: Vírus, Senhas, Privacidade e Navegadores, Na Prática, O que você vai aprender, Síntese

### Community 426 - "ENCONTRO 11 — Pensamento Computacional: Decomposição e Padrões"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 11 — Pensamento Computacional: Decomposição e Padrões, Na Prática, O que você vai aprender, Síntese

### Community 427 - "ENCONTRO 12 — Pensamento Computacional: Abstração e Algoritmos Desplugados"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 12 — Pensamento Computacional: Abstração e Algoritmos Desplugados, Na Prática, O que você vai aprender, Síntese

### Community 428 - "Configurar GEMINI.bat como agente de IA"
Cohesion: 0.33
Nodes (5): 1. Instalar o Gemini CLI, 2. Usar as credenciais do Google AI Studio, 3. Iniciar o agente no projeto, Configurar GEMINI.bat como agente de IA, Verificação permitida

### Community 429 - "Configurar OpenClaude com Alibaba Cloud"
Cohesion: 0.33
Nodes (5): 1. Instalar o OpenClaude, 2. Configurar autenticação e modelo, 3. Configurar endpoint do provedor, Configurar OpenClaude com Alibaba Cloud, Verificação permitida

### Community 430 - "ENCONTRO 13 — Algoritmos: Sequência, Decisão e Repetição"
Cohesion: 0.40
Nodes (5): Conteúdo, ENCONTRO 13 — Algoritmos: Sequência, Decisão e Repetição, Na Prática, O que você vai aprender, Síntese

### Community 431 - "FUNDAMENTOS_DA_TECNOLOGIA_E_PROGRAMACAO/gerar_slides.js"
Cohesion: 0.50
Nodes (4): addContentSlide(), addFooter(), pptxgen, pres

### Community 432 - "Status Geral"
Cohesion: 0.33
Nodes (5): Criar tabela de pendências, Passo 1: Definir a estrutura, Passo 2: Criar o script SQL, Passo 3: Commit e push, Status Geral

### Community 433 - "Inicializador OpenClaude com Alibaba Qwen"
Cohesion: 0.33
Nodes (5): 1. Validar os requisitos, 2. Configurar a sessão, 3. Iniciar o agente, Inicializador OpenClaude com Alibaba Qwen, Verificação permitida

### Community 434 - "Status Geral"
Cohesion: 0.33
Nodes (5): Passo 1: Identificar a estrutura, Passo 2: Atualizar instruções, Passo 3: Commit e publicação, Registrar Pasta PROVA_PRATICA no CLAUDE, Status Geral

### Community 435 - "🎯 FASE 3: ENCERRAMENTO E INTEGRAÇÃO"
Cohesion: 0.40
Nodes (5): ✅ Checklist Final para o Aluno, 🎯 FASE 3: ENCERRAMENTO E INTEGRAÇÃO, 🎯 Mensagem Final, 📚 Referências e Materiais de Apoio, 📊 RESUMO DAS 3 FASES

### Community 436 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.09
Nodes (23): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+15 more)

### Community 437 - "Dashboard de Controle de Pendências"
Cohesion: 0.09
Nodes (21): Adicionar Matérias Iniciais, Build para Produção, 📋 Campos de Uma Pendência, Cards de Pendência, 🚀 Como Usar, Cores (variáveis CSS), 💾 Dados Persistentes, Dashboard de Controle de Pendências (+13 more)

### Community 438 - "calcular_notas_pratica.py"
Cohesion: 0.14
Nodes (21): analisar_entregas(), calcular_nota_automatica_total(), calcular_nota_docs(), calcular_nota_sheets(), calcular_nota_slides(), calcular_notas_automaticas(), gerar_feedback(), gerar_js_file() (+13 more)

### Community 439 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 440 - "corretor_prova_pratica.py"
Cohesion: 0.15
Nodes (21): analisar_arquivo(), corrigir_google_docs(), corrigir_google_sheets(), corrigir_google_slides(), corrigir_organizacao(), corrigir_seguranca(), encontrar_arquivos_equipe(), gerar_relatorio_js() (+13 more)

### Community 441 - "Conhecimentos"
Cohesion: 0.09
Nodes (21): 1. Comunicação em Equipes de Trabalho, 2. Segurança da Informação, 3. Internet (World Wide Web), 4.1. Editor de Textos, 4.2. Editor de Planilhas Eletrônicas, 4.3. Editor de Apresentações, 4. Software de Escritório, 5.1. Fundamentos de Hardware (+13 more)

### Community 442 - "AULA 02 — Sistema Operacional e Organização Digital"
Cohesion: 0.10
Nodes (20): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min) (+12 more)

### Community 443 - "9. Recursos Necessários"
Cohesion: 0.40
Nodes (5): 9.1 Ambiente Físico, 9.2 Softwares e Ferramentas, 9.3 Material Didático, 9.4 Documentação, 9. Recursos Necessários

### Community 444 - "Tempo mínimo do modal de atualização de pendências"
Cohesion: 0.40
Nodes (4): 1. Ajustar o fluxo de atualização, 2. Preservar o tratamento de falhas, Tempo mínimo do modal de atualização de pendências, Verificação permitida

### Community 445 - "7. Recursos Necessários"
Cohesion: 0.40
Nodes (5): 7.1 Ambiente Físico, 7.2 Softwares e Ferramentas, 7.3 Material Didático, 7.4 Documentação, 7. Recursos Necessários

### Community 446 - "5. Banco de dados"
Cohesion: 0.50
Nodes (4): 5. Banco de dados, RLS, Tabela `email_contato`, Tabela `unidade` (já existia no projeto)

### Community 447 - "dependencies"
Cohesion: 0.50
Nodes (3): pptxgenjs, dependencies, pptxgenjs

### Community 448 - "5. ANOMALIAS ELIMINADAS PELA NORMALIZAÇÃO"
Cohesion: 0.50
Nodes (4): 5.1 Anomalia de Inserção, 5.2 Anomalia de Atualização, 5.3 Anomalia de Exclusão, 5. ANOMALIAS ELIMINADAS PELA NORMALIZAÇÃO

### Community 449 - "7. NORMALIZAÇÃO DE LINGUAGENS DE BANCO DE DADOS"
Cohesion: 0.50
Nodes (4): 7.1 DDL - Data Definition Language, 7.2 DML - Data Manipulation Language, 7.3 DCL - Data Control Language, 7. NORMALIZAÇÃO DE LINGUAGENS DE BANCO DE DADOS

### Community 450 - "12. DEPLOYMENT E OPERAÇÃO"
Cohesion: 0.50
Nodes (4): 12.1 Instalação do Banco, 12.2 Operação Diária, 12.3 Escalabilidade Futura, 12. DEPLOYMENT E OPERAÇÃO

### Community 451 - "2. ARQUITETURA DE BANCO DE DADOS"
Cohesion: 0.50
Nodes (4): 2.1 Componentes da Arquitetura Cliente-Servidor, 2.2 Arquitetura Escolhida: Centralizada com Replicação Futura, 2.3 Justificativa da Escolha, 2. ARQUITETURA DE BANCO DE DADOS

### Community 452 - "3. ANÁLISE COMPARATIVA DE SGBDs"
Cohesion: 0.50
Nodes (4): 3.1 Critérios de Seleção, 3.2 Matriz de Avaliação, 3.3 Justificativa Final: MySQL 8.0, 3. ANÁLISE COMPARATIVA DE SGBDs

### Community 453 - "Ignorar arquivo ALIBABA_IA.txt"
Cohesion: 0.50
Nodes (3): 1. Atualizar o Git Ignore, Ignorar arquivo ALIBABA_IA.txt, Verificação permitida

### Community 454 - "7. NORMALIZAÇÃO ATÉ 3ª FORMA NORMAL"
Cohesion: 0.50
Nodes (4): 7.1 Processo de Normalização, 7.2 Desnormalizações Controladas, 7.3 Anomalias Eliminadas, 7. NORMALIZAÇÃO ATÉ 3ª FORMA NORMAL

### Community 455 - "9. FERRAMENTAS DE MANIPULAÇÃO E EXECUÇÃO"
Cohesion: 0.50
Nodes (4): 9.1 Ambientes Disponíveis, 9.2 Ferramentas Instaladas, 9.3 Configuração do Ambiente, 9. FERRAMENTAS DE MANIPULAÇÃO E EXECUÇÃO

### Community 456 - "REFERÊNCIAS E RECURSOS"
Cohesion: 0.50
Nodes (4): Documentação Oficial, Ferramentas Utilizadas, Padrões de Projeto, REFERÊNCIAS E RECURSOS

### Community 457 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 458 - "Compositor"
Cohesion: 0.10
Nodes (24): carregar_config(), cmd_gerar(), cmd_template(), cmd_validar(), Compositor, construir_template(), cor(), dividir_excedentes() (+16 more)

### Community 459 - "LISTAS-EMAIL.js"
Cohesion: 0.31
Nodes (7): listaPorId(), LISTAS_EMAIL, paraContatosResend(), paraCSV(), resumo(), separarNome(), todosOsContatos()

### Community 461 - "PARTE 1 — IMPRESSÃO 3D"
Cohesion: 0.50
Nodes (3): PARTE 1 — IMPRESSÃO 3D, RIO DO SUL MAIS TECH, SENAI | PREFEITURA MUNICIPAL DE RIO DO SUL

### Community 462 - "AULA 06 — Segurança da Informação"
Cohesion: 0.10
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 463 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 464 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 465 - "AULA 06 — Segurança da Informação"
Cohesion: 0.11
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 466 - "11. Referências"
Cohesion: 0.50
Nodes (4): 11.1 Ferramentas, 11.2 Recursos Educacionais, 11.3 Comunidades, 11. Referências

### Community 467 - "4. Situação de Aprendizagem — Desafio Central"
Cohesion: 0.50
Nodes (4): 4.1 Contexto, 4.2 Desafio Principal, 4.3 Resultados Esperados, 4. Situação de Aprendizagem — Desafio Central

### Community 468 - "8. Metodologia e Estratégias Transversais"
Cohesion: 0.50
Nodes (4): 8.1 Abordagem Pedagógica, 8.2 Diferenciação, 8.3 Avaliação Contínua, 8. Metodologia e Estratégias Transversais

### Community 469 - "11. Observações Importantes"
Cohesion: 0.50
Nodes (4): 11.1 Alinhamento com Ementa, 11.2 Ferramentas, 11.3 Flexibilidade, 11. Observações Importantes

### Community 470 - "ClaudeGeradoAulasService"
Cohesion: 0.12
Nodes (9): ClaudeGeradoAulasService, Inicializa o cliente Anthropic, Verifica se as aulas já foram geradas Args: conteudo_aulas_json: Dict com…, Extrai JSON válido de um texto que pode conter formatação markdown Args: texto:…, Gera conteúdo de slides em markdown baseado na aula Args: aula_data: dict com…, Gera conteúdo de apostila em HTML baseado na aula Args: aula_data: dict com…, Cria registro JSON para armazenar em conteudo_aulas Args: plano_aulas: dict com…, Gera plano de aulas usando Claude API Args: ementa_content: Conteúdo da ementa… (+1 more)

### Community 471 - "6. Metodologia e Estratégias Transversais"
Cohesion: 0.50
Nodes (4): 6.1 Abordagem Pedagógica, 6.2 Diferenciação, 6.3 Avaliação Contínua (60% da nota final), 6. Metodologia e Estratégias Transversais

### Community 472 - "AULA 06 — Segurança da Informação"
Cohesion: 0.11
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 473 - "8. Base de alunos — `LISTAS-EMAIL.js`"
Cohesion: 0.67
Nodes (3): 8. Base de alunos — `LISTAS-EMAIL.js`, ⚠️ Dois pontos de atenção, Funções exportadas

### Community 474 - "5. SCRIPTS SQL - DDL E DML"
Cohesion: 0.67
Nodes (3): 5.1 Scripts DDL (Data Definition Language), 5.2 Scripts DML (Data Manipulation Language), 5. SCRIPTS SQL - DDL E DML

### Community 475 - "Pesquisa — Planilha Excel de Controle de Abate de Suínos"
Cohesion: 0.11
Nodes (17): Abas criadas, ABATE, AGENDAMENTOS (Porcos a chegar), Arquivo Excel Gerado, Boas Práticas Operacionais, Campos Principais, COLONOS, Contexto (+9 more)

### Community 476 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 479 - "ATIVIDADE PRÁTICA — Planilha Eletrônica"
Cohesion: 0.12
Nodes (15): ATIVIDADE PRÁTICA — Planilha Eletrônica, Colunas da planilha, Contexto, Critérios de Avaliação, Dados para digitar, Divisão da Dupla, Formatação obrigatória, Fórmula da coluna F — Peso Médio (+7 more)

### Community 480 - "Arquivos"
Cohesion: 0.12
Nodes (15): Arquivos, `ATIVIDADE-EXCEL-13-08-2026.docx`, `ATIVIDADE-EXCEL.md`, AULA 07 — 11/08/2026, `Controle_Abate_Pamplona.xlsx`, `GASTOS-CASA.ini`, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo (+7 more)

### Community 481 - "ATIVIDADE INTEGRADORA — UC1 Introdução à TIC"
Cohesion: 0.12
Nodes (15): ATIVIDADE INTEGRADORA — UC1 Introdução à TIC, Distribuição do Tempo, Identificação da Dupla, Orientações Gerais, Pontuação Total, SENAI · Aprendizagem Industrial · Turma TI01, SEÇÃO 1 — Aula 01 · História da Computação, Hardware e Software, SEÇÃO 2 — Aula 02 · Comunicação, Elementos e Equipes (+7 more)

### Community 482 - "PLANO DE AULAS — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.13
Nodes (14): 1. Estrutura Geral, 2. Blocos de Conteúdo, 3. Cobertura da Ementa, 4. Ambientes e Recursos, 5. Avaliação, 6. Encadeamento dos Produtos do Aluno, 7. Arquivos Gerados, 8. Preparação Prévia do Professor (+6 more)

### Community 483 - "Distribuição de Alunos por Desafio — PROVA SIMPLES UC1 TIC"
Cohesion: 0.13
Nodes (14): Distribuição de Alunos por Desafio — PROVA SIMPLES UC1 TIC, Equipe 01 — 3 alunos, Equipe 02 — 4 alunos, Equipe 03 — 3 alunos, Equipe 04 — 3 alunos, Equipe 05 — 3 alunos, Equipe 06 — 3 alunos, Equipe 07 — 3 alunos (+6 more)

### Community 485 - "GERADOR-AULAS — Sistema Inteligente de Geração de Aulas por Ementas"
Cohesion: 0.14
Nodes (13): 📌 CHECKLIST DE IMPLEMENTAÇÃO, 🎯 CONTEXTO E VISÃO GERAL, 💾 ESTRUTURA DE SAÍDA: PASTAS E ARQUIVOS, GERADOR-AULAS — Sistema Inteligente de Geração de Aulas por Ementas, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Logs e Auditoria, Métricas de Qualidade, 📊 MÉTRICAS E MONITORAMENTO (+5 more)

### Community 487 - "13. Textos de Reforço — Regra Obrigatória"
Cohesion: 0.33
Nodes (6): 13. Textos de Reforço — Regra Obrigatória, 🎯 Critérios de Conformidade, 📝 Definição, 🔄 Fluxo de Criação, ✅ Quantidade Obrigatória, ⚠️ Status de Implementação

### Community 488 - "📋 Temas Sugeridos por Módulo"
Cohesion: 0.33
Nodes (6): Módulo 1 — Leitura e Compreensão Textual, Módulo 2 — Produção Textual, Módulo 3 — Gramática Aplicada, Módulo 4 — Comunicação Oral, Módulo 5 — Autonomia e Estratégias de Estudo, 📋 Temas Sugeridos por Módulo

### Community 489 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): .agents, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 490 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): ANALISES, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 491 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): .claude, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 492 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): docs, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 493 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, scripts

### Community 494 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): APRENDIZAGEM-INDUSTRIAL, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 495 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): BANCO_DE_DADOS, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 496 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): .claude, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 497 - "MATERIAIS — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.33
Nodes (5): 1. O que guardar nesta pasta, 2. O que **não** guardar aqui, 3. Insumos previstos por aula, 4. Convenção de nomes, MATERIAIS — Introdução à Tecnologia da Informação e Comunicação

### Community 498 - "11. Script de Geração (`gerar_slides.js`)"
Cohesion: 0.40
Nodes (5): 11. Script de Geração (`gerar_slides.js`), Dependências, Funcionalidades principais, Objetivo, Uso

### Community 499 - "12. Como trabalhar com esta pasta"
Cohesion: 0.40
Nodes (5): 12. Como trabalhar com esta pasta, Adicionar material complementar, Adicionar um novo encontro à apostila, Atualizar a ementa, Regenerar os slides

### Community 500 - "Parte I — Planejamento completo das aulas"
Cohesion: 0.14
Nodes (14): Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min), Atividade 2 (prática no computador): "Organizar e Empacotar" (50 min), Atividades Práticas, AULA 02 — Sistema Operacional e Organização Digital, Avaliação Formativa, Estratégias de Ensino, Introdução à Tecnologia da Informação e Comunicação — Documento Mestre, Objetivos de Aprendizagem (+6 more)

### Community 501 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): GESTAO_E_CONTROLE_MATERIAIS, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 502 - "CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.15
Nodes (12): 1. O que é esta pasta, 2. Estrutura da pasta, 3. As 10 aulas, 4. Avaliação, 5. Encadeamento dos produtos do aluno, 6. Como trabalhar com esta pasta, 7. Checklist de Conformidade, CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação (+4 more)

### Community 503 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.08
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 504 - "GERADOR DE SLIDES — Dashboard Django + Supabase"
Cohesion: 0.05
Nodes (38): 1. Pré-requisitos, 2. Clonar / Baixar projeto, 3. Instalar dependências, 4. Configurar `.env`, 5. Criar Tabelas Supabase, 6. Criar Storage Bucket, 7. Rodar Migrations, 8. Criar Superuser (Admin) (+30 more)

### Community 505 - "Parte I — Planejamento completo das aulas"
Cohesion: 0.14
Nodes (14): Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min), Atividade 2 (prática no computador): "Organizar e Empacotar" (50 min), Atividades Práticas, AULA 02 — Sistema Operacional e Organização Digital, Avaliação Formativa, Estratégias de Ensino, Introdução à Tecnologia da Informação e Comunicação — Documento Mestre, Objetivos de Aprendizagem (+6 more)

### Community 506 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, TECNICO-INFORMATICA-INTERNET

### Community 507 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, .superpowers

### Community 508 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, .vscode

### Community 509 - "9. Sobre os Slides (`Slides_Reforco_Linguagens.pptx`)"
Cohesion: 0.50
Nodes (4): 9. Sobre os Slides (`Slides_Reforco_Linguagens.pptx`), Estrutura de Slides, Geração Automática, Paleta de Cores

### Community 510 - "CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.15
Nodes (12): 1. O que é esta pasta, 2. Estrutura da pasta, 3. As 10 aulas, 4. Avaliação, 5. Encadeamento dos produtos do aluno, 6. Como trabalhar com esta pasta, 7. Checklist de Conformidade, CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação (+4 more)

### Community 511 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.06
Nodes (31): 1. Acolhimento e diagnóstico inicial (20 min), 2. História da Computação (50 min), 3. Hardware × Software (45 min), 4. Mouse: uso e domínio (30 min), 5. Teclado: regiões e atalhos essenciais (35 min), 6. Área de Trabalho (Desktop) (25 min), 7. Pastas e Arquivos: a organização digital (35 min), As gerações de computadores (+23 more)

### Community 512 - "AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC"
Cohesion: 0.18
Nodes (11): Atividade 1 (desplugada): "Hardware ou Software?" + Peças na Mesa (30 min), Atividade 2 (prática no computador): "Missão Área de Trabalho" (45 min), Atividade 3 (opcional, se houver tempo): "Caça ao Atalho" (15 min), Atividades Práticas, AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC, Avaliação Formativa, Estratégias de Ensino, Objetivos de Aprendizagem (+3 more)

### Community 513 - "14. Checklist de Conformidade"
Cohesion: 0.67
Nodes (3): 14. Checklist de Conformidade, ✅ Obrigatório, ⚠️ Para Validação

### Community 514 - "15. Referências Rápidas"
Cohesion: 0.67
Nodes (3): 15. Referências Rápidas, Arquivos relacionados (fora desta pasta), Contato / Suporte

### Community 515 - "8. Sobre a Apostila (`Apostila_Reforco_Linguagens.md`)"
Cohesion: 0.67
Nodes (3): 8. Sobre a Apostila (`Apostila_Reforco_Linguagens.md`), Estrutura, Público-alvo

### Community 516 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.67
Nodes (3): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 517 - "Gabarito técnico — Prova Simples UC1 TIC em equipes"
Cohesion: 0.15
Nodes (12): 1. Organização e entrega — 1,0 ponto automático, 2. Google Docs — 2,0 pontos automáticos, 3. Google Sheets — 2,0 pontos automáticos, 4. Google Slides — 1,5 ponto automático, 5. Segurança e interpretação — 0,5 ponto automático, 6. Rubrica docente — 3,0 pontos, Apresentação e cooperação — até 1,0 ponto, Clareza e linguagem — até 1,0 ponto (+4 more)

### Community 518 - "AULA 07 — Software de Escritório: Editor de Textos"
Cohesion: 0.08
Nodes (26): 1. Tipos e Funções de Editores de Texto (20 min), 2. A Interface e os Controles de Exibição (20 min), 3. Formatação de Fonte e Parágrafo (40 min), 4. Configuração de Página (25 min), 5. Tabelas, Figuras e Gráficos (40 min), 6. Revisão: Ortografia, Dicionário e Controle de Alterações (35 min), 7. Arquivamento e Impressão (20 min), Atividade 1 (prática): "Reforma do Documento" (45 min) (+18 more)

### Community 519 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 520 - "AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem"
Cohesion: 0.08
Nodes (25): 1. Internet × World Wide Web (30 min), 2. Navegadores (35 min), 3. Sites e Mecanismos de Busca (40 min), 4. Download e Gravação de Arquivos (25 min), 5. Correio Eletrônico (40 min), 6. Armazenamento e Compartilhamento em Nuvem (30 min), 7. Direitos Autorais e Citação de Fontes (25 min), 8. Políticas de Uso (15 min) (+17 more)

### Community 521 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 522 - "AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe"
Cohesion: 0.08
Nodes (24): 1. Abertura: "Um dia sem entender ninguém" (20 min), 2. Elementos da Comunicação (50 min), 3. Tipos de Ruído (25 min), 4. Níveis de Fala e Linguagem Profissional (45 min), 5. Comunicação em Equipes de Trabalho (40 min), 6. Gestão de Conflitos (35 min), Atividade 1 (desplugada): "Telefone Industrial" (30 min), Atividade 2 (desplugada): "Tradutor de Níveis de Fala" (30 min) (+16 more)

### Community 524 - "Conteúdo Programático"
Cohesion: 0.15
Nodes (13): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+5 more)

### Community 525 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.09
Nodes (23): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+15 more)

### Community 526 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.09
Nodes (23): 1. Para que serve uma planilha (20 min), 2. Estrutura: Linhas, Colunas, Células e Endereços (30 min), 3. Formatação de Células (30 min), 4. Fórmulas e Funções Básicas (55 min), 5. Classificação e Filtragem (30 min), 6. Tabelas, Quadros e Gráficos (35 min), 7. Configuração de Página e Impressão (20 min), A função SE em detalhe (+15 more)

### Community 527 - "views.py"
Cohesion: 0.12
Nodes (16): GeracaoSlideAdmin, GeracaoSlide, api_gerador_aulas(), deletar_geracao(), download_slide(), geracao_detalhe(), nova_geracao_aulas(), Detalhes de uma geração específica (+8 more)

### Community 528 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 529 - "Parte I — Planejamento completo das aulas"
Cohesion: 0.14
Nodes (14): Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min), Atividade 2 (prática no computador): "Organizar e Empacotar" (50 min), Atividades Práticas, AULA 02 — Sistema Operacional e Organização Digital, Avaliação Formativa, Estratégias de Ensino, Introdução à Tecnologia da Informação e Comunicação — Documento Mestre, Objetivos de Aprendizagem (+6 more)

### Community 530 - "AULA 09 — Editor de Apresentações e Projeto Integrador"
Cohesion: 0.10
Nodes (20): 1. Tipos e Finalidades de Apresentação (25 min), 2. Estrutura de uma Boa Apresentação (30 min), 3. Formatação, Layouts e Temas (35 min), 4. Figuras, Objetos, Tabelas e Gráficos (35 min), 5. Recursos Multimídia (30 min), 6. Controles de Exibição e Modo Apresentador (20 min), 7. Arquivamento e Exportação (15 min), "Apresentação de Ocorrência ao Setor" (+12 more)

### Community 531 - "AULA 06 — Segurança da Informação"
Cohesion: 0.11
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 532 - "Diagnóstico atual — 04/09/2026, 14:20"
Cohesion: 0.14
Nodes (13): `analise_tamanhos.py`, ANALISES, Diagnóstico atual — 04/09/2026, 14:20, Duplicados identificados — 42,9 MB recuperáveis, Execução, Maiores arquivos, Maiores pastas, Onde está o peso (+5 more)

### Community 533 - "AULA 06 — Segurança da Informação"
Cohesion: 0.11
Nodes (19): 1. Abertura: "O que eu perderia hoje?" (20 min), 2. Os Pilares da Segurança da Informação (40 min), 3. Legislação Aplicável (25 min), 4. Tipos de Golpes na Internet (40 min), 5. Contas e Senhas (35 min), 6. Navegação Segura (25 min), 7. Backup (30 min), 8. Códigos Maliciosos (Malware) (30 min) (+11 more)

### Community 534 - "Parte I — Planejamento completo das aulas"
Cohesion: 0.14
Nodes (14): Atividade 1 (desplugada): "Arquitetos da Pasta" (30 min), Atividade 2 (prática no computador): "Organizar e Empacotar" (50 min), Atividades Práticas, AULA 02 — Sistema Operacional e Organização Digital, Avaliação Formativa, Estratégias de Ensino, Introdução à Tecnologia da Informação e Comunicação — Documento Mestre, Objetivos de Aprendizagem (+6 more)

### Community 535 - "Slide"
Cohesion: 0.20
Nodes (8): NovoSlideForm, Formulário para criar novo slide com metadados, Tabela de slides sincronizada com Supabase, Slide, novo_slide(), Novo slide com salvamento de metadados ANTES de gerar, Detalhes de um slide específico, slide_detalhe()

### Community 536 - "Conteúdo Programático"
Cohesion: 0.15
Nodes (13): 1. Retomada (15 min), 2. O que é um Texto Técnico (40 min), 3. Normas Aplicáveis à Redação Técnica (25 min), 4.1 Relatório, 4.2 Ata, 4.3 Memorando, 4.4 Resumo, 4. Os Quatro Documentos Essenciais (70 min) (+5 more)

### Community 537 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 538 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 539 - "Conteúdo Programático"
Cohesion: 0.20
Nodes (10): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Conteúdo Programático (+2 more)

### Community 540 - "Status Geral"
Cohesion: 0.29
Nodes (6): Criar apresentação Introdução TIC — Gelvazio Camargo, Passo 1: Importar a apresentação de referência, Passo 2: Adaptar o conteúdo, Passo 3: Exportar o PowerPoint, Passo 4: Commit e push, Status Geral

### Community 541 - "Status Geral"
Cohesion: 0.33
Nodes (5): Completar INTRODUCAO-TIC-VICTOR-ANATO, Passo 1: Consolidar as dez aulas, Passo 2: Preservar o material original, Passo 3: Commit e push, Status Geral

### Community 542 - "Status Geral"
Cohesion: 0.33
Nodes (5): Converter INTRODUCAO-TIC-VICTOR-ANATO para Markdown, Passo 1: Ler a apresentação, Passo 2: Criar o Markdown, Passo 3: Commit e push, Status Geral

### Community 543 - "modelo-slide-senai-2026 — Especificação técnica para reprodução"
Cohesion: 0.17
Nodes (12): 10. Mapa de substituição de conteúdo, 1. Identidade do arquivo, 2. Dimensões do palco, 3. Paleta de cores (tema "Simple Light"), 5. Layouts (4 no total), 7. Inventário de mídia, 8. Receita de reprodução com pptxgenjs, 9. Checklist de conformidade (+4 more)

### Community 544 - "2. Gabarito comentado"
Cohesion: 0.17
Nodes (12): 1. Gabarito rápido, 2. Gabarito comentado, 3. Mapa de acertos por capacidade (uso pedagógico), 4. Registro de aplicação, Eixo 1 — Hardware, Software e Sistema Operacional (Aulas 01 e 02), Eixo 2 — Comunicação Profissional (Aula 03), Eixo 3 — Textos Técnicos (Aula 04), Eixo 4 — Internet, Web e Nuvem (Aula 05) (+4 more)

### Community 545 - "Conteúdo Programático"
Cohesion: 0.20
Nodes (10): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Conteúdo Programático (+2 more)

### Community 546 - "CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.17
Nodes (11): 1. O que é esta pasta, 2. Estrutura da pasta, 5. Regras de Trabalho — Claude, 7. Como trabalhar com esta pasta, 8. Checklist de Conformidade, CLAUDE.md — UC Introdução à Tecnologia da Informação e Comunicação, ✅ Commit e Push obrigatórios — SEMPRE AO FINAL DE CADA RESPOSTA, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto (+3 more)

### Community 547 - "modelo-slide-senai-2026.md"
Cohesion: 0.18
Nodes (9): ✅ Checklist para Reproduzir, Dimensões, 📋 Especificações Gerais, Fontes Utilizadas, Layout Visual, 📝 Notas Importantes, 🎯 Padrões Visuais Identificados, 📐 Recomendações CSS para Claude Design (+1 more)

### Community 548 - "ESTRUTURA-PROVAS — Documentação Consolidada"
Cohesion: 0.08
Nodes (25): 1. Planejamento, 2. Execução (Durante Aulas), 3. Acompanhamento (Após Aulas), 4. Fechamento (Fim da UC), Atribuições do Docente Presencial, Avaliações Objetivas (7 Critérios), Avaliações Práticas (7 Critérios), Checklists de Avaliações (+17 more)

### Community 549 - "Bloco 2 — Operando o Computador"
Cohesion: 0.08
Nodes (25): Analogia do Trabalho, As Gerações de Computadores, Atividade 1 — Caça aos Componentes, Atividade 2 — Organização Digital, Avaliação Formativa, Bloco 1 — Fundamentos, Bloco 2 — Operando o Computador, Bloco 3 — Prática e Fechamento (+17 more)

### Community 550 - "SupabaseService"
Cohesion: 0.13
Nodes (11): Client, Listar todos os cursos do Supabase (tabela: curso), Login com email/password, Cadastro com email/password, Upload de PPTX para storage do Supabase, Criar registro de slide na tabela, Atualizar registro de slide, Listar slides (opcionalmente filtrado por user_id) (+3 more)

### Community 551 - "🚀 GERADOR-SLIDES com Supabase — Setup Rápido"
Cohesion: 0.10
Nodes (19): 1️⃣ Prepare as Credenciais Supabase, 2️⃣ Configure `.env`, 3️⃣ Criar Tabelas no Supabase, 4️⃣ Criar Storage Bucket, 5️⃣ Rodar Migrations Django, 6️⃣ Criar Superuser (Admin), 7️⃣ Iniciar Servidor, 8️⃣ Acessar (+11 more)

### Community 552 - "GERADOR-SLIDES com Django + Supabase Auth"
Cohesion: 0.12
Nodes (17): Banco de Dados, 📝 Checklist de Deploy, 🚀 Como Executar, 📚 Dependências, 📁 Estrutura do Projeto, 🔐 Fluxo de Autenticação, 🔌 Fluxo de Geração com Supabase, GERADOR-SLIDES com Django + Supabase Auth (+9 more)

### Community 553 - "Status Geral"
Cohesion: 0.13
Nodes (14): Criar padrão e gerador de slides — GERADOR-SLIDES, Padrão extraído do arquivo base, Passo 10: Commit, Passo 1: Extrair e medir o padrão do PPTX base, Passo 2: Criar `padrao_slides.json`, Passo 3: Criar `PADRAO-DE-SLIDES.md`, Passo 4: Criar `SINTAXE-MARKDOWN.md`, Passo 5: Criar `gerar_slides.py` (+6 more)

### Community 554 - "Relatório de risco Git / LFS — aulas-senai"
Cohesion: 0.10
Nodes (20): 1. Refinar o `.gitignore` — trocar o bloqueio total por um limite de tamanho, 1. Veredito, 2.1 Já gravados no histórico — e já no GitHub, 2.2 No working tree — hoje bloqueados pelo `.gitignore`, 2. Ativar o LFS para `.pptx` — se quiser versioná-los mesmo, 2. Os arquivos na zona de aviso (50 – 100 MB), 3. Deduplicar antes de qualquer coisa, 3. Onde está o risco real (+12 more)

### Community 555 - "PADRÃO DE SLIDES — Projeto aulas-senai"
Cohesion: 0.14
Nodes (14): 10. Rodapé, 11. Checklist de conformidade, 12. Erros comuns, 13. Por que Python e não JavaScript, 14. Arquivos do padrão, 1. Princípio, 2. Especificação técnica, 3. Paleta (+6 more)

### Community 556 - "GERADOR DE SLIDES SENAI"
Cohesion: 0.18
Nodes (7): Documentação, GERADOR DE SLIDES SENAI, Markdown mínimo, O padrão, Os três comandos, Requisitos, Uso rápido

### Community 557 - "check_auth"
Cohesion: 0.40
Nodes (3): check_auth(), Verificar se usuário está autenticado, AuthenticationMiddleware

### Community 558 - "Conteúdo Programático"
Cohesion: 0.20
Nodes (10): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Conteúdo Programático (+2 more)

### Community 559 - "Checklist Prova Prática"
Cohesion: 0.22
Nodes (8): Checklist Prova Prática, Conteudo, CRITÉRIO 1: Da evidência de aplicação da(s) avaliação(ões) — ⚠️ ELIMINATÓRIO, CRITÉRIOS - AVALIAÇÕES PRÁTICAS (12,5% da nota), Informacao Geral, Notas de Implementacao, Posicionamento do Documento, Quando Usar Prova Prática

### Community 560 - "SINTAXE MARKDOWN — Entrada do Gerador de Slides"
Cohesion: 0.22
Nodes (9): 1. Esqueleto mínimo, 2. Front-matter, 3. Estrutura de slides, 5. Ordem de composição, 6. Marcação inline, 7. Quebra automática, 8. Erros mais comuns, 9. Referência rápida (+1 more)

### Community 561 - "4. Conteúdo"
Cohesion: 0.29
Nodes (7): 4.1 Bullets, 4.2 Caixa de destaque, 4.3 Tabela, 4.4 Imagem, 4.5 Notas do apresentador, 4.6 Diretiva de layout, 4. Conteúdo

### Community 562 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 563 - "🔧 Configuração Inicial"
Cohesion: 0.33
Nodes (6): 1. Variáveis de Ambiente, 2. Obter Credenciais Supabase, 3. Criar Tabelas no Supabase (SQL), 4. Criar Storage Bucket, 5. Rodar Migrações Django, 🔧 Configuração Inicial

### Community 564 - "🐛 Troubleshooting"
Cohesion: 0.40
Nodes (5): Erro: "Acesso negado ao bucket", Erro: "SUPABASE_URL não configurado", Erro: "Usuário não autenticado", Storage cheio?, 🐛 Troubleshooting

### Community 565 - "ENTRADAS-AULAS-MARKDOWN"
Cohesion: 0.40
Nodes (4): ENTRADAS-AULAS-MARKDOWN, Fluxo, Formato do Markdown, Validação

### Community 566 - "✅ Funcionalidades Implementadas"
Cohesion: 0.22
Nodes (9): 1. **Criação Automática de Pendências**, 2. **Descrições Automáticas**, 3. **Sincronização Bidirecional**, 4. **Interface de Pendências**, 5. **Campos da Tabela `pendencias`**, ⚡ Fluxo de Atualização Obrigatório, ✅ Funcionalidades Implementadas, 🎯 Implementação Completa do Sistema de Pendências (26-08-2026) (+1 more)

### Community 567 - "🔔 Sistema de Pendências — Regra Crítica"
Cohesion: 0.25
Nodes (8): ✅ Checklist de implementação, Definição de Pendências, 📊 Estrutura da Tabela "pendencias", Fluxo Automático de Pendências, 🔄 Fluxo de Inserção de Pendências, Implementação Obrigatória, ❌ O que NÃO fazer, 🔔 Sistema de Pendências — Regra Crítica

### Community 568 - "🎯 CONTEXTO-PROJETO"
Cohesion: 0.29
Nodes (7): 📊 API Endpoints, 🚀 Como Usar, 🎯 CONTEXTO-PROJETO, 📋 Estrutura de Cursos e UCs Existentes, 🔧 Ferramentas Criadas, ⏳ Ferramentas Planejadas (Parcialmente Implementadas), ✅ Implementações Realizadas

### Community 569 - "📊 PADRÃO OBRIGATÓRIO DE INFOGRÁFICOS"
Cohesion: 0.33
Nodes (6): 🎨 CSS BASE OBRIGATÓRIO, ✅ Elementos Obrigatórios, 🎨 Estrutura Recomendada de Cores por Tema, ✅ Exemplo de Estrutura, ❌ O Que NUNCA Fazer, 📊 PADRÃO OBRIGATÓRIO DE INFOGRÁFICOS

### Community 570 - "Estrutura de Unidades Curriculares"
Cohesion: 0.40
Nodes (5): Curso Ficha Produto Mais Tech, Curso Gestão e Controle de Materiais, Curso Técnico de Informática para Internet, Curso Técnico em Desenvolvimento de Sistemas, Estrutura de Unidades Curriculares

### Community 571 - "Checklist Prova Objetiva"
Cohesion: 0.40
Nodes (4): Checklist Prova Objetiva, Conteudo, Informacao Geral, Notas de Implementacao

### Community 572 - "Graphify"
Cohesion: 0.67
Nodes (3): Graphify, Inventário atual da raiz, Inventário atual de `sistema/`

### Community 573 - "Modelo Prova Objetiva CT ELETROMECÂNICA (1) (2)"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Modelo Prova Objetiva CT ELETROMECÂNICA (1) (2), Notas de Implementacao

### Community 574 - "Modelo Prova Prática CT ELETROTÉCNICA"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Modelo Prova Prática CT ELETROTÉCNICA, Notas de Implementacao

### Community 575 - "PROMPT - Rubricas Capacidades Socioemocionais"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Notas de Implementacao, PROMPT - Rubricas Capacidades Socioemocionais

### Community 576 - "Prova Objetiva"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Notas de Implementacao, Prova Objetiva

### Community 577 - "Prova Prática (3)"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Notas de Implementacao, Prova Prática (3)

### Community 578 - "Template Prova Prática"
Cohesion: 0.40
Nodes (4): Conteudo, Informacao Geral, Notas de Implementacao, Template Prova Prática

### Community 579 - "7. Tipos de slide"
Cohesion: 0.40
Nodes (5): 7.1 Capa, 7.2 Seção (divisória), 7.3 Conteúdo, 7.4 Colunas, 7. Tipos de slide

### Community 580 - "TASKS — Rastreamento de Gerações de Slides"
Cohesion: 0.40
Nodes (4): Como Consultar, Estrutura, Fluxo Automático, TASKS — Rastreamento de Gerações de Slides

### Community 581 - "AULA 08 — Software de Escritório: Planilhas Eletrônicas"
Cohesion: 0.20
Nodes (10): Atividade 1 (desplugada): "A Planilha no Papel" (25 min), Atividade 2 (prática): "Controle de Produção e Estoque" (60 min), Atividades Práticas, AULA 08 — Software de Escritório: Planilhas Eletrônicas, Avaliação Formativa, Estratégias de Ensino, Objetivos de Aprendizagem, Observações do Professor (+2 more)

### Community 582 - "🛠️ Operações Comuns"
Cohesion: 0.50
Nodes (4): Criar Superuser Django (Admin), Listar Slides do Supabase (CLI), 🛠️ Operações Comuns, Verificar Storage

### Community 583 - "📊 Modelos Django"
Cohesion: 0.50
Nodes (4): GeracaoSlide, 📊 Modelos Django, Slide ⭐ (Novo), UsuarioSupabase

### Community 595 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 596 - "AULA 10 — Avaliação Objetiva e Avaliação Prática"
Cohesion: 0.18
Nodes (11): AULA 10 — Avaliação Objetiva e Avaliação Prática, Composição da Nota, Momento 1 — Revisão Relâmpago (20 min), Momento 2 — Avaliação Objetiva (60 min), Momento 4 — Avaliação Prática (100 min), Momento 5 — Fechamento da Unidade Curricular (25 min), Objetivo da Aula, Observações do Professor (+3 more)

### Community 597 - "AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.18
Nodes (11): AVALIAÇÃO PRÁTICA — Introdução à Tecnologia da Informação e Comunicação, CENÁRIO, Conteúdo obrigatório, Distribuição dos Pontos, ENTREGA, Exigências, Formatação obrigatória, Instruções Gerais (+3 more)

### Community 598 - "Instruções — Corretor híbrido da Prova Simples UC1 TIC"
Cohesion: 0.18
Nodes (10): Aplicação com os estudantes, Avaliação docente — 3,0 pontos, Como a nota é calculada, Correção automática — 7,0 pontos, Estrutura obrigatória da planilha dos estudantes, Instalação, Instruções — Corretor híbrido da Prova Simples UC1 TIC, Leitura do feedback (+2 more)

### Community 599 - "Conhecimentos"
Cohesion: 0.18
Nodes (11): 1. Comunicação em Equipes de Trabalho, 2. Segurança da Informação, 3. Internet (World Wide Web), 5.1. Fundamentos de Hardware, 5.2. Sistema Operacional, 5. Informática, 6. Textos Técnicos, 7. Comunicação (+3 more)

### Community 600 - "6. Estrutura slide a slide"
Cohesion: 0.20
Nodes (10): 6. Estrutura slide a slide, Slide 1 — "Apresentação" · layout `TITLE_AND_TWO_COLUMNS`, Slide 2 — "Plano de Ensino" · layout `TITLE_AND_TWO_COLUMNS`, Slide 3 — "Capacidades Socioemocionais" · layout `TITLE_ONLY`, Slide 4 — "Conhecimentos (Literatura)" · layout `TITLE_ONLY`, Slide 5 — "Conteudo" · layout `TITLE_ONLY`, Slide 6 — "Combinados!" · layout `TITLE_ONLY`, Slide 7 — "Vale nota Professor?" · layout `TITLE_ONLY` (+2 more)

### Community 601 - "🎨 Estrutura dos 9 Slides"
Cohesion: 0.20
Nodes (10): 🎨 Estrutura dos 9 Slides, Slide 1 — Apresentação, Slide 2 — Plano de Ensino, Slide 3 — Capacidades Socioemocionais, Slide 4 — Conhecimentos, Slide 5 — Conteúdo, Slide 6 — Combinados!, Slide 7 — Vale Nota Professor? (+2 more)

### Community 602 - "Conteúdo Programático"
Cohesion: 0.20
Nodes (10): 1. Retomada e correção da tarefa (20 min), 2. O que é um Sistema Operacional (45 min), 3. A Interface: barra de tarefas e barras de ferramentas (30 min), 4. Periféricos na prática (35 min), 5. Organização de Arquivos em Pastas (35 min), 6. Pesquisa de Arquivos e Diretórios (25 min), 7. Compactação de Arquivos (25 min), Conteúdo Programático (+2 more)

### Community 603 - "AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos"
Cohesion: 0.20
Nodes (10): Atividade 1 (desplugada): "Leitura de Documento Real" (40 min), Atividade 2 (prática): "Ocorrência na Linha 3" — Relatório + Ata (50 min), Atividades Práticas, AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos, Avaliação Formativa, Estratégias de Ensino, Objetivos de Aprendizagem, Observações do Professor (+2 more)

### Community 604 - "AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.20
Nodes (10): AVALIAÇÃO OBJETIVA — Introdução à Tecnologia da Informação e Comunicação, EIXO 1 — Hardware, Software e Sistema Operacional (questões 01 a 08), EIXO 2 — Comunicação Profissional (questões 09 a 14), EIXO 3 — Textos Técnicos (questões 15 a 19), EIXO 4 — Internet, Web e Nuvem (questões 20 a 26), EIXO 5 — Segurança da Informação (questões 27 a 33), EIXO 6 — Editor de Textos (questões 34 a 36), EIXO 7 — Planilhas e Apresentações (questões 37 a 40) (+2 more)

### Community 605 - "CRITÉRIOS DE CORREÇÃO — Avaliação Prática"
Cohesion: 0.20
Nodes (10): Conteúdo (1,0), CRITÉRIOS DE CORREÇÃO — Avaliação Prática, Formatação (1,0), QUADRO-RESUMO DA NOTA, Registro Qualitativo (acompanhamento formativo das Aulas 01 a 09), Relação com as Capacidades da Ementa, TAREFA 1 — Organização, Backup e Comunicação (1,5 ponto), TAREFA 2 — Relatório Técnico (2,0 pontos) (+2 more)

### Community 606 - "3. Unidade Curricular 1: Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.20
Nodes (10): 3. Unidade Curricular 1: Introdução à Tecnologia da Informação e Comunicação, Ambientes Pedagógicos e Recursos, Capacidades Básicas, Capacidades Socioemocionais, Carga Horária, Eixo Estruturante BNCC / Competência Integradora, Ferramentas de Ensino, Organização das provas práticas (+2 more)

### Community 607 - ".list_materias"
Cohesion: 0.33
Nodes (5): Listar matérias (opcionalmente filtradas por curso), cursos(), gerador_aulas(), Página de gerenciamento de cursos, Página de gerencimento de geração de aulas

### Community 608 - "Análise para 3 Alunos (15 anos)"
Cohesion: 0.22
Nodes (8): Adequação Etária (15 anos), Análise de Tempo — Prova Prática Simples UC1 TIC (Equipipes de 3), Análise para 3 Alunos (15 anos), Conclusão, Distribuição Atual (2 alunos), Estrutura Atual da Prova (2h = 120 min), Recomendação: Adaptar para 3 alunos, Tempo disponível por entrega

### Community 609 - "Fase 3: Geração de Conteúdo"
Cohesion: 0.25
Nodes (8): 3.1 Slides Interativos (HTML5/PDF), 3.2 Apostila em Markdown/HTML, 3.3 Guia do Professor, 3.4 Atividades Formativas, Fase 1: Análise da Ementa, Fase 2: Planejamento de Aulas, Fase 3: Geração de Conteúdo, 🔄 FLUXO DE GERAÇÃO DE AULAS

### Community 610 - "📝 REGRAS DE GERAÇÃO"
Cohesion: 0.25
Nodes (8): Regra 1: Fidelidade à Ementa, Regra 2: Contexto Profissional e Industrial, Regra 3: Integração de Competências Socioemocionais, Regra 4: Estrutura de Aula, Regra 5: Quantidade de Slides, Regra 6: Nenhuma Duplicata, Regra 7: Linguagem Clara, 📝 REGRAS DE GERAÇÃO

### Community 612 - "Conhecimentos"
Cohesion: 0.29
Nodes (7): 1. O Impacto da Falta de Ética nos Ambientes de Trabalho, 2. Código de Ética Profissional, 3. Acidentes do Trabalho e Doenças Ocupacionais, 4. Medidas de Controle, 5. Riscos Ocupacionais, 6. Segurança do Trabalho, Conhecimentos

### Community 613 - "🎯 Novo Fluxo: Criar Slide com Metadados"
Cohesion: 0.33
Nodes (6): Diagrama de fluxo, 🎯 Novo Fluxo: Criar Slide com Metadados, Passo 1: Acessar novo formulário, Passo 2: Preencher formulário, Passo 3: Salvar metadados, Passo 4: Upload do PPTX (depois)

### Community 614 - "dashboard"
Cohesion: 0.33
Nodes (5): Obter informações de uso do storage, dashboard(), listar_arquivos_entrada(), Lista arquivos markdown na pasta de entrada, Dashboard principal com lista de gerações

### Community 615 - "Tabelas Envolvidas"
Cohesion: 0.33
Nodes (6): 1. `materia` table, 2. `avaliacao` table, 3. Nova tabela: `aula_gerada` (proposta), Fluxo de Sincronização, 🔗 INTEGRAÇÃO COM SUPABASE, Tabelas Envolvidas

### Community 616 - "MATERIAIS — Introdução à Tecnologia da Informação e Comunicação"
Cohesion: 0.33
Nodes (5): 1. O que guardar nesta pasta, 2. O que **não** guardar aqui, 3. Insumos previstos por aula, 4. Convenção de nomes, MATERIAIS — Introdução à Tecnologia da Informação e Comunicação

### Community 617 - "markdown_to_docx"
Cohesion: 0.47
Nodes (5): determine_lines_needed(), main(), markdown_to_docx(), Determina a quantidade de linhas necessárias, Converte Markdown para DOCX

### Community 618 - "🛠️ TECNOLOGIAS E FERRAMENTAS"
Cohesion: 0.40
Nodes (5): Backend, Frontend, IA/LLM, 🛠️ TECNOLOGIAS E FERRAMENTAS, Validação

### Community 619 - "🤝 INTEGRAÇÃO COM ESTRUTURA EXISTENTE"
Cohesion: 0.40
Nodes (5): Dashboard professor-senai, Estrutura de Pastas Existente, 🤝 INTEGRAÇÃO COM ESTRUTURA EXISTENTE, Sistema de Pendências, Supabase Sync

### Community 620 - "🔮 FUNCIONALIDADES FUTURAS (Roadmap)"
Cohesion: 0.40
Nodes (5): 🔮 FUNCIONALIDADES FUTURAS (Roadmap), Phase 1: MVP (Planejado para Q4 2026), Phase 2: Expansão (Planejado para Q1 2027), Phase 3: Integração Completa (Planejado para Q2 2027), Phase 4: Avançado (Planejado para Q3 2027)

### Community 621 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, PENDENCIAS-PROFESSOR

### Community 622 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): GERADOR-INFOGRAFICOS, 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações

### Community 623 - "4. Unidade Curricular 2: Saúde e Segurança no Trabalho — *contexto do módulo*"
Cohesion: 0.40
Nodes (5): 4. Unidade Curricular 2: Saúde e Segurança no Trabalho — *contexto do módulo*, Ambientes Pedagógicos e Recursos, Capacidades Básicas, Capacidades Socioemocionais, Eixo Estruturante BNCC / Competência Integradora

### Community 624 - "6. Configuração Supabase"
Cohesion: 0.40
Nodes (5): 6. Configuração Supabase, Controle de perfil (frontend), Estrutura `gabaritos` (coluna JSONB da tabela `curso`), Funções RPC disponíveis, Tabela `curso`

### Community 625 - "🔗 Grafo de conhecimento — SEMPRE na raiz do projeto"
Cohesion: 0.40
Nodes (4): 🔗 Grafo de conhecimento — SEMPRE na raiz do projeto, Onde atualizar o grafo, Onde buscar as informações, Tecnico em Desenvolvimento de Sistemas

### Community 626 - "🚀 CASOS DE USO E EXEMPLOS"
Cohesion: 0.50
Nodes (4): Caso de Uso 1: Geração de UC Completa, Caso de Uso 2: Geração Iterativa com Revisão, Caso de Uso 3: Customização por Turma, 🚀 CASOS DE USO E EXEMPLOS

### Community 627 - "📚 REFERÊNCIAS"
Cohesion: 0.50
Nodes (4): Estrutura de Ementas, Projetos Similares (Inspiração), 📚 REFERÊNCIAS, Tecnologias

### Community 628 - "👤 Responsabilidades"
Cohesion: 0.50
Nodes (4): IA (Claude), Professor, 👤 Responsabilidades, Sistema (GERADOR-AULAS)

### Community 629 - "🔄 Como Usar no Claude Design"
Cohesion: 0.50
Nodes (4): 🔄 Como Usar no Claude Design, Passo 1: Preparar Conteúdo, Passo 2: Criar Slides, Passo 3: Adaptar por Contexto

### Community 630 - "4. Tipografia"
Cohesion: 0.50
Nodes (4): 4. Tipografia, Alinhamento e espaçamento, Escala de tamanhos observada, Fontes declaradas

### Community 631 - "TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)"
Cohesion: 0.50
Nodes (4): Aba `BASE`, Aba `RESUMO`, Análise e gráfico, TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)

### Community 632 - "4. Software de Escritório"
Cohesion: 0.50
Nodes (4): 4.1. Editor de Textos, 4.2. Editor de Planilhas Eletrônicas, 4.3. Editor de Apresentações, 4. Software de Escritório

### Community 633 - "📋 ESTRUTURA DE ENTRADA: AS EMENTAS"
Cohesion: 0.67
Nodes (3): Componentes Obrigatórios de uma Ementa, 📋 ESTRUTURA DE ENTRADA: AS EMENTAS, Exemplo de Ementa Estruturada

### Community 643 - "analise_tamanhos.py"
Cohesion: 0.31
Nodes (9): achar_duplicados(), coletar(), eh_oculta(), formatar(), gravar_csv(), main(), Agrupa arquivos identicos (mesmo tamanho + mesmo MD5) acima de `minimo` bytes., Converte bytes em string legivel (B, KB, MB, GB). (+1 more)

### Community 644 - "🎓 Implementar Integração com Claude API para Geração de Aulas"
Cohesion: 0.25
Nodes (8): Campos Obrigatórios na Tabela `materia`, Estrutura do Campo `conteudo_aulas`, Estrutura Obrigatória da Tabela `materia`, Fluxo de Geração com Claude API, 🎓 Implementar Integração com Claude API para Geração de Aulas, Regra de Leitura - OBRIGATÓRIA, Template - Mostrar Status de Aulas, Views.py - Função para Verificar Status

### Community 645 - "📄 Páginas Implementadas — Gerador de Aulas"
Cohesion: 0.50
Nodes (4): API `POST /api/gerador-aulas/` — Processamento de Ementa, `/gerador-aulas/nova/` — Formulário de Nova Geração, `/gerador-aulas/` — Página de Gerenciamento de Aulas, 📄 Páginas Implementadas — Gerador de Aulas

### Community 646 - "⚙️ Configuração Avançada"
Cohesion: 0.50
Nodes (4): Aumentar Limite de Upload, ⚙️ Configuração Avançada, Personalizar Bucket, Sincronização com Google Drive (Futuro)

### Community 647 - "TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)"
Cohesion: 0.50
Nodes (4): Aba `BASE`, Aba `RESUMO`, Análise e gráfico, TAREFA 3 — Planilha de Análise de Paradas (1,5 ponto)

### Community 648 - "auth_views.py"
Cohesion: 0.21
Nodes (8): login(), logout(), Cadastro com Supabase, signup(), Meta, Representa um usuário autenticado via Supabase, UsuarioSupabase, require_http_methods

### Community 649 - "main"
Cohesion: 0.47
Nodes (5): docx_to_pdf(), main(), markdown_to_docx(), Converte DOCX para PDF, Converte Markdown para DOCX

### Community 650 - "3. Situação de Aprendizagem — Desafio Central"
Cohesion: 0.50
Nodes (4): 3.1 Contexto, 3.2 Desafio Principal, 3.3 Resultados Esperados, 3. Situação de Aprendizagem — Desafio Central

## Knowledge Gaps
- **5732 isolated node(s):** `token`, `contatos`, `unidades`, `PARTICULAS`, `links` (+5727 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **17 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Introdução à Tecnologia da Informação e Comunicação — Documento Mestre` connect `Parte I — Planejamento completo das aulas` to `APRENDIZAGEM-INDUSTRIAL/INTRODUCAO-TIC/AULAS/AULA-10-AVALIACOES.md`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `Parte I — Planejamento completo das aulas` connect `Parte I — Planejamento completo das aulas` to `AULA 01 — Introdução à Computação: História, Hardware, Software e Primeiros Passos no PC`, `AULA 08 — Software de Escritório: Planilhas Eletrônicas`, `AULA 05 — Internet e World Wide Web: Navegação, Pesquisa, E-mail e Nuvem`, `AULA 09 — Editor de Apresentações e Projeto Integrador`, `AULA 06 — Segurança da Informação`, `AULA 10 — Avaliação Objetiva e Avaliação Prática`, `AULA 04 — Textos Técnicos: Relatórios, Atas, Memorandos e Resumos`, `AULA 07 — Software de Escritório: Editor de Textos`, `AULA 03 — Comunicação Profissional: Elementos, Níveis de Fala e Trabalho em Equipe`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `Introdução à Tecnologia da Informação e Comunicação — Documento Mestre` connect `Parte I — Planejamento completo das aulas` to `APRENDIZAGEM-INDUSTRIAL/INTRODUCAO-TIC/AULAS/AULA-10-AVALIACOES.md`?**
  _High betweenness centrality (0.005) - this node is a cross-community bridge._
- **What connects `token`, `contatos`, `unidades` to the rest of the system?**
  _5732 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Currículo Técnico - Informática para Internet` be split into smaller, more focused modules?**
  _Cohesion score 0.022988505747126436 - nodes in this community are weakly interconnected._
- **Should `EMENTA — TESTES DE FRONTEND` be split into smaller, more focused modules?**
  _Cohesion score 0.046511627906976744 - nodes in this community are weakly interconnected._
- **Should `5. CHECKLIST DE AVALIAÇÃO RESPONDIDO` be split into smaller, more focused modules?**
  _Cohesion score 0.05 - nodes in this community are weakly interconnected._