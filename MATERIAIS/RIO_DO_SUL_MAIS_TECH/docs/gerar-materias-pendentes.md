# Tarefa: Gerar Matérias Pendentes — RIO_DO_SUL_MAIS_TECH

**Data:** 2026-09-08  
**Status:** 🔄 Em Progresso  
**Objetivo:** Criar estrutura de AULAS/ e gerar PLANO-AULAS.md + AULA-01.md/.html para 5 matérias pendentes

---

## 📋 Escopo

### Matérias a Processar (5 total)
1. COMPETENCIAS_SOCIOEMOCIONAIS_E_EMPREENDEDORISMO
2. EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS
3. NOCOES_ELETRICIDADE_CIRCUITOS_BASICOS
4. OFICINAS_IMPRESSAO_3D_ROBOTICA
5. REFORCO_LINGUAGENS

### Arquivos a Gerar por Matéria
- ⬜ Pasta `AULAS/`
- ⬜ `PLANO-AULAS.md` (baseado em EMENTA.md existente)
- ⬜ `AULAS/AULA-01.md` (primeiro encontro)
- ⬜ `AULAS/AULA-01.html` (versão HTML da aula)

---

## 🔧 Arquivos Envolvidos

| Arquivo | Tipo | Ação |
|---------|------|------|
| COMPETENCIAS_SOCIOEMOCIONAIS_E_EMPREENDEDORISMO/ | Pasta | Criar AULAS/ + arquivos |
| EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS/ | Pasta | Criar AULAS/ + arquivos |
| NOCOES_ELETRICIDADE_CIRCUITOS_BASICOS/ | Pasta | Criar AULAS/ + arquivos |
| OFICINAS_IMPRESSAO_3D_ROBOTICA/ | Pasta | Criar AULAS/ + arquivos |
| REFORCO_LINGUAGENS/ | Pasta | Criar AULAS/ + arquivos |

---

## ⚠️ Riscos e Dependências

- ✅ Todas as matérias possuem EMENTA.md (pré-requisito atendido)
- ⚠️ Títulos e estrutura extraídos das ementas existentes
- ⚠️ Conteúdo inicial será template genérico (professores devem customizar)

---

## 📋 Passos Executáveis

### 1. COMPETENCIAS_SOCIOEMOCIONAIS_E_EMPREENDEDORISMO
- ✅ **Concluído** — Pasta AULAS/ criada
- ✅ **Concluído** — PLANO-AULAS.md gerado (9 encontros)
- ✅ **Concluído** — AULAS/AULA-01.md gerado
- ✅ **Concluído** — AULAS/AULA-01.html gerado

### 2. EXPLORACAO_CARREIRAS_INDUSTRIAIS_TECNOLOGICAS
- ✅ **Concluído** — Pasta AULAS/ criada
- ✅ **Concluído** — PLANO-AULAS.md gerado (9 encontros)
- ✅ **Concluído** — AULAS/AULA-01.md gerado
- ✅ **Concluído** — AULAS/AULA-01.html gerado

### 3. NOCOES_ELETRICIDADE_CIRCUITOS_BASICOS
- ✅ **Concluído** — Pasta AULAS/ criada
- ✅ **Concluído** — PLANO-AULAS.md gerado (9 encontros)
- ✅ **Concluído** — AULAS/AULA-01.md gerado
- ✅ **Concluído** — AULAS/AULA-01.html gerado

### 4. OFICINAS_IMPRESSAO_3D_ROBOTICA
- ✅ **Concluído** — Pasta AULAS/ criada
- ✅ **Concluído** — PLANO-AULAS.md gerado (9 encontros)
- ✅ **Concluído** — AULAS/AULA-01.md gerado
- ✅ **Concluído** — AULAS/AULA-01.html gerado

### 5. REFORCO_LINGUAGENS
- ✅ **Concluído** — Pasta AULAS/ criada
- ✅ **Concluído** — PLANO-AULAS.md gerado (16 encontros, 63h total)
- ✅ **Concluído** — AULAS/AULA-01.md gerado
- ✅ **Concluído** — AULAS/AULA-01.html gerado

---

## ✅ Verificação

- [ ] Todas as 5 pastas AULAS/ criadas
- [ ] Todos os 5 PLANO-AULAS.md gerados
- [ ] Todos os 5 AULA-01.md gerados
- [ ] Todos os 5 AULA-01.html gerados
- [ ] Commit realizado com mensagem descritiva
- [ ] graphify executado para atualizar grafo

---

**Criado em:** 2026-09-08 15:30 UTC
